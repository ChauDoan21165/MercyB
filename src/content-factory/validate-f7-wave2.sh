...
extra_count="$(find "$DIR" -maxdepth 1 -type f -name 'video-*.md' | wc -l | tr -d ' ')"
if [[ "$extra_count" != "10" ]]; then
  echo "expected exactly 10 video markdown files, found $extra_count"
  fail=1
fi
...
echo "F7 wave2 validation passed: 10 scripts, required fields, >=3 shots, Vietnamese diacritics."
