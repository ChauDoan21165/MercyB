...
for i in $(seq 51 60); do
  ...
  if ! grep -q '\[điền' "$file"; then
    echo "missing [dien] placeholder slot in video-$i.md"
    fail=1
  fi
done

wave_count="$(find "$DIR" -maxdepth 1 -type f -name 'video-*.md' | awk -F/ '/video-(5[1-9]|60)[.]md$/ { count++ } END { print count + 0 }')"
if [[ "$wave_count" != "10" ]]; then
  echo "expected exactly 10 wave6 video markdown files, found $wave_count"
  fail=1
fi
...
echo "F7 wave6 validation passed: 10 scripts, required fields, >=3 shots, Vietnamese diacritics, [dien] placeholders."
