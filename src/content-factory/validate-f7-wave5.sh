...
wave_count="$(find "$DIR" -maxdepth 1 -type f -name 'video-*.md' | awk -F/ '/video-(4[1-9]|50)[.]md$/ { count++ } END { print count + 0 }')"
if [[ "$wave_count" != "10" ]]; then
  echo "expected exactly 10 wave5 video markdown files, found $wave_count"
  fail=1
fi
...
echo "F7 wave5 validation passed: 10 scripts, required fields, >=3 shots, Vietnamese diacritics."
