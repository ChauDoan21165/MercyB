#!/usr/bin/env bash
# F2 Content Factory wave 6 validation.
...
files = {
    "interference": (root / "f2-interference-taxonomy-wave6.json", "patterns", 8),
    "family": (root / "f2-family-bridge-scripts-wave6.json", "scripts", 6),
}
...
for label, (path, key, expected) in files.items():
    ...
    if data.get("wave") != 6:
        bad.append(f"{label}: wave must be 6")
...
print("PASS: F2 wave 6 content validation")
PY
