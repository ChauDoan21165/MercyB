...
wave_count="$(find "$DIR" -maxdepth 1 -type f -name 'video-*.md' | awk -F/ '/video-(2[1-9]|30)[.]md$/ { count++ } END { print count + 0 }')"
if [[ "$wave_count" != "10" ]]; then
  echo "expected exactly 10 wave3 video markdown files, found $wave_count"
  fail=1
fi
...
echo "F7 wave3 validation passed: 10 scripts, required fields, >=3 shots, Vietnamese diacritics."
