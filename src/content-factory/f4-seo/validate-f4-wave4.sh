...
# Honesty guard: no invented score claims
score_patterns = [
    r"tăng\s+\d+[\.,]?\d*\s*band",
    r"đạt\s+band\s+\d",
    r"lên\s+band\s+\d",
    r"band\s+\d+\s+sau\s+\d+\s+(tuần|tháng|ngày)",
    r"tăng\s+\d+[\.,]?\d*\s+điểm\s+IELTS",
    r"tăng\s+\d+[\.,]?\d*\s+điểm\s+TOEIC",
    r"đạt\s+\d+\+?\s+TOEIC",
    r"đạt\s+IELTS\s+\d",
]
for pattern in score_patterns:
    if re.search(pattern, text, re.I):
        errors.append(f"honesty violation: invented score claim matches pattern '{pattern}'")
