...
export const lessons: ChineseLesson[] = [
  {
    ...
    "roleplay_prompts_en": [
      "You are a PhD candidate presenting your dissertation hypothesis to a committee of three professors. Topic: 'Do large language models improve engineers' technical-writing productivity?' (1) State the hypothesis precisely (specific, falsifiable); (2) give its theoretical basis; (3) concede one concrete limitation; (4) close with 由此可见 / 综上所述. Avoid 革命性 / 颠覆 / 突破 (too strong, not academic).",
      "You are a supervisor reading a graduate student's proposal. Their hypothesis is 'AI can replace programmers.' Respond in academic register: point out that the hypothesis is too broad, not falsifiable, with no independent variable defined, then propose a concrete way to narrow it. Use 此假设 / 该研究 / 笔者建议.",
      "You are a student writing a master's-thesis outline on 'The effectiveness of AI in teaching Chinese.' Convert the rough draft 'I think AI helps learners a lot' into C1 academic Chinese using 笔者假设 / 旨在 / 实证依据 / 局限性 — at least three sentences, each using at least one 书面 connector."
    ],
    ...
    "register_notes_en": "Register: write in 书面语, not 口语 — replace 我觉得 with 笔者认为 or the passive, prefer epistemic-hedged 可能/似乎/或许 over 一定/必然, and use formal connectors (然而, 此外, 综上所述); Chinese graders dock points for 我觉得 and for unhedged 一定 without evidence.",
    "idiom_glosses": [
      {
        "idiom": "言之有据",
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a final-year student writing your thesis. You read an interesting line in Smith (2019) — but Smith was quoting Jones (2010), and you have NOT read Jones in the original. Write the citation to standard (转引自) and explain to a classmate why you cannot simply write 'Jones (2010)' when you haven't read the original. Use 学术诚信 / 不容懈怠.",
      "You are an associate professor giving feedback on a student's draft with three passages paraphrased too closely to the English original (>70% shared keywords). Respond in Chinese academic register: identify the problem, explain its severity, and propose two concrete fixes (a deeper rewrite / converting to a direct quote). Use 严重 / 学术不端 / 建议.",
      "You are a journal editor writing a desk-reject letter for 'serious citation errors.' Write 4-5 sentences, polite but firm: thank the author + state the problem (twin reasons: missing 转引 + disguised paraphrase) + state clearly that resubmission to this journal is NOT accepted + wish them well elsewhere. Use 敬告作者 / 鉴于 / 经审议."
    ],
    ...
    "register_notes_en": "Register: every borrowed idea is cited — 直接引用 (verbatim, quotes + author + year + page), 释义引用 (paraphrase, author + year, ≥60% reworded), 转引 (secondary source, marked 转引自); 听说X说 and yearless attribution are 口语 errors a reviewer flags first.",
    "idiom_glosses": [
      {
        "idiom": "言之凿凿",
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a PhD candidate presenting a dissertation on 'AI bias.' A committee member challenges: 'bias in the dataset is unavoidable, so your study is meaningless.' Rebut in C1 academic register: (1) concede their valid point; (2) distinguish 'unavoidable' from 'cannot be mitigated'; (3) expose the implicit assumption in the question; (4) close with 此论断 / 笔者愚以为. Avoid defensiveness.",
      "You are the second reviewer of a paper. Reviewer 1 accepted; you disagree with the methodology. Write a 4-5 sentence rebuttal: (1) acknowledge the contribution; (2) name the methodology issue specifically (sample size insufficient + control group missing); (3) propose fixes; (4) recommend 'major revision.' Use 与第一审稿人观点相左 / 笔者建议 / 有待商榷.",
      "In a seminar a student asks 'Can AI be conscious?' As the lecturer you must answer WITHOUT taking a side (an open philosophical question): (1) name the two main schools; (2) give each school's argument; (3) point out the symmetry or differentia; (4) encourage the student to investigate further. Use 众说纷纭 / 各有道理 / 有待进一步研究."
    ],
    ...
    "register_notes_en": "Register: rebut in four moves — concede (您的观点不无道理) → pivot (然而 / 但笔者认为) → specific counter → constructive 建议; never ad hominem, and toward a senior raise hedging and humility (笔者愚以为, 不知可否商榷).",
    "idiom_glosses": [
      {
        "idiom": "据理力争",
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a graduate student who just got very 'clean' experimental results (p < 0.001, large effect size). Write the Discussion in C1 hedging register: (1) report the finding with neutral wording; (2) concede the sample-size limit; (3) give two alternative interpretations; (4) close with '有参考价值, 有待进一步研究'. Avoid 证明 / 必然 / 肯定.",
      "You are a reviewer reading a paper that claims 'AI tools are 100% accurate on task X.' Write a hedging-focused rebuttal: point out that '100%' is too strong a claim, request a specific confidence interval instead, and suggest a discussion of edge cases. Use 此论断或欠妥 / 笔者建议…似乎更为稳妥 / 在某种程度上.",
      "You are an engineer presenting an A/B-test result internally. Sample size 5,000 users, p = 0.04. Write a paragraph in C1 academic hedging register: (1) report the finding specifically; (2) acknowledge the p-value is near the boundary; (3) suggest replication before a 100% rollout; (4) close recommending a move from test to gradual rollout 25% → 50% → 100%."
    ],
    ...
    "register_notes_en": "Register: hedge in three layers — report fact (初步显示) → explain mechanism (似乎/可能) → extrapolate cautiously (有待进一步研究) — matching hedge strength to effect size; 1-2 hedges per claim, and never 证明/必然/完全 in interpretation (use 支持/在多数情况下/相当程度上).",
    "idiom_glosses": [
      {
        "idiom": "不无道理",
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a graduate student writing the 'Term Definitions' section of a dissertation on 'AI alignment.' Define three terms: (1) 'alignment' (Russell vs Anthropic vs Bostrom — pick one, cite it); (2) 'value learning'; (3) 'mesa-optimizer.' Use the Aristotelian structure (genus + differentia) and the format 本研究中, X 指…. Avoid 顾名思义就是.",
      "You are a reviewer responding to a paper that uses 'transformer' very loosely (sometimes the architecture, sometimes a family of models, sometimes a specific instance like BERT). Write a C1-register rebuttal: point out the inconsistency and ask the author to (a) pick one definition, (b) cite a source, (c) stay consistent throughout. Use 术语使用不一致 / 建议明确界定 / 以避免歧义.",
      "You are an AI engineer translating a paper from English to Chinese. 'embedding' appears 30+ times — sometimes a vector representation, sometimes the embedding process, sometimes a specific layer. Write a terminology note for the translator team: (1) the Chinese term for each sense (嵌入向量 / 嵌入过程 / 嵌入层); (2) when to use which; (3) the consistency rule."
    ],
    ...
    "register_notes_en": "Register: define Aristotelian-style (genus + differentia) with formal markers (本研究中X是指…, 所谓X即指…, 笔者拟将X界定为…); avoid circular 'X就是X', and never follow 顾名思义 with a full definition.",
    "idiom_glosses": [
      {
        "idiom": "顾名思义",
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a master's student presenting a thesis on 'The effectiveness of automated code review.' The professor asks: 'why did you choose quantitative over qualitative?' Answer in C1 register: (1) state the research question clearly (measuring effectiveness → quantitative); (2) acknowledge that qualitative has value but for a different question; (3) explicitly note you will add a small qualitative phase if a reviewer requests it. Use 研究问题之性质 / 各有千秋 / 相辅相成.",
      "You are a reviewer responding to a paper that uses pure quantitative methods on a question that clearly needs qualitative ('why do users abandon the app after two days'). Write a C1 rebuttal: (1) acknowledge the data analysis is rigorous; (2) point out the mismatch between research question and method; (3) suggest adding 15-20 semi-structured interviews; (4) note the paper currently 'falls short of answering its own question.' Use 方法与问题之间存在错位 / 笔者建议 / 鉴于.",
      "You are a PI writing a grant proposal on 'AI literacy in K-12 education.' Reviewer 1 is typically skeptical of qualitative methods. Write a persuasive methodology paragraph balancing quantitative (pre/post test, 500 students) and qualitative (focus groups with teachers). Use 混合方法 / 各有千秋 / 互补 / 三角验证 (triangulation)."
    ],
    ...
    "register_notes_en": "Register: method follows the research question, not taste — use contrast connectors (前者…后者, 与之相反, 较之X) and 各有千秋/各有利弊 framing; never equate 定量 with rigor or 定性 with 'soft' — each has its own rigor standards.",
    "idiom_glosses": [
      {
        "idiom": "各有千秋",
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are Reviewer 2 for a paper claiming '99.9% accuracy on medical image diagnosis.' Reviewer 1 accepted; you suspect data leakage and over-fitting. Write your review in C1 register: (1) acknowledge the potential contribution; (2) name three methodology issues with concrete evidence; (3) propose specific fixes (hold-out test set / external validation / temporal split); (4) recommend 'major revision' with a 6-week deadline. Use 此论尚需斟酌 / 笔者建议 / 鉴于.",
      "You are a PhD candidate assigned to critique a paper in reading group. It is from a famous lab and uses a community-accepted method, but you find a serious weakness. Prepare a 3-minute critique: (1) acknowledge the paper's status and contributions; (2) hedge appropriately so as not to sound disrespectful; (3) present the weakness with concrete examples; (4) pose a constructive question for the author if they are in the seminar. Use academic register + 学生愚见 / 不揣冒昧.",
      "You are an editor-in-chief writing a letter explaining a reject decision. The paper has a creative idea but execution flaws (sample size 30 instead of the needed 200, no power analysis, no comparison baseline). Write a 5-6 sentence C1-register letter: acknowledge the creativity + name the specific flaws + suggest a more suitable journal (or revise-and-resubmit) + close politely. Use 经审议 / 鉴于 / 敬告作者."
    ],
    ...
    "register_notes_en": "Register: critique in five moves — acknowledge contribution → transition (然而) → numbered issues (其一/其二) → constructive 建议 → overall verdict (瑕不掩瑜 or recommend reject); grade softness to severity and attribute to the text (本文/该研究…), never the author.",
    "idiom_glosses": [
      {
        "idiom": "瑕不掩瑜",
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a PhD candidate in your defense. An evaluator asks a question you didn't see coming: 'if someone replicates your results and fails, how do you explain it?' Answer in C1 Q&A register: (1) thank them for the question; (2) acknowledge the possibility; (3) provide concrete falsifiability conditions; (4) commit to making code/data open. Avoid defensiveness. Use 您所提之问题极具启发性 / 笔者亦深以为….",
      "You are a speaker at an international AI conference. An audience member challenges your dataset source (suggesting data leakage). You aren't 100% sure but suspect there is. Answer honestly yet gracefully: (1) thank them; (2) acknowledge the concern is valid; (3) say you will verify post-conference; (4) invite continued dialogue. Avoid defensiveness. Use 诚如您所言 / 限于…笔者拟 / 不吝赐教.",
      "You are a junior researcher asking your very first conference question — nervous, addressing a famous-lab speaker. Phrase it humble + technically substantive: (1) self-introduce briefly; (2) acknowledge the speaker's contribution; (3) ask the actual technical question (an edge case in their methodology); (4) end with 不揣冒昧 / 不知当否, 请赐教."
    ],
    ...
    "register_notes_en": "Register: open every answer with thanks + acknowledgment (感谢评委指出, 诚如您所言), use the humble 笔者 / 笔者愚以为, and for an unknown concede the gap (笔者尚未深入考虑) — never fake an answer, never get defensive; 抛砖引玉 is the canonical humility frame.",
    "idiom_glosses": [
      {
        "idiom": "抛砖引玉",
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a graduate student who just finished a paper on 'AI fairness in hiring.' Write a 200-character Chinese abstract following the four elements (background → method → result → conclusion). The first sentence must state the contribution. Avoid 近年来 / 随着…的发展. Use 本文提出 / 实验表明 / 综上所述.",
      "You are an editor asked to review an abstract submitted to an ACM journal. It is 380 characters, has no numbers, and opens with '近年来deep learning在各个领域取得了显著进步'. Write 4-5 sentences of C1 feedback: (1) over the length limit; (2) the opening sentence is vacuous; (3) no quantitative results; (4) suggest concretely — cut to 200 characters, lead with the method name + a benchmark number. Use 过于冗长 / 信息密度不足 / 建议.",
      "You are a student writing a literature review for a master's thesis on 'Reinforcement Learning from Human Feedback.' Write the lit-review introduction paragraph (150 characters): (1) state the scope; (2) introduce the 3-4 main schools (RLHF / DPO / Constitutional AI); (3) state where your work fits. Use 提纲挈领 / 脉络 / 流派分歧 / 在此脉络中, 本研究…."
    ],
    ...
    "register_notes_en": "Register: an abstract is four elements (background+gap / contribution / method / results with ≥2 numbers) — the opener 近年来随着X的迅猛发展… is banned; a literature review groups the field into 2-4 schools (提纲挈领), never a chronological 堆砌 of one-line summaries.",
    "idiom_glosses": [
      {
        "idiom": "开宗明义",
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a reviewer for a paper on 'Object Detection in Autonomous Vehicles.' The idea is good (novel multi-modal fusion) and the data is solid, but (1) there is no fair baseline comparison, (2) the ablation study tests only 1/3 of the components, (3) the generalization claim is too broad (tested on one dataset). Write a complete review letter (~250 characters): opening acknowledgment + 3 numbered critical issues + 2 minor comments + decision (major revision). Use 本稿选题前沿 / 笔者建议 / 兹提出三点.",
      "You are an editor writing a reject letter. Reviewer 1 said accept, Reviewer 2 reject, Reviewer 3 major revision. You decide reject due to fundamental methodology flaws. Write to the author: (1) thank them for the submission; (2) summarize the review consensus; (3) name three fundamental issues specifically; (4) encourage resubmission to an alternative venue. Tone: professional, no personal attack, no 'sorry but no.' Use 经审议 / 鉴于 / 不便接受.",
      "You are a reviewer responding to a paper from a famous lab (a NeurIPS chair) — high reputational pressure. You find a real data inconsistency (Table 3 and Table 5 contradict each other). Write a review that flags it without seeming disrespectful: (1) open by acknowledging the lab's contribution to the field; (2) point out the inconsistency with clear evidence (page/table numbers); (3) request clarification (do not claim 'this is wrong'); (4) decision: minor revision. Use 请作者核实 / 似存在不一致 / 实事求是."
    ],
    ...
    "register_notes_en": "Register: a review letter runs acknowledge → summarize (proves you read it) → genuine positives → numbered major issues → minor → clear decision; use 笔者 or the passive, never 我认为, no emotional language (令人失望) or sarcasm, and prioritize issues rather than blending them.",
    "idiom_glosses": [
      {
        "idiom": "字斟句酌",
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a postdoc at HKUST preparing an RGC ECS (Early Career Scheme) application. Present the proposal in 3 minutes to your Doktorvater: (1) a specific research question (with verification conditions); (2) an open-source / dataset contribution; (3) one clear HK-relevant item — not surface 'tagged HK.' Use 笔者所提议 / 填补…空白 / 经费预算共计.",
      "You are Prof. Chen reviewing a postdoc's weak-ish proposal for Singapore NRF AcRF Tier 2. The reviewer praises the creativity but criticizes 'Plan B missing / scope too broad.' Express the feedback formally: name the problems and propose narrowing — WITHOUT blunt words like 'wrong / weak'; use academic euphemism (有待进一步…).",
      "You are a researcher moving from a Taiwan university (NTHU) to HKUST. In an exchange with a Singapore senior collaborator (Prof. Tan at NTU), express: (a) the reason for changing funding environment (NSTC → RGC + NRF collaboration); (b) how to split authorship if you co-apply to NRF AcRF Tier 3. Use 书面 register — NOT colloquial."
    ],
    ...
    "register_notes_en": "Register: scope must be falsifiable-specific (not 'AI能改变教育'), never call your own proposal 颠覆性/突破 (reviewers judge that, not the PI), and frame to the funder's stated priority; formal verbs 旨在/拟在/填补…空白, and a Plan B that preserves scientific value.",
    "idiom_glosses": [
      { "idiom": "未雨绸缪", "literal": "trước mưa chuẩn bị (wèi yǔ chóu móu)", "meaning": "Chuẩn bị sẵn trước khi có vấn đề. Dùng trong Chapter 8 (Risk Assessment): '本计划在第八章未雨绸缪, 提出风险预案' (kế hoạch này chuẩn bị sẵn ở chương 8, đưa ra phương án dự phòng rủi ro). (书面/正式)", "example": "RGC评审看重未雨绸缪的态度, 故笔者特设第八章讨论风险与备选方案。", "literal_en": "prepare before it rains (wèi yǔ chóu móu)", "meaning_en": "To prepare in advance before trouble arises — used in Chapter 8 (Risk Assessment): 本计划在第八章未雨绸缪, 提出风险预案 ('this plan prepares ahead in Chapter 8, proposing risk contingencies'). (written/formal)", "example_en": "RGC reviewers value a prepare-ahead attitude, so I have devoted a dedicated Chapter 8 to discussing risks and alternative plans." },      { "idiom": "厚积薄发", "literal": "tích sâu phát mỏng (hòu jī bó fā)", "meaning": "Tích lũy lâu dài rồi mới phát. Dùng để diễn đạt rằng PI đã có nền tảng tốt trước khi đề xuất nghiên cứu mới. Cụm khen ngợi cao trong giới học thuật. (书面)", "example": "笔者过去六年在低资源语言NLP领域之研究, 厚积薄发, 始有此次研究构想。", "literal_en": "accumulate thickly, release thinly (hòu jī bó fā)", "meaning_en": "To accumulate deeply over years before releasing a little — used to convey that the PI has a solid foundation before proposing new research; high praise in academia. (written)", "example_en": "My past six years of research in low-resource-language NLP — long accumulation finally bearing fruit — gave rise to the present research idea." },      { "idiom": "管中窥豹", "literal": "nhìn báo qua ống (guǎn zhōng kuī bào)", "meaning": "Nhìn qua một góc hẹp, không thấy toàn cảnh — academic euphemism cho 'nghiên cứu hạn chế'. Dùng khi acknowledge limitation trong proposal. (书面)", "example": "本研究虽聚焦三种东南亚语言, 仍属管中窥豹, 后续工作可扩展至更广语种。", "literal_en": "peer at a leopard through a tube (guǎn zhōng kuī bào)", "meaning_en": "To see only a narrow part, not the whole — an academic euphemism for 'limited in scope', used when acknowledging a limitation in a proposal. (written)", "example_en": "Although this study focuses on three Southeast Asian languages, it remains a partial glimpse; follow-up work can extend to a wider range of languages." },      { "idiom": "言简意赅", "literal": "lời gọn ý đầy đủ (yán jiǎn yì gāi)", "meaning": "Diễn đạt ngắn gọn nhưng đầy đủ ý — tiêu chuẩn proposal-writing. Hàm ý: reviewer chỉ có 30 phút đọc proposal, viết dài lê thê = bị loại. (书面)", "example": "RGC计划书贵在言简意赅, 二十页之内务必涵盖问题、方法、风险三大要素。", "literal_en": "words concise, meaning complete (yán jiǎn yì gāi)", "meaning_en": "Concise yet complete in meaning — a proposal-writing standard; the implication is that a reviewer has only 30 minutes, so long-winded writing gets rejected. (written)", "example_en": "An RGC proposal succeeds by being concise yet complete: within twenty pages it must cover the three pillars of problem, method, and risk." }    ],
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a PhD candidate about to defend at HKUST. The committee has three members: an HKUST internal, an NUS Singapore external, and an NTU Taiwan external. Open formally in 30 seconds: '感谢答辩委员会拨冗审阅本论文' + a summary of three parts (research question / main findings / limitations). Do NOT exceed 25 minutes.",
      "An external examiner points out you over-generalize in the conclusion ('all low-resource languages' when you tested only 3). Respond formally: '笔者必须坦承' (I must concede) + a concrete fix (narrow the scope or add an experiment). Do NOT be defensive.",
      "An examiner challenges that p=0.04 is not 'significant' enough. Express a hedged rebuttal: '笔者并不否认现有方法存在局限, 但若以此否定整篇论文之贡献, 则似乎稍嫌严苛' + offer to soften 'significant' to 'statistically observable improvement.' Structure: defend, but be willing to compromise."
    ],
    ...
    "register_notes_en": "Register: never defensive — 笔者必须坦承 scores higher than 'I disagree'; answer critique in five moves (thanks → where already addressed → concede → propose → revision plan), with 谦词 (拨冗审阅, 请教各位, 指正) and 笔者, never 我.",
    "idiom_glosses": [
      { "idiom": "受益匪浅", "literal": "lợi ích không nhỏ (shòu yì fěi qiǎn)", "meaning": "Học hỏi được nhiều — cụm 谦词 dùng khi nói cảm ơn ủy viên defense. (书面/谦词)", "example": "感谢各位委员的指正, 笔者受益匪浅, 必将认真汲取。", "literal_en": "benefit not shallow (shòu yì fěi qiǎn)", "meaning_en": "To have learned a great deal — a humble phrase used when thanking defense committee members. (written/humble)", "example_en": "I thank the committee for your corrections; I have benefited greatly and will conscientiously take them on board." },      { "idiom": "字斟句酌", "literal": "cân nhắc từng chữ từng câu (zì zhēn jù zhuó)", "meaning": "Cân nhắc từng chữ — diễn đạt sự thận trọng cao trong viết lại sau defense. (书面)", "example": "修订版中, 笔者将字斟句酌, 确保结论之表述与证据相符。", "literal_en": "weigh each word, deliberate each phrase (zì zhēn jù zhuó)", "meaning_en": "To weigh every word — conveys great care when rewriting after a defense. (written)", "example_en": "In the revised version I will weigh every word, ensuring the wording of the conclusion matches the evidence." },      { "idiom": "言之凿凿", "literal": "lời nói chắc nịch (yán zhī záo záo)", "meaning": "Phát biểu khẳng định vững chắc. Trong defense thường dùng để CHỈ TRÍCH: 'X之结论言之凿凿, 然证据基础尚显薄弱' (X kết luận chắc nịch, nhưng cơ sở chứng cứ còn mỏng). Hàm ý: over-confident. (书面)", "example": "笔者第三章某些结论言之凿凿, 在评委指正后已修正为更保守之表述。", "literal_en": "speech is rock-solid (yán zhī záo záo)", "meaning_en": "To state something with firm certainty; in a defense it is often used as CRITICISM — X之结论言之凿凿, 然证据基础尚显薄弱 ('X's conclusion is stated firmly, yet its evidentiary basis is still thin'), implying over-confidence. (written)", "example_en": "Some conclusions in my Chapter 3 were stated too assertively; after the committee's corrections they have been revised to more conservative wording." },      { "idiom": "斟酌损益", "literal": "cân nhắc thêm bớt (zhēn zhuó sǔn yì)", "meaning": "Cân nhắc lợi hại — gốc Tam Quốc Diễn Nghĩa, dùng để diễn đạt trade-off khi sửa luận án. (书面/古典)", "example": "针对评委之多重建议, 笔者将斟酌损益, 重点处理影响外部效度的两点。", "literal_en": "weigh what to add and what to cut (zhēn zhuó sǔn yì)", "meaning_en": "To weigh gains against losses — from the Romance of the Three Kingdoms, used to express the trade-offs when revising a dissertation. (written/classical)", "example_en": "Faced with the committee's many suggestions, I will weigh gains and losses, prioritizing the two points that affect external validity." }    ],
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are the chair of a colloquium at HKUST with 60 attendees and a speaker from NTU Singapore. Open formally in 90 seconds: '诸位同仁, 欢迎莅临' + introduce the speaker (three points: position, contributions, today's topic) + the format (45+15 min). Do NOT read a long CV.",
      "You are a keynote speaker at NCKU Taiwan. In the thanks-to-funders portion, thank NSTC + NUS NRF + a collaborator. Express it formally: '请允许笔者借此机会感谢…对本项目之资助, 此一资助使我们得以…'. Do NOT omit the specific use of funds (postdocs / compute / travel).",
      "When an audience question runs too long (>3 minutes), as chair you must interrupt politely: '感谢张教授的提问 — 由于时间关系, 请您简明扼要地提出关键点'. This is expected, not rude. Practice the cut-off line."
    ],
    ...
    "register_notes_en": "Register: chair/lecture with 书面 formal openers (诸位同仁, 欢迎莅临, 致以热烈掌声 — not 大家好/谢谢), thank funders with a concrete use, and cut over-long questions politely (由于时间关系, 请您简明扼要).",
    "idiom_glosses": [
      { "idiom": "百闻不如一见", "literal": "trăm nghe không bằng một thấy (bǎi wén bù rú yī jiàn)", "meaning": "Trăm lần nghe không bằng một lần thấy. Chair dùng khi giới thiệu speaker famous: '陈教授的研究, 早已闻名学界, 真所谓百闻不如一见'. (书面/古典)", "example": "陈教授的研究成果早已闻名两岸, 今日得以亲聆讲座, 真所谓百闻不如一见。", "literal_en": "a hundred hearings are not worth one seeing (bǎi wén bù rú yī jiàn)", "meaning_en": "Hearing a hundred times is not as good as seeing once — used by a chair introducing a famous speaker: 陈教授的研究, 早已闻名学界, 真所谓百闻不如一见 ('Prof. Chen's work is long renowned; truly, seeing beats hearing'). (written/classical)", "example_en": "Prof. Chen's work has long been renowned on both sides of the strait; to hear the lecture in person today truly proves that one seeing is worth a hundred hearings." },      { "idiom": "言之有物", "literal": "lời nói có nội dung (yán zhī yǒu wù)", "meaning": "Phát biểu có substance, không rỗng tuếch. Cao khen cho keynote tốt. (书面)", "example": "陈教授今日之报告言之有物, 既有理论深度, 又有实证支撑。", "literal_en": "speech has substance (yán zhī yǒu wù)", "meaning_en": "A talk with real substance, not empty rhetoric — high praise for a good keynote. (written)", "example_en": "Prof. Chen's talk today had real substance — both theoretical depth and empirical support." },      { "idiom": "抛砖引玉", "literal": "ném gạch dẫn ngọc (pāo zhuān yǐn yù)", "meaning": "Tự khiêm 'tôi nêu ý sơ để dẫn ý hay của các vị'. Speaker dùng cuối talk khi mời discussion: '笔者所言不过抛砖引玉, 期待诸位高见'. (书面/谦词)", "example": "笔者今日所述, 仅为抛砖引玉, 真知灼见还需诸位同仁补充。", "literal_en": "throw a brick to draw jade (pāo zhuān yǐn yù)", "meaning_en": "A self-deprecating 'I offer a crude idea to draw out your better ones' — used by a speaker closing a talk to invite discussion: 笔者所言不过抛砖引玉, 期待诸位高见 ('my remarks are but a brick to draw your jade; I await your insights'). (written/humble)", "example_en": "What I have presented today is merely a brick to draw out jade; the real insights must be supplied by my colleagues." },      { "idiom": "高屋建瓴", "literal": "đứng trên nhà cao đổ nước (gāo wū jiàn líng)", "meaning": "Nhìn từ cao xuống, có tầm nhìn lớn. Chair dùng để khen keynote: '陈教授高屋建瓴, 把领域过去十年勾勒得极为清晰'. (书面/古典)", "example": "陈教授对神经形态计算的回顾高屋建瓴, 既见树木, 又见森林。", "literal_en": "pour water from a high rooftop (gāo wū jiàn líng)", "meaning_en": "A commanding view from on high, a sweeping perspective — used by a chair to praise a keynote: 陈教授高屋建瓴, 把领域过去十年勾勒得极为清晰 ('Prof. Chen took a commanding view, mapping the field's past decade with great clarity'). (written/classical)", "example_en": "Prof. Chen's review of neuromorphic computing took a commanding view — seeing both the trees and the forest." }    ],
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a postdoc at NTU Singapore (NRF-funded). On a call with a PI at HKUST (RGC-funded), discussing co-authorship for a NeurIPS submission, open formally: '在确定共同作者之前, 我们有必要先就CRediT分类法之十四类贡献达成一致'. Propose a concrete split of the 14-item CRediT and justify first authorship.",
      "The PI proposes adding a Taiwan researcher (NTHU) merely for 'collaboration with Taiwan.' You are uncomfortable — they only provided one dataset and do not meet ICMJE criteria. Express it formally: 'Tan博士之贡献应限于Acknowledgment一栏, 未达ICMJE之三标准, 列为作者恐有学术诚信顾虑'. Do NOT be aggressive.",
      "After publication, a co-author wants to add an industry sponsor's name with no substantial intellectual contribution (funding via gift only). Express a formal denial: '未参与研究核心工作之资助方, 应于Acknowledgment一栏致谢, 而非列为作者; 此乃ICMJE与CRediT之共同立场'."
    ],
    ...
    "register_notes_en": "Register: authorship follows ICMJE's three criteria + the CRediT taxonomy, not negotiation — gift/ghost/honorary authorship is a research-integrity violation; resolve disputes through the institutional channel, never the journal editor first or social media.",
    "idiom_glosses": [
      { "idiom": "明算账", "literal": "tính rõ tài khoản (míng suàn zhàng)", "meaning": "Tính sổ rõ ràng — friend / partner mà tính rõ thì không tổn hại quan hệ. Trong co-authorship: 'CRediT tốt = 朋友是朋友, 论文是论文, 明算账'. (口语 nhưng dùng được trong informal academic discussion)", "example": "陈教授常说: 学术合作就要明算账, 把14项CRediT列清, 对大家都好。", "literal_en": "settle accounts clearly (míng suàn zhàng)", "meaning_en": "To keep clear accounts — even between friends, clear reckoning does not harm the relationship; in co-authorship: good CRediT = 朋友是朋友, 论文是论文, 明算账 ('friends are friends, papers are papers, keep the accounts clear'). (spoken, but usable in informal academic discussion)", "example_en": "Prof. Chen often says: academic collaboration calls for clear accounts — listing all 14 CRediT roles plainly is better for everyone." },      { "idiom": "实至名归", "literal": "thực đến tên về (shí zhì míng guī)", "meaning": "Có thực tế (đóng góp) thì tên gọi (tác giả) về — deserved recognition. Dùng để khen author order fair: 'Linh获第一作者, 实至名归'. (书面)", "example": "Linh所做之核心贡献清晰可数, 列为第一作者实至名归。", "literal_en": "substance arrives, the name follows (shí zhì míng guī)", "meaning_en": "Where the substance (contribution) is real, the name (authorship) rightly follows — well-deserved recognition; used to commend a fair author order: Linh获第一作者, 实至名归 ('Linh getting first author is well deserved'). (written)", "example_en": "Linh's core contributions are clearly countable; listing her as first author is thoroughly deserved." },      { "idiom": "公道自在人心", "literal": "công đạo ở lòng người (gōng dào zì zài rén xīn)", "meaning": "Sự công bằng tự ở lòng người — đã làm gì, mọi người tự biết. Dùng khi resolved authorship dispute fairly: '我们按CRediT透明分配, 公道自在人心'. (书面/古典)", "example": "若每位作者贡献均有文档化, 公道自在人心, 何须事后争议?", "literal_en": "justice resides in people's hearts (gōng dào zì zài rén xīn)", "meaning_en": "Fairness lives in people's hearts — everyone knows who did what; used when an authorship dispute has been settled fairly: 我们按CRediT透明分配, 公道自在人心 ('we allocated transparently by CRediT; justice is in people's hearts'). (written/classical)", "example_en": "If every author's contribution is documented, fairness will be self-evident — why should there be any dispute afterward?" },      { "idiom": "丑话说在前头", "literal": "lời xấu nói trước (chǒu huà shuō zài qián tou)", "meaning": "Nói khó nghe trước — proactive disclosure of difficult terms. Dùng khi mở đầu CRediT discussion: '丑话说在前头, 我们先谈作者顺序, 免得日后伤和气'. (口语 nhưng acceptable academic informal)", "example": "丑话说在前头: 谁做了什么贡献, 我们现在写清楚, 比paper发表后再吵更好。", "literal_en": "say the unpleasant words up front (chǒu huà shuō zài qián tou)", "meaning_en": "To state the awkward things up front — proactive disclosure of hard terms; used to open a CRediT discussion: 丑话说在前头, 我们先谈作者顺序, 免得日后伤和气 ('let me say the hard part first — let's settle author order now so we don't sour the relationship later'). (spoken, but acceptable in informal academic use)", "example_en": "Let me say the awkward part up front: writing down now who contributed what is better than quarreling after the paper is published." }    ],
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a job candidate for a tenure-track position at NUS / NTU. Give a 5-minute summary of your teaching philosophy + a sample syllabus for a CS course you would teach. Include: (1) a concrete tiered-assessment design; (2) an explicit AI/ChatGPT policy; (3) at least one mechanism for diverse-background students.",
      "The search committee chair (Prof. Lim) challenges your 30% project weight as 'too high' and the workload of the tiered assignments. Respond formally with data: justify the engineering-heavy nature + the 2-stage iteration + actual TA hours + auto-test scripts. Do NOT be defensive.",
      "Draft a formal email to a senior colleague at HKUST requesting a teaching-statement sample. Open with '尊敬的X教授' + the reason (an interview at NUS / HK Polytechnic) + exactly what you ask for (a 1-2 page sample). Close with 不胜感激."
    ],
    ...
    "register_notes_en": "Register: write a syllabus/teaching-statement in international academic register (rubric, learning outcomes), with an explicit AI-tool policy (2024+) and a formal integrity clause (依…学术诚信条例处理); in HK/SG/Taiwan contexts avoid mainland-flavor terms (深入贯彻, 加强, 抓紧).",
    "idiom_glosses": [
      { "idiom": "因材施教", "literal": "tùy tài mà dạy (yīn cái shī jiào)", "meaning": "Dạy theo năng lực mỗi sinh viên — gốc Khổng Tử (Lunyu). Dùng trong teaching statement: '笔者主张因材施教, 反对一刀切'. (书面/古典)", "example": "因材施教是儒家教学传统, 在多元背景的国际班级中尤为重要。", "literal_en": "teach according to aptitude (yīn cái shī jiào)", "meaning_en": "To teach according to each student's ability — from Confucius (the Analects); used in a teaching statement: 笔者主张因材施教, 反对一刀切 ('I advocate teaching to aptitude and oppose a one-size-fits-all approach'). (written/classical)", "example_en": "Teaching according to aptitude is a Confucian pedagogical tradition, and is especially important in an international class of diverse backgrounds." },      { "idiom": "诲人不倦", "literal": "dạy người không mệt (huì rén bù juàn)", "meaning": "Dạy người không biết mệt — từ Lunyu, mô tả tinh thần giảng viên tận tụy. Dùng formal trong teaching statement: '笔者愿以诲人不倦之精神, 投身教学'. (书面/古典)", "example": "教学相长, 诲人不倦, 这是笔者一贯之教育信念。", "literal_en": "instruct others tirelessly (huì rén bù juàn)", "meaning_en": "To teach others without growing weary — from the Analects, describing a devoted teacher's spirit; used formally in a teaching statement: 笔者愿以诲人不倦之精神, 投身教学 ('I will devote myself to teaching in a tireless spirit'). (written/classical)", "example_en": "Teaching and learning advance together, instructing others tirelessly — this has been my consistent educational conviction." },      { "idiom": "授人以渔", "literal": "dạy người câu cá (shòu rén yǐ yú)", "meaning": "Cho cá không bằng dạy cách câu — teach skill, not just knowledge. Dùng trong teaching philosophy: '笔者主张授人以渔之教学理念'. (书面/古典)", "example": "工程类课程更应授人以渔, 让学生掌握终身学习的方法。", "literal_en": "teach a person to fish (shòu rén yǐ yú)", "meaning_en": "Giving a fish is not as good as teaching how to fish — teach the skill, not just the knowledge; used in a teaching philosophy: 笔者主张授人以渔之教学理念 ('I advocate a teach-them-to-fish pedagogy'). (written/classical)", "example_en": "Engineering courses should all the more teach students to fish — equipping them with methods for lifelong learning." },      { "idiom": "教学相长", "literal": "dạy và học cùng lớn lên (jiào xué xiāng zhǎng)", "meaning": "Dạy và học cùng tiến bộ — gốc Lễ Ký (Liji). Dùng formal: '通过与学生互动, 笔者亦获益良多, 真所谓教学相长'. (书面/古典)", "example": "教学相长之精神, 让笔者从学生中获得新的研究灵感。", "literal_en": "teaching and learning grow each other (jiào xué xiāng zhǎng)", "meaning_en": "Teaching and learning advance together — from the Book of Rites (Liji); used formally: 通过与学生互动, 笔者亦获益良多, 真所谓教学相长 ('through interacting with students I too gain much — truly, teaching and learning grow together'). (written/classical)", "example_en": "The spirit of teaching-and-learning-together lets me draw new research inspiration from my students." }    ],
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a supervisor who has just found a student with a Turnitin similarity of 73%. Draft a formal email inviting the student to a private meeting. Open: '王同学, 关于您论文第三章, 笔者注意到一处需要进一步澄清之事项, 请您于下周三上午十点至本人办公室面谈'. NO accusatory tone, and do NOT email at 11pm.",
      "In the meeting, the student admits paraphrasing but didn't know they had to cite. Apply the educational-handling formula: rewrite + workshop + 3-month timeline + documentation. Express it formally: '此一不足应予教育性处理, 而非纪律性处分'. Document everything in the supervisor file.",
      "You have a potential COI because the student cites your own preprint (an old version). Draft a formal COI declaration to the department head: '谨此披露一项潜在利益冲突: 学生张某之论文引用了笔者本人之pre-publication作品…'. Cc the RIE Office. This is proactive disclosure, not defensive."
    ],
    ...
    "register_notes_en": "Register: name misconduct formally (学术不端, 涉嫌, 披露, 依既定程序处理), never casually (这家伙作弊); the supervisor only gathers evidence — the Research Integrity Office adjudicates, and nothing is disclosed publicly before the investigation concludes.",
    "idiom_glosses": [
      { "idiom": "防患未然", "literal": "phòng họa khi chưa thành (fáng huàn wèi rán)", "meaning": "Phòng tai họa trước khi xảy ra. Trong context ethics: '严格的IRB审查可防患未然, 避免事后撤稿'. (书面/古典)", "example": "学术伦理审查正是为防患未然——在数据收集前阻断潜在问题。", "literal_en": "guard against trouble before it forms (fáng huàn wèi rán)", "meaning_en": "To forestall disaster before it happens — in an ethics context: 严格的IRB审查可防患未然, 避免事后撤稿 ('rigorous IRB review heads off trouble in advance, avoiding later retraction'). (written/classical)", "example_en": "Research-ethics review exists precisely to forestall trouble — blocking potential problems before data collection." },      { "idiom": "实事求是", "literal": "căn cứ sự thật mà tìm chân lý (shí shì qiú shì)", "meaning": "Khách quan, không thiên vị. Dùng trong investigation: 'Investigation必须实事求是, 不预设立场'. (书面)", "example": "调查涉嫌学术不端事件, 必须实事求是, 既不护短, 也不预判。", "literal_en": "seek truth from facts (shí shì qiú shì)", "meaning_en": "Objective and unbiased — an investigation 必须实事求是, 不预设立场 ('must proceed from facts, with no preconceived stance'). (written)", "example_en": "Investigating a suspected research-misconduct case must proceed from facts — neither shielding wrongdoing nor prejudging." },      { "idiom": "言行一致", "literal": "lời và việc nhất quán (yán xíng yī zhì)", "meaning": "Nói được làm được — basic ethical standard. Dùng để mô tả researcher reliability. (书面)", "example": "学术诚信的基础在于言行一致——发表的方法须与实际做法相符。", "literal_en": "word and deed in accord (yán xíng yī zhì)", "meaning_en": "Saying and doing align — a basic ethical standard, used to describe a researcher's reliability. (written)", "example_en": "The foundation of research integrity is consistency between word and deed — the published method must match what was actually done." },      { "idiom": "玉不琢不成器", "literal": "ngọc không mài không thành đồ vật (yù bù zhuó bù chéng qì)", "meaning": "Ngọc không mài giũa không thành đồ — junior researcher cần được formative ethics education. Dùng trong educational handling: '此次教育性处理, 正是让学生从无心抄袭中学会规范——玉不琢不成器'. (书面/古典)", "example": "对于无心违规的硕士生, 教育性处理胜过纪律处分——玉不琢不成器。", "literal_en": "jade uncarved becomes no vessel (yù bù zhuó bù chéng qì)", "meaning_en": "Jade that is not carved never becomes a vessel — a junior researcher needs formative ethics education; used of an educational (rather than punitive) response: 此次教育性处理, 正是让学生从无心抄袭中学会规范——玉不琢不成器 ('this educational handling lets the student learn the norms from an unintentional plagiarism — uncarved jade makes no vessel'). (written/classical)", "example_en": "For a master's student who broke a rule unintentionally, an educational response beats a disciplinary penalty — jade must be carved to become a vessel." }    ],
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are on an interdisciplinary project (CS + Linguistics + Public Health). Open the mid-project meeting: point out 'team progress is uneven; the three field members work in parallel without converging.' Propose three mechanisms: a biweekly seminar, a concept glossary, pre-submission co-review. Express it formally: '本项目横跨…三个领域, 因此团队成员之间须建立共同之概念词汇'.",
      "A senior collaborator (Tan, NTU Linguistics) is last author per linguistics convention, but their real contribution is cross-field coordinator. Express the authorship arrangement for Tan: 'last author… 我接受这个安排, 但我会写一段Author Roles声明清楚我的具体contribution'. A multi-disciplinary team needs an explicit Author Roles statement.",
      "You submit the same research to three different venues (Nature Human Behaviour synthetic, ACL CS, Lancet Digital Health public health). Co-PI Kumar warns: 'same paper = self-plagiarism risk.' Respond formally: 'Strategy: 先投X作为flagship, 其它两个写成companion papers, 重点不同'."
    ],
    ...
    "register_notes_en": "Register: surface each field's evidence standards and vocabulary early (CS benchmarks vs linguistics description vs public-health RCT hierarchy); avoid 各自为政, pick one authorship convention with an explicit Author Roles statement, and never 同步投稿 the same paper to multiple venues.",
    "idiom_glosses": [
      { "idiom": "兼容并蓄", "literal": "vừa dung vừa chứa (jiān róng bìng xù)", "meaning": "Tiếp nhận cả hai, không loại trừ — đón nhận đa dạng phương pháp. (书面/古典)", "example": "跨学科研究须兼容并蓄, 各方法论各有所长。", "literal_en": "embrace and hold all together (jiān róng bìng xù)", "meaning_en": "To take in all and exclude none — embracing a diversity of methods. (written/classical)", "example_en": "Interdisciplinary research must be inclusive of all approaches, since each methodology has its own strengths." },      { "idiom": "异曲同工", "literal": "khúc khác công cùng (yì qǔ tóng gōng)", "meaning": "Cách khác nhưng đạt cùng kết quả — different approaches reaching same insight. (书面/古典)", "example": "公卫与语言学对'语言可及性'之研究虽方法不同, 然其结论异曲同工。", "literal_en": "different tunes, equal skill (yì qǔ tóng gōng)", "meaning_en": "Different methods achieving the same result — different approaches reaching the same insight. (written/classical)", "example_en": "Public health and linguistics study 'language accessibility' by different methods, yet their conclusions converge." },      { "idiom": "求同存异", "literal": "tìm điểm chung, giữ điểm khác (qiú tóng cún yì)", "meaning": "Find common ground while preserving differences — interdisciplinary collaboration ethos. (书面)", "example": "本项目三方合作之精神在于求同存异——共建概念词汇, 但保持各field之独立standards。", "literal_en": "seek common ground, keep differences (qiú tóng cún yì)", "meaning_en": "Find common ground while preserving differences — the ethos of interdisciplinary collaboration. (written)", "example_en": "The spirit of this project's three-way collaboration is to seek common ground while keeping differences — jointly building a shared conceptual vocabulary while each field retains its own independent standards." },      { "idiom": "鸡同鸭讲", "literal": "gà với vịt nói chuyện (jī tóng yā jiǎng)", "meaning": "Nói chuyện không hiểu nhau — what happens when interdisciplinary team lacks shared vocabulary. (口语 / informal academic)", "example": "若不先建立共同之概念词汇, 跨学科会议就成了鸡同鸭讲。", "literal_en": "a chicken talking to a duck (jī tóng yā jiǎng)", "meaning_en": "Talking past each other — what happens when an interdisciplinary team lacks a shared vocabulary. (spoken / informal academic)", "example_en": "Without first establishing a shared conceptual vocabulary, an interdisciplinary meeting becomes a chicken talking to a duck." }    ],
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are a 6th-year assistant professor about to submit your tenure case at Stanford/Berkeley/MIT. Present the three narrative arcs to the Dean: (1) PhD topic → independent direction; (2) external recognition (NSF CAREER + Sloan); (3) mentoring impact (PhD students placed at R1). Express it formally: '笔者今欲就…贡献, 申请由X晋升Y'.",
      "The Dean warns about the 'independence trap': the committee will ask 'is your PhD advisor still a co-author?' Express a formal defense with concrete numbers: 'X papers in 6 years, advisor co-author on only Y (all early-career, none since).' Numbers beat narrative when proving independence.",
      "External-letter strategy: the Dean recommends 10 'home-team friends'; you must list 5 'arms-length' (never co-authored). Propose Prof. Tan/Kumar/Chen/Lim/Akinwumi from HK/SG/Taiwan/Stanford. Justify the diverse pool: 4 international + 1 internal. The tenure case emphasizes impact beyond the lab/department."
    ],
    ...
    "register_notes_en": "Register: frame the case around research independence, external recognition, and mentoring impact in formal terms (晋升, 累计, 主持, 反映对学界与机构之承担), not casual self-praise (我做了很多); quantify with cumulative numbers and never inflate.",
    "idiom_glosses": [
      { "idiom": "厚积薄发", "literal": "tích sâu phát mỏng (hòu jī bó fā)", "meaning": "Tích lũy lâu rồi phát một lần — tenure case là kết quả 6 năm tích lũy. (书面/古典)", "example": "Tenure case乃六年厚积薄发之结晶。", "literal_en": "accumulate thickly, release thinly (hòu jī bó fā)", "meaning_en": "Long accumulation, then a single release — a tenure case is the fruit of six years of accumulation. (written/classical)", "example_en": "A tenure case is the crystallization of six years of deep accumulation finally released." },      { "idiom": "独当一面", "literal": "tự đảm đương một mặt (dú dāng yī miàn)", "meaning": "Độc lập gánh vác một mặt — independence proven. Dùng trong research statement: '过去六年, 笔者已能独当一面, 主持NSF CAREER项目'. (书面)", "example": "笔者在过去六年已能独当一面, 主持外部资助项目, 培养博士生进入学界。", "literal_en": "shoulder one front alone (dú dāng yī miàn)", "meaning_en": "To carry a whole front independently — proven independence; used in a research statement: 过去六年, 笔者已能独当一面, 主持NSF CAREER项目 ('over the past six years I have been able to stand on my own, leading an NSF CAREER project'). (written)", "example_en": "Over the past six years I have been able to stand on my own — leading externally funded projects and training PhD students into academia." },      { "idiom": "众望所归", "literal": "kỳ vọng của mọi người đều hướng về (zhòng wàng suǒ guī)", "meaning": "Mọi người tin tưởng kỳ vọng — used by external letter writer to endorse. (书面/古典)", "example": "Linh获tenure乃众望所归, 系内系外皆肯定其贡献。", "literal_en": "where everyone's hopes converge (zhòng wàng suǒ guī)", "meaning_en": "The one everyone trusts and expects — used by an external letter-writer to endorse a candidate. (written/classical)", "example_en": "Linh receiving tenure is what everyone expected — both inside and outside the department affirm her contribution." },      { "idiom": "千锤百炼", "literal": "ngàn lần đập trăm lần luyện (qiān chuí bǎi liàn)", "meaning": "Nhiều lần luyện tập, hoàn thiện — describes tenure track as forging process. (书面/古典)", "example": "六年tenure track乃千锤百炼之过程, 将一名assistant professor锻造为成熟的scholar。", "literal_en": "a thousand hammerings, a hundred temperings (qiān chuí bǎi liàn)", "meaning_en": "Tempered through countless trials — describing the tenure track as a forging process. (written/classical)", "example_en": "The six-year tenure track is a process of countless temperings, forging an assistant professor into a mature scholar." }    ],
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are invited by CNA to interview about a new Nature paper. Open formally: '在接受媒体采访之前, 笔者须明确区分三类发言: 个人观点、研究发现、以及代表机构之声明'. When the journalist asks a policy question ('when will this be commercialized'), reframe: '这个问题超出我研究的范围… 我建议您采访industry partner'.",
      "An overseas scholar publicly challenges your paper on arXiv. A reporter asks how you respond. Express it formally: per academic norms, a written response via journal correspondence — NOT a social-media confrontation. Thank them for the challenge + point to the formal channel.",
      "You are writing an op-ed for The Lancet on 'AI for low-resource languages.' Draft the opening 3 paragraphs of formal disclosure: identity (NUS researcher) + COI (NRF funding + Microsoft Research collaboration). Express it: '撰写评论文章时, 笔者须在标题或开头三段中明确披露身份与利益相关性'."
    ],
    ...
    "register_notes_en": "Register: always pre-state which voice you speak in — personal view vs paper-bound finding vs cleared institutional statement; disclose affiliation/funding/COI up front, route peer challenges to formal channels (2-4 weeks), and never feud on social media.",
    "idiom_glosses": [
      { "idiom": "三思而后行", "literal": "ba lần suy nghĩ rồi mới làm (sān sī ér hòu xíng)", "meaning": "Cẩn thận suy nghĩ trước khi nói/làm — gốc Lunyu. Trong public speech: 'public speech需三思而后行, 一旦出口收不回来'. (书面/古典)", "example": "面对媒体采访, 学者更需三思而后行, 因为公开发言可能被反复转载、误读。", "literal_en": "think thrice, then act (sān sī ér hòu xíng)", "meaning_en": "Think carefully before speaking or acting — from the Analects; in public speaking: 三思而后行, 一旦出口收不回来 ('think thrice before acting — once it leaves your mouth it cannot be recalled'). (written/classical)", "example_en": "Facing a media interview, a scholar must think thrice before acting, because public statements can be repeatedly reposted and misread." },      { "idiom": "一言九鼎", "literal": "một lời chín chiếc đỉnh (yī yán jiǔ dǐng)", "meaning": "Một lời nặng như chín đỉnh — public credibility's weight. Dùng để nhấn mạnh responsibility: '学者发言一言九鼎, 不可轻率'. (书面/古典)", "example": "学者面对公众发言, 当如一言九鼎, 因公信力一旦失去, 难以重建。", "literal_en": "one word, nine tripods (yī yán jiǔ dǐng)", "meaning_en": "A single word as weighty as nine bronze tripods — the weight of public credibility; used to stress responsibility: 学者发言一言九鼎, 不可轻率 ('a scholar's word carries great weight and must not be rash'). (written/classical)", "example_en": "When a scholar speaks publicly, each word should carry the weight of nine tripods, for once credibility is lost it is hard to rebuild." },      { "idiom": "明哲保身", "literal": "khôn ngoan giữ thân (míng zhé bǎo shēn)", "meaning": "Khôn ngoan giữ mình — sometimes pejorative (avoid commenting on hard issues). Dùng cẩn thận: '面对争议性话题, 学者亦不宜一味明哲保身'. (书面)", "example": "面对气候变化等公共议题, 学者不宜一味明哲保身, 应以专业知识履行公共责任。", "literal_en": "the wise keep themselves safe (míng zhé bǎo shēn)", "meaning_en": "To play it safe wisely — sometimes pejorative (dodging hard issues); use with care: 面对争议性话题, 学者亦不宜一味明哲保身 ('on controversial topics a scholar should not just play it safe'). (written)", "example_en": "On public issues such as climate change, a scholar should not merely play it safe, but use their expertise to fulfill a public responsibility." },      { "idiom": "言而有信", "literal": "lời nói có niềm tin (yán ér yǒu xìn)", "meaning": "Lời đã nói thì giữ lời — gốc Lunyu. Dùng cho public speech ethics: 'as researchers, 言而有信 — 对说过的研究承诺负责'. (书面/古典)", "example": "学者发言贵在言而有信——对自己研究的局限性诚实交代, 才能赢得长期公信力。", "literal_en": "speak and be trustworthy (yán ér yǒu xìn)", "meaning_en": "Keep the word once given — from the Analects; used for the ethics of public speech: as researchers, 言而有信 — 对说过的研究承诺负责 ('be true to your word — accountable for the research promises you have made'). (written/classical)", "example_en": "A scholar's public speech is valued for being true to its word — only by honestly stating one's research limitations can long-term credibility be won." }    ],
  ...
  },
  {
    ...
    "roleplay_prompts_en": [
      "You are an advisor meeting a PhD student stuck on step 2 for two weeks. Open formally: 'Wang同学, 我看了你的实验日志, 你停在第二步两周了. 怎么了?'. When the student worries about p=0.06, apply the philosophy 'I ask before I answer' — move the student to solve it themselves through questions.",
      "The student fears the paper isn't 'best.' Respond with the formal philosophy: '笔者所秉持之导师理念, 在于以学生之长期学术发展为重, 而非以快速发表论文为唯一指标'. Reframe the focus: capability > a single paper outcome.",
      "The student asks for career advice (industry vs academia). Express the boundary: 'I can't decide for you' + share the advisor's perspective on potential and trade-offs. Close with the lifelong-relationship formula: '不论你选industry还是academia, 我都support — 你永远是我的学术家庭一员'."
    ],
    ...
    "register_notes_en": "Register: speak of students formally (笔者所指导之学生, not 我学生), mentor by asking before answering, give formative critique as 3 critical issues (never line-by-line on a first draft), and route mental-health / misconduct / harassment cases to the proper office — never adjudicate yourself.",
    "idiom_glosses": [
      { "idiom": "因材施教", "literal": "tùy tài mà dạy (yīn cái shī jiào)", "meaning": "Dạy theo năng lực mỗi học sinh — gốc Khổng Tử. Trong supervision: '导师须因材施教, 不可对所有学生一刀切'. (书面/古典)", "example": "因材施教是导师工作之根本——快上手的学生需挑战, 慢思考的学生需鼓励。", "literal_en": "teach according to aptitude (yīn cái shī jiào)", "meaning_en": "To teach according to each student's ability — from Confucius; in supervision: 导师须因材施教, 不可对所有学生一刀切 ('a supervisor must teach to aptitude, not treat all students alike'). (written/classical)", "example_en": "Teaching according to aptitude is the heart of a supervisor's work — fast-starting students need challenge, slow-thinking students need encouragement." },      { "idiom": "诲人不倦", "literal": "dạy không mệt (huì rén bù juàn)", "meaning": "Dạy người không biết chán — gốc Lunyu. Cho mentoring philosophy. (书面/古典)", "example": "导师之德, 在于诲人不倦——即使学生重复犯同样的错误, 也耐心指出。", "literal_en": "instruct others tirelessly (huì rén bù juàn)", "meaning_en": "To teach others without growing tired — from the Analects; for a mentoring philosophy. (written/classical)", "example_en": "A supervisor's virtue lies in tireless instruction — patiently pointing out the same mistake even when a student repeats it." },      { "idiom": "桃李满天下", "literal": "đào lý khắp thiên hạ (táo lǐ mǎn tiān xià)", "meaning": "Đào và lý (học sinh) khắp nơi — successful mentor's legacy. Dùng cho senior advisor's career achievement. (书面/古典)", "example": "陈教授退休时, 桃李满天下——他指导的50位博士生分布于全球各R1机构。", "literal_en": "peaches and plums fill the world (táo lǐ mǎn tiān xià)", "meaning_en": "Peaches and plums (one's students) everywhere — a successful mentor's legacy; used of a senior advisor's career achievement. (written/classical)", "example_en": "When Prof. Chen retired, his students were everywhere — the 50 PhDs he supervised are spread across R1 institutions worldwide." },      { "idiom": "授人以渔", "literal": "dạy người câu cá (shòu rén yǐ yú)", "meaning": "Cho cá không bằng dạy cách câu — give skill, not just answer. (书面/古典)", "example": "笔者倾向授人以渔——通过提问引导学生自己找答案, 而非直接给出。", "literal_en": "teach a person to fish (shòu rén yǐ yú)", "meaning_en": "Giving a fish is not as good as teaching how to fish — give the skill, not just the answer. (written/classical)", "example_en": "I tend to teach students to fish — guiding them with questions to find answers themselves rather than handing them over." }    ],
