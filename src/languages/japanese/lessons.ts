...
const _cache = new Map<JapaneseCefrLevel, JapaneseLesson[]>();

const _importers: Record<
  JapaneseCefrLevel,
  () => Promise<{ default: JapaneseLesson[] }>
> = {
...
export async function loadAllLessons(): Promise<JapaneseLesson[]> {
  const levels: JapaneseCefrLevel[] = ["A1", "A2", "B1", "B2", "C1", "C2"];
...
// generation time.
export const JAPANESE_TOTAL_LESSONS = 147;
