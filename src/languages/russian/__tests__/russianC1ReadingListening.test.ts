...
import { RUSSIAN_CATEGORIES } from "@/languages/russian";
import { normalizeRussianLesson } from "@/languages/russian/normalize";
import { lessons as russianC1ReadingListening } from "@/languages/russian/lessons-c1-reading-listening";

const CYRILLIC_RE = /[Ѐ-ӿ]/;
const CJK_HANGUL_JAPANESE_RE = /[぀-ヿ㐀-鿿가-힯]/;

describe("Russian C1 reading + listening batch", () => {
  it("ships a compact 10-15 lesson C1 batch", () => {
    expect(russianC1ReadingListening.length).toBeGreaterThanOrEqual(10);
    expect(russianC1ReadingListening.length).toBeLessThanOrEqual(15);
  });

  it("keeps every C1 lesson in the app-ready content shape", () => {
    const seen = new Set<string>();
    const categoryIds = new Set(RUSSIAN_CATEGORIES.map((category) => category.id));

    for (const lesson of russianC1ReadingListening) {
      ...
      // Bilingual explanations: Vietnamese is the default support layer,
      // English sits alongside for a future native_language toggle.
      ...
      for (const sentence of lesson.sentences) {
        ...
        // Reading/listening focus cues are bilingual at this level.
        expect(sentence.pronunciation_focus.length).toBeGreaterThan(0);
        expect(sentence.pronunciation_focus_en?.length).toBeGreaterThan(0);
      }
    }
  });

  it("covers both reading and listening discourse skills", () => {
    const ids = russianC1ReadingListening.map((lesson) => lesson.id).join(" ");
    expect(ids).toMatch(/reading/);
    expect(ids).toMatch(/listening/);
    expect(ids).toMatch(/discourse/);
  });

  it("normalizes every C1 lesson into the shared renderer shape", () => {
    for (const [index, lesson] of russianC1ReadingListening.entries()) {
  ...
  it("keeps C1 Russian Cyrillic separate from CJK, Hangul, and Japanese", () => {
    const russianText = russianC1ReadingListening
