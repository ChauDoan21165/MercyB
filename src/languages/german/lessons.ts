...
const _cache = new Map<GermanCefrLevel, GermanLesson[]>();

const _importers: Record<
  GermanCefrLevel,
  () => Promise<{ default: GermanLesson[] }>
> = {
...
export async function loadAllLessons(): Promise<GermanLesson[]> {
  const levels: GermanCefrLevel[] = ["A1", "A2", "B1", "B2", "C1", "C2"];
...
// generation time.
export const GERMAN_TOTAL_LESSONS = 121;
