// Punjabi C2 pre-merge samples for Vietnamese- and English-speaking learners.
// Gurmukhi is primary; romanization is a reading aid.
//
// Scope/status: study-support pre-merge samples only, not certification,
...
export type PunjabiC2PreMergeFocus =
...
export type PunjabiC2PreMergeContext = "public" | "professional" | "community";
export type PunjabiC2PreMergeStyle = "pre_merge" | "runner_readiness" | "pipeline_readiness" | "pre_integration";

export type PunjabiC2PreMergePhrase = {
  gurmukhi: string;
  romanization: string;
  vi: string;
  en: string;
...
export type PunjabiC2PreMergeCheck = {
  check_vi: string;
  check_en: string;
  signal_vi: string;
  signal_en: string;
...
export type PunjabiC2PreMergeTrap = {
  trap_vi: string;
  trap_en: string;
  repair_vi: string;
  repair_en: string;
...
export type PunjabiC2PreMergeSample = {
  id: string;
  focus: PunjabiC2PreMergeFocus;
  context: PunjabiC2PreMergeContext;
  style: PunjabiC2PreMergeStyle;
  title_vi: string;
  title_en: string;
  scenario_vi: string;
  scenario_en: string;
  ...
  sample_vi: string;
  sample_en: string;
  pre_merge_phrases: PunjabiC2PreMergePhrase[];
  checks: PunjabiC2PreMergeCheck[];
  learner_trap?: PunjabiC2PreMergeTrap;
  canada_practical?: boolean;
};

export const C2_PRE_MERGE_SAMPLES_DISCLAIMER = {
  vi: "Bộ pre-merge Punjabi C2 này chỉ hỗ trợ học tập, không phải chứng nhận hay xếp lớp chính thức. Gurmukhi là chính; phiên âm chỉ giúp đọc. Thẩm định bản xứ được hoãn lại. Shahmukhi chỉ được nhắc để nhận biết hệ chữ.",
  en: "This C2 Punjabi pre-merge sample pack supports study only, not certification or official placement. Gurmukhi is primary; romanization only supports reading. Native review is deferred. Shahmukhi is noted only for script awareness.",
} as const;

export const c2PreMergeSamples: PunjabiC2PreMergeSample[] = [
  {
    id: "pa_c2_pre_merge_nuanced_disagreement_professional",
    focus: "nuanced_disagreement",
    context: "professional",
    style: "pre_merge",
    title_vi: "pre-merge: bất đồng tinh tế",
    title_en: "pre-merge: nuanced disagreement",
    ...
    sample_gurmukhi: "ਦਿਸ਼ਾ ਠੀਕ ਹੈ, ਪਰ pre-merge ਤੋਂ ਪਹਿਲਾਂ ਇਸ ਦਲੀਲ ਨੂੰ ਇੱਕ ਵਾਰੀ ਹੋਰ ਪਰਖ ਲਈਏ।",
    sample_romanization: "disha theek hai, par pre-merge ton pehlaan is dalil nu ikk vari hor parakh laie.",
    sample_vi: "Hướng đi đúng, nhưng trước pre-merge ta hãy kiểm tra lập luận này thêm một lần.",
    sample_en: "The direction is right, but before pre-merge let's test this argument once more.",
    pre_merge_phrases: [
    ...
    learner_trap: {
      ...
      trap_en: "Direct negation before pre-merge makes the tone too tense.",
  ...
  {
    id: "pa_c2_pre_merge_diplomacy_community_canada",
    focus: "diplomacy",
    context: "community",
    style: "runner_readiness",
    title_vi: "pre-merge: ngoại giao phạm vi",
    title_en: "pre-merge: scope diplomacy",
    scenario_vi: "Một nhóm cộng đồng ở Canada muốn thêm đề xuất sau hạn ổn định.",
    scenario_en: "A Canadian community group wants to add a suggestion after the pre-A11 merge review deadline.",
    ...
    sample_gurmukhi: "ਸੁਝਾਅ ਲਈ ਧੰਨਵਾਦ। ਇਸ pre-merge ਵਿੱਚ ਹੱਦ ਨਹੀਂ ਬਦਲੇਗੀ, ਪਰ ਅਗਲੇ ਚੱਕਰ ਲਈ ਇਸ ਨੂੰ ਸੰਭਾਲ ਲੈਂਦੇ ਹਾਂ।",
    sample_romanization: "sujhaa lai dhanvaad. is pre-merge vich hadd nahin badlegi, par agle chakkar lai is nu sambhaal lainde haan.",
    sample_vi: "Cảm ơn đề xuất. Trong pre-merge này phạm vi sẽ không đổi, nhưng ta sẽ giữ lại cho vòng sau.",
    sample_en: "Thank you for the suggestion. In this pre-merge the scope will not change, but we will keep it for the next cycle.",
    pre_merge_phrases: [
  ...
  {
    id: "pa_c2_pre_merge_mediation_public",
    focus: "mediation",
    context: "public",
    style: "pipeline_readiness",
    title_vi: "pre-merge: trung gian công khai",
    title_en: "pre-merge: public mediation",
    ...
    sample_gurmukhi: "ਦੋਵੇਂ ਪਾਸਿਆਂ ਦੀ ਗੱਲ ਦਰਜ ਹੈ; pre-merge ਸਾਂਝੇ ਮਾਪਦੰਡ ਦੇ ਆਧਾਰ ਤੇ ਕੀਤਾ ਜਾਵੇਗਾ।",
    sample_romanization: "dovein pasian di gall darj hai; pre-merge sanjhe mapdand de aadhaar te kita javega.",
    sample_vi: "Ý kiến của cả hai bên đã được ghi nhận; pre-merge sẽ dựa trên tiêu chí chung.",
    sample_en: "Both sides' points are recorded; the pre-merge will be based on shared criteria.",
    pre_merge_phrases: [
  ...
  {
    id: "pa_c2_pre_merge_deescalation_professional",
    focus: "deescalation",
    context: "professional",
    style: "pre_merge",
    title_vi: "pre-merge: hạ nhiệt trách nhiệm",
    title_en: "pre-merge: de-escalating ownership",
    scenario_vi: "Cuộc họp ổn định cuối căng vì một lỗi nhỏ còn được nhắc lại.",
    scenario_en: "A pre-merge meeting is tense because a small issue keeps being repeated.",
    ...
    sample_gurmukhi: "ਮੁੱਦਾ ਦਰਜ ਹੋ ਗਿਆ ਹੈ; ਹੁਣ ਵਿਅਕਤੀ ਨਹੀਂ, ਸੁਧਾਰ ਦੇ ਕਦਮ ਨੂੰ pre-merge ਵਿੱਚ ਪੱਕਾ ਕਰੀਏ।",
    sample_romanization: "mudda darj ho giya hai; hun vyakti nahin, sudhaar de kadam nu pre-merge vich pakka kariye.",
    sample_vi: "Vấn đề đã được ghi nhận; bây giờ không nói về cá nhân, hãy ổn định bước sửa trong pre-merge.",
    sample_en: "The issue has been recorded; now let's focus on the repair step, not the person, in pre-merge.",
    pre_merge_phrases: [
  ...
  {
    id: "pa_c2_pre_merge_audience_adaptation_canada",
    focus: "audience_adaptation",
    context: "community",
    style: "pre_integration",
    title_vi: "pre-merge: thông báo cho nhóm đa thế hệ",
    title_en: "pre-merge: notice for a multi-generational group",
  ...
  {
    id: "pa_c2_pre_merge_sensitive_topic_public",
    focus: "sensitive_topic_framing",
    context: "public",
    style: "pipeline_readiness",
    title_vi: "pre-merge: khung chủ đề nhạy cảm",
    title_en: "pre-merge: sensitive-topic framing",
    scenario_vi: "Một nhận xét công khai kéo bản ổn định cuối sang chính trị, tôn giáo, tiền bạc, hoặc danh tính cá nhân.",
    scenario_en: "A public comment pulls the pre-merge toward politics, religion, money, or personal identity.",
    ...
    sample_gurmukhi: "ਇਹ ਵਿਸ਼ਾ ਸੰਵੇਦਨਸ਼ੀਲ ਹੈ, ਇਸ ਲਈ pre-merge ਨੂੰ ਕੰਮ ਨਾਲ ਜੁੜੇ ਹਿੱਸੇ ਤੱਕ ਹੀ ਰੱਖੀਏ।",
    sample_romanization: "ih visha sanvedansheel hai, is lai pre-merge nu kamm naal jurre hisse takk hi rakhie.",
    sample_vi: "Chủ đề này nhạy cảm, vì vậy hãy giữ pre-merge chỉ trong phần liên quan công việc.",
    sample_en: "This topic is sensitive, so let's keep pre-merge only to the work-related part.",
    pre_merge_phrases: [
  ...
  {
    id: "pa_c2_pre_merge_public_calibration_professional",
    focus: "public_communication_calibration",
    context: "professional",
    style: "runner_readiness",
    title_vi: "pre-merge: cân chỉnh thông báo",
    title_en: "pre-merge: calibrated announcement",
    ...
    sample_gurmukhi: "ਇਹ ਹਿੱਸਾ pre-merge ਵਿੱਚ ਹੈ; ਵਰਤੋਂ ਦੌਰਾਨ ਆਉਣ ਵਾਲੀ ਪ੍ਰਤੀਕਿਰਿਆ ਨੂੰ ਫਿਰ ਵੀ ਧਿਆਨ ਨਾਲ ਵੇਖਾਂਗੇ।",
    sample_romanization: "ih hissa pre-merge vich hai; varton dauran aun vali pratikiria nu phir vi dhian naal vekhange.",
    sample_vi: "Phần này đã ở pre-merge; tuy vậy trong quá trình sử dụng chúng tôi vẫn sẽ theo dõi phản hồi cẩn thận.",
    sample_en: "This part is in pre-merge; still, during use we will watch feedback carefully.",
    pre_merge_phrases: [
      { gurmukhi: "pre-merge ਵਿੱਚ ਹੈ", romanization: "pre-merge vich hai", vi: "Đang ở pre-merge.", en: "Is in pre-merge." },
  ...
  {
    id: "pa_c2_pre_merge_register_safety_community",
    focus: "register_safety",
    context: "community",
    style: "pre_integration",
    title_vi: "pre-merge: an toàn register cộng đồng",
    title_en: "pre-merge: community register safety",
...
export const c2PreMergeSamplesByFocus = (focus: PunjabiC2PreMergeFocus) =>
  c2PreMergeSamples.filter((sample) => sample.focus === focus);

export const c2PreMergeSamplesByContext = (context: PunjabiC2PreMergeContext) =>
  c2PreMergeSamples.filter((sample) => sample.context === context);
