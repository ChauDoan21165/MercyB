// src/languages/punjabi/b1CompletionRecordSamples.ts
//
// Punjabi B1 completion record samples for Vietnamese-speaking and English-speaking
...
export type PunjabiB1CompletionRecordFocus =
  | "explain_situation"
  | "retell_event"
  | "clarify_next_steps"
  | "service_recovery"
...
export type PunjabiCompletionRecordLine = {
  pa: string;
  romanization: string;
  en: string;
  vi: string;
...
export type PunjabiCompletionRecordTrap = {
  trap_en: string;
  trap_vi: string;
  better: PunjabiCompletionRecordLine;
};

export type PunjabiCompletionRecordQA = {
  prompt_en: string;
  prompt_vi: string;
  modelAnswer: PunjabiCompletionRecordLine;
  followUpQuestion: PunjabiCompletionRecordLine;
};

export type PunjabiB1CompletionRecordCard = {
  id: string;
  level: "B1";
  focus: PunjabiB1CompletionRecordFocus;
  title_en: string;
  title_vi: string;
  completionRecordPurpose_en: string;
  completionRecordPurpose_vi: string;
  canadaContext: string;
  checkpointCriteria_en: string[];
  checkpointCriteria_vi: string[];
  usefulLanguage: PunjabiCompletionRecordLine[];
  qa: PunjabiCompletionRecordQA;
  commonTraps: PunjabiCompletionRecordTrap[];
  completionRecordNote_en: string;
  completionRecordNote_vi: string;
};

export type PunjabiB1CompletionRecordScope = {
  level: "B1";
  note_en: string;
  note_vi: string;
  awareness_en: string;
...
export const punjabiB1CompletionRecordScope: PunjabiB1CompletionRecordScope = {
  level: "B1",
  note_en:
    "Native review is deferred. This completion record keeps stable independent communication samples for later A11 use after inventory seal, catalog, and pre-integration checks.",
  note_vi:
...
export const punjabiB1CompletionRecordSamples: PunjabiB1CompletionRecordCard[] = [
  {
    id: "pa-b1-completion-record-01-explain-situation",
    level: "B1",
    focus: "explain_situation",
    title_en: "Explain a situation clearly",
    title_vi: "Giải thích tình huống rõ ràng",
    completionRecordPurpose_en: "Keep a stable sample for explaining an access problem, delay, or disruption without overexplaining.",
    completionRecordPurpose_vi: "Giữ mẫu ổn định để giải thích vấn đề truy cập, sự chậm trễ hoặc gián đoạn mà không giải thích dài dòng.",
    ...
    completionRecordNote_en: "Recorded as a completion-record-ready situation-explanation pattern for later review and reuse.",
    completionRecordNote_vi: "Được lưu như mẫu giải thích tình huống đơn giản để xem lại và dùng lại sau.",
  },
  {
    id: "pa-b1-completion-record-02-retell-event",
    level: "B1",
    focus: "retell_event",
    title_en: "Retell an event in order",
    title_vi: "Kể lại sự việc theo thứ tự",
    completionRecordPurpose_en: "Keep a short, stable sequence sample for late arrivals, transit delays, and schedule changes.",
    completionRecordPurpose_vi: "Giữ mẫu trình tự ngắn, ổn định cho việc đến muộn, trễ xe và thay đổi lịch.",
    ...
    completionRecordNote_en: "Recorded as a completion-record-ready sequence model for messages, appointments, and quick follow-ups.",
    completionRecordNote_vi: "Được lưu như mẫu trình tự cho tin nhắn, cuộc hẹn và theo dõi nhanh.",
  },
  {
    id: "pa-b1-completion-record-03-clarify-next-steps",
    level: "B1",
    focus: "clarify_next_steps",
    title_en: "Clarify next steps",
    title_vi: "Làm rõ bước tiếp theo",
    completionRecordPurpose_en: "Keep a polite clarification sample for forms, deadlines, email follow-up, and written confirmation.",
    completionRecordPurpose_vi: "Giữ mẫu làm rõ lịch sự cho biểu mẫu, hạn chót, theo dõi email và xác nhận bằng văn bản.",
    ...
    completionRecordNote_en: "Recorded for safe clarification and written follow-up practice.",
    completionRecordNote_vi: "Được lưu để luyện làm rõ an toàn và theo dõi bằng văn bản.",
  },
  {
    id: "pa-b1-completion-record-04-service-recovery",
    level: "B1",
    focus: "service_recovery",
    title_en: "Recover after a service mistake",
    title_vi: "Khắc phục sau lỗi dịch vụ",
    completionRecordPurpose_en: "Keep a repair sample for wrong charges, missing bookings, or service errors that need correction.",
    completionRecordPurpose_vi: "Giữ mẫu sửa chữa cho phí sai, đặt chỗ bị thiếu hoặc lỗi dịch vụ cần được điều chỉnh.",
    ...
    completionRecordNote_en: "Recorded as a service-recovery script for correction, timing, and reference tracking.",
    completionRecordNote_vi: "Được lưu như kịch bản khắc phục dịch vụ cho sửa lỗi, thời gian xử lý và theo dõi tham chiếu.",
  },
  {
    id: "pa-b1-completion-record-05-issue-resolution",
    level: "B1",
    focus: "issue_resolution",
    title_en: "Resolve a practical issue",
    title_vi: "Giải quyết một vấn đề thực tế",
    completionRecordPurpose_en: "Keep a repair-and-resolution sample for housing, delivery, and workplace issues that need a concrete fix.",
    completionRecordPurpose_vi: "Giữ mẫu sửa chữa và giải quyết cho vấn đề nhà ở, giao hàng và công việc cần giải pháp cụ thể.",
    ...
    completionRecordNote_en: "Recorded for practical fix language across housing, delivery, and workplace settings.",
    completionRecordNote_vi: "Được lưu cho ngôn ngữ sửa lỗi thực tế ở bối cảnh nhà ở, giao hàng và công việc.",
  },
  {
    id: "pa-b1-completion-record-06-follow-up-message",
    level: "B1",
    focus: "follow_up_message",
    title_en: "Send a follow-up message",
    title_vi: "Gửi tin nhắn theo dõi",
    completionRecordPurpose_en: "Keep a clean follow-up sample for email, text, and short written summaries after a conversation.",
    completionRecordPurpose_vi: "Giữ mẫu theo dõi gọn cho email, tin nhắn và tóm tắt ngắn sau cuộc trò chuyện.",
    ...
    completionRecordNote_en: "Recorded as a concise follow-up template for written records and reminders.",
    completionRecordNote_vi: "Được lưu như mẫu theo dõi ngắn gọn cho hồ sơ viết và lời nhắc.",
  },
  {
    id: "pa-b1-completion-record-07-workplace-housing-school-community",
    level: "B1",
    ...
    completionRecordPurpose_en: "Keep one flexible sample for cross-setting coordination when the same issue follows you across places.",
    completionRecordPurpose_vi: "Giữ một mẫu linh hoạt cho phối hợp nhiều bối cảnh khi cùng một vấn đề đi theo bạn qua nhiều nơi.",
    ...
    completionRecordNote_en: "Recorded as a cross-setting coordination pattern for Canadian daily life.",
    completionRecordNote_vi: "Được lưu như mẫu phối hợp nhiều bối cảnh cho đời sống hằng ngày ở Canada.",
  },
  {
    id: "pa-b1-completion-record-08-register-safe-repair",
    level: "B1",
    focus: "register_safe_repair",
    title_en: "Repair a too-direct message",
    title_vi: "Sửa một câu quá trực diện",
    completionRecordPurpose_en: "Keep a register-safe repair sample so a blunt line can be turned into a polite request.",
    completionRecordPurpose_vi: "Giữ mẫu sửa mức độ lịch sự để một câu quá thẳng có thể đổi thành yêu cầu lịch sự.",
    ...
    completionRecordNote_en: "Recorded for register repair, where a direct line becomes a usable polite request.",
    completionRecordNote_vi: "Được lưu cho sửa mức độ lịch sự, nơi một câu trực diện trở thành yêu cầu lịch sự có thể dùng được.",
  },
  {
    id: "pa-b1-completion-record-09-completion-record-signoff",
    level: "B1",
    focus: "completion_record_signoff",
    title_en: "Close and seal the completion record",
    title_vi: "Kết và đóng gói mẫu lưu trữ",
    completionRecordPurpose_en: "Keep a pre-A11 completion-record signoff sample that mentions inventory seal, catalog, pre-integration sealing, handoff, and stable reuse.",
    completionRecordPurpose_vi: "Giữ mẫu signoff hồ sơ hoàn tất trước A11 có nhắc đến kiểm kê đóng gói, danh mục, đóng gói trước tích hợp, bàn giao và dùng lại ổn định.",
    canadaContext: "Useful when preparing a final note for a team lead, teacher, landlord, or coordinator in Canada.",
    checkpointCriteria_en: ["Say the sample is ready.", "Name the completion record or handoff step.", "Ask for the final confirmation."],
    checkpointCriteria_vi: ["Nói mẫu đã sẵn sàng.", "Nêu bước hồ sơ hoàn tất hoặc bàn giao.", "Xin xác nhận cuối cùng."],
    usefulLanguage: [
      { pa: "ਇਹ ਨਮੂਨਾ ਕੰਪਲੀਸ਼ਨ ਰਿਕਾਰਡ ਲਈ ਤਿਆਰ ਹੈ।", romanization: "eh namuna completion record lai tayar hai.", en: "This sample is ready for the completion record.", vi: "Mẫu này đã sẵn sàng cho hồ sơ hoàn tất." },
    ...
    qa: {
      ...
      modelAnswer: {
        pa: "ਇਹ ਨਮੂਨਾ ਕੰਪਲੀਸ਼ਨ ਰਿਕਾਰਡ ਲਈ ਤਿਆਰ ਹੈ। ਕਿਰਪਾ ਕਰਕੇ ਸਾਈਨ-ਆਫ਼ ਕਰੋ ਜੀ ਅਤੇ ਅਗਲਾ ਪੜਾਅ ਸ਼ੁਰੂ ਕਰੋ।",
        romanization: "eh namuna completion record lai tayar hai. kirpa karke sign-off karo ji ate agla parra shuru karo.",
        en: "This sample is ready for the completion record. Please sign off and start the next stage.",
        vi: "Mẫu này đã sẵn sàng cho hồ sơ hoàn tất. Vui lòng xác nhận hoàn tất và bắt đầu giai đoạn tiếp theo.",
      },
      followUpQuestion: {
    ...
    commonTraps: [
      {
        trap_en: "Treating the completion record note like a live operational instruction.",
        trap_vi: "Xem ghi chú hồ sơ hoàn tất như hướng dẫn vận hành trực tiếp.",
        better: {
          pa: "ਇਹ ਕੰਪਲੀਸ਼ਨ ਰਿਕਾਰਡ ਨੋਟ ਹੈ, ਲਾਈਵ ਹਦਾਇਤ ਨਹੀਂ।",
          romanization: "eh completion record note hai, live hadait nahin.",
          en: "This is a completion record note, not a live instruction.",
          vi: "Đây là ghi chú hồ sơ hoàn tất, không phải hướng dẫn trực tiếp.",
        },
      },
    ],
    completionRecordNote_en: "Recorded as the pre-A11 handoff and seal point for this compact B1 completion record pack.",
    completionRecordNote_vi: "Được lưu như điểm bàn giao và đóng gói trước A11 cho bộ B1 ngắn gọn này.",
  },
];
