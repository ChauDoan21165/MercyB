// Punjabi C2 release-candidate sample guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_RELEASE_CANDIDATE_SAMPLES_DISCLAIMER,
  c2ReleaseCandidateSamples,
  c2ReleaseCandidateSamplesByContext,
  c2ReleaseCandidateSamplesByFocus,
  type PunjabiC2ReleaseCandidateContext,
  type PunjabiC2ReleaseCandidateFocus,
  type PunjabiC2ReleaseCandidateSample,
} from "@/languages/punjabi/c2ReleaseCandidateSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2ReleaseCandidateFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2ReleaseCandidateContext[] = ["public", "professional", "community"];

describe("Punjabi C2 release-candidate samples - coverage", () => {
  it("ships a compact app-consumable release-candidate set", () => {
    expect(c2ReleaseCandidateSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2ReleaseCandidateSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required release-candidate focus", () => {
    const seen = new Set(c2ReleaseCandidateSamples.map((item) => item.focus));
  ...
  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2ReleaseCandidateSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
      expect(c2ReleaseCandidateSamplesByContext(context).every((item) => item.context === context)).toBe(true);
    }
  });

  it("has unique ids and focus filter", () => {
    const ids = c2ReleaseCandidateSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2ReleaseCandidateSamplesByFocus(focus);
...
describe("Punjabi C2 release-candidate samples - bilingual integrity", () => {
  it("each item has VI+EN scenario and readiness goal", () => {
    for (const item of c2ReleaseCandidateSamples) {
  ...
  it("each sample and phrase has Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2ReleaseCandidateSamples) {
      ...
      expect(item.release_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.release_phrases) {
  ...
  it("includes release-candidate, closure-validation, pre-integration, and readiness styles", () => {
    const styles = new Set(c2ReleaseCandidateSamples.map((item) => item.style));
  ...
  it("each item carries release-readiness checks", () => {
    for (const item of c2ReleaseCandidateSamples) {
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2ReleaseCandidateSamples.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2ReleaseCandidateSamples.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 release-candidate samples - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_RELEASE_CANDIDATE_SAMPLES_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_RELEASE_CANDIDATE_SAMPLES_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_RELEASE_CANDIDATE_SAMPLES_DISCLAIMER.vi} ${C2_RELEASE_CANDIDATE_SAMPLES_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2ReleaseCandidateSamples).toLowerCase();
...
const _typecheck: PunjabiC2ReleaseCandidateSample[] = c2ReleaseCandidateSamples;
void _typecheck;
