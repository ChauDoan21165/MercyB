// src/languages/punjabi/__tests__/punjabiFinalExportReadiness.test.ts
//
// Guards the Punjabi Wave 24 final export readiness packet. This is not A11 integration.

import { describe, expect, it } from "vitest";

import {
  PUNJABI_FINAL_EXPORT_READINESS,
  PUNJABI_FINAL_EXPORT_READINESS_AREAS,
  PUNJABI_FINAL_EXPORT_READINESS_ROOT,
  PUNJABI_FINAL_EXPORT_READINESS_SCOPE,
  PUNJABI_FINAL_EXPORT_READINESS_ROUTES,
  type PunjabiFinalExportReadinessArea,
} from "../finalExportReadiness";
...
const REQUIRED_AREAS: PunjabiFinalExportReadinessArea[] = [
  "module_inventory",
  "export_groups",
  "level_coverage",
  "gurmukhi_first",
...
function allStrings(): string[] {
  ...
  walk(PUNJABI_FINAL_EXPORT_READINESS_ROOT);
  return out;
}

describe("Punjabi final export readiness - scope", () => {
  it("declares Wave 24 only and not A11 integration", () => {
    expect(PUNJABI_FINAL_EXPORT_READINESS_SCOPE.wave).toBe("Wave 24");
    expect(PUNJABI_FINAL_EXPORT_READINESS_SCOPE.not_a11_integration).toBe(true);
  });

  it("declares Punjabi identity and Gurmukhi primary", () => {
    expect(PUNJABI_FINAL_EXPORT_READINESS_SCOPE.code).toBe("pa");
    expect(PUNJABI_FINAL_EXPORT_READINESS_SCOPE.name_pa).toMatch(GURMUKHI);
    expect(PUNJABI_FINAL_EXPORT_READINESS_SCOPE.primary_script).toBe("Gurmukhi");
  });

  it("keeps Shahmukhi awareness-only and native review deferred", () => {
    const text = `${PUNJABI_FINAL_EXPORT_READINESS_SCOPE.script_note_en} ${PUNJABI_FINAL_EXPORT_READINESS_SCOPE.native_review_en}`.toLowerCase();
  ...
  it("excludes audio/scoring/infrastructure/push/deploy", () => {
    const text = PUNJABI_FINAL_EXPORT_READINESS_SCOPE.excluded_en.toLowerCase();
...
describe("Punjabi final export readiness - coverage", () => {
  it("declares all requested export-readiness areas", () => {
    expect(new Set(PUNJABI_FINAL_EXPORT_READINESS_AREAS)).toEqual(new Set(REQUIRED_AREAS));
  });

  it("has items for every requested area", () => {
    const areas = new Set(PUNJABI_FINAL_EXPORT_READINESS.map((item) => item.area));
    for (const area of REQUIRED_AREAS) expect(areas.has(area)).toBe(true);
  });

  it("covers A1-C2 across readiness items", () => {
    const levels = new Set(PUNJABI_FINAL_EXPORT_READINESS.flatMap((item) => item.levels));
    for (const level of ALL_LEVELS) expect(levels.has(level)).toBe(true);
  });

  it("keeps the readiness packet compact but useful", () => {
    expect(PUNJABI_FINAL_EXPORT_READINESS.length).toBeGreaterThanOrEqual(10);
    expect(PUNJABI_FINAL_EXPORT_READINESS.length).toBeLessThanOrEqual(14);
  });
});

describe("Punjabi final export readiness - item shape", () => {
  it("uses app-consumable bilingual Gurmukhi-first data", () => {
    for (const item of PUNJABI_FINAL_EXPORT_READINESS) {
      ...
      expect(item.readiness_tags.length).toBeGreaterThan(0);
    }
  });

  it("includes statuses and boundary markers", () => {
    const statuses = new Set(PUNJABI_FINAL_EXPORT_READINESS.map((item) => item.status));
  ...
  it("includes learner traps and must-not-claim markers", () => {
    const traps = PUNJABI_FINAL_EXPORT_READINESS.filter((item) => item.learner_trap_vi && item.learner_trap_en);
    const forbidden = PUNJABI_FINAL_EXPORT_READINESS.filter((item) => item.must_not_claim_vi && item.must_not_claim_en);
    expect(traps.length).toBeGreaterThanOrEqual(8);
    expect(forbidden.length).toBeGreaterThanOrEqual(2);
  });

  it("references the expected modules for export readiness", () => {
    const modules = PUNJABI_FINAL_EXPORT_READINESS.flatMap((item) => item.expected_modules).join(" ");
  ...
  it("references the export groups expected by final export readiness", () => {
    const groups = PUNJABI_FINAL_EXPORT_READINESS.flatMap((item) => item.export_groups).join(" ");
    expect(groups).toContain("core_exports");
    expect(groups).toContain("review_exports");
    expect(groups).toContain("boundary_exports");
    expect(groups).toContain("content_exports");
    expect(groups).toContain("risk_exports");
    expect(groups).toContain("route_exports");
    expect(groups).toContain("hardening_exports");
    expect(groups).toContain("regression_exports");
    expect(groups).toContain("safe_exports");
  });
});

describe("Punjabi final export readiness - boundary checks", () => {
  it("covers Gurmukhi-first, VI/EN support, Canada domains, hardening, and regression", () => {
    const text = PUNJABI_FINAL_EXPORT_READINESS.map((item) => `${item.readiness_en} ${item.regression_en} ${item.title_en}`).join(" ").toLowerCase();
  ...
  it("guards no audio/pronunciation/Azure claims", () => {
    const item = PUNJABI_FINAL_EXPORT_READINESS.find((entry) => entry.id === "export-forbidden-claims");
  ...
  it("guards native-review deferral and no-A11 boundaries", () => {
    const text = PUNJABI_FINAL_EXPORT_READINESS.map((item) => `${item.readiness_en} ${item.must_not_claim_en ?? ""} ${item.learner_trap_en ?? ""}`).join(" ").toLowerCase();
    expect(text).toContain("native review is deferred");
    expect(text).toContain("completion is not claimed");
  ...
  it("defines readiness routes for inventory, support, and boundary checks", () => {
    expect(PUNJABI_FINAL_EXPORT_READINESS_ROUTES.length).toBe(3);
    for (const route of PUNJABI_FINAL_EXPORT_READINESS_ROUTES) {
...
describe("Punjabi final export readiness - no unrelated scripts", () => {
