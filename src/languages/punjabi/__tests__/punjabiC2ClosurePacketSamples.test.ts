// Punjabi C2 pre-closure-packet sample guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_PRE_CLOSURE_PACKET_SAMPLES_DISCLAIMER,
  c2ClosurePacketSamples,
  c2ClosurePacketSamplesByContext,
  c2ClosurePacketSamplesByFocus,
  type PunjabiC2ClosurePacketContext,
  type PunjabiC2ClosurePacketFocus,
  type PunjabiC2ClosurePacketSample,
} from "@/languages/punjabi/c2ClosurePacketSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2ClosurePacketFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2ClosurePacketContext[] = ["public", "professional", "community"];

describe("Punjabi C2 pre-closure-packet samples - coverage", () => {
  it("ships a compact app-consumable pre-closure-packet sample set", () => {
    expect(c2ClosurePacketSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2ClosurePacketSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required pre-closure-packet focus", () => {
    const seen = new Set(c2ClosurePacketSamples.map((item) => item.focus));
  ...
  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2ClosurePacketSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
      expect(c2ClosurePacketSamplesByContext(context).every((item) => item.context === context)).toBe(true);
    }
  });

  it("has unique ids and focus filter", () => {
    const ids = c2ClosurePacketSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2ClosurePacketSamplesByFocus(focus);
...
describe("Punjabi C2 pre-closure-packet samples - bilingual integrity", () => {
  it("each item has VI+EN scenario and pre-closure-packet goal", () => {
    for (const item of c2ClosurePacketSamples) {
  ...
  it("each sample and phrase has Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2ClosurePacketSamples) {
      ...
      expect(item.pre_closure_packet_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.pre_closure_packet_phrases) {
  ...
  it("includes pre-closure-packet, runner-readiness, pipeline-readiness, and pre-integration styles", () => {
    const styles = new Set(c2ClosurePacketSamples.map((item) => item.style));
    expect(styles.has("pre_closure_packet")).toBe(true);
  ...
  it("each item carries pre-closure-packet checks", () => {
    for (const item of c2ClosurePacketSamples) {
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2ClosurePacketSamples.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2ClosurePacketSamples.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 pre-closure-packet samples - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_PRE_CLOSURE_PACKET_SAMPLES_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_PRE_CLOSURE_PACKET_SAMPLES_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_PRE_CLOSURE_PACKET_SAMPLES_DISCLAIMER.vi} ${C2_PRE_CLOSURE_PACKET_SAMPLES_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2ClosurePacketSamples).toLowerCase();
...
const _typecheck: PunjabiC2ClosurePacketSample[] = c2ClosurePacketSamples;
void _typecheck;
