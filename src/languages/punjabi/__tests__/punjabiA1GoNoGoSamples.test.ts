...
import punjabiA1GoNoGoSamples, {
  goNoGoScriptAwareness,
  punjabiA1GoNoGoSamples as namedGoNoGoSamples,
  type PunjabiA1GoNoGoDomain,
  type PunjabiA1GoNoGoStyle,
} from "@/languages/punjabi/a1GoNoGoSamples";

const GURMUKHI_SCRIPT = /[\u0A00-\u0A7F]/;
const LATIN = /[a-zA-Z]/;

describe("Punjabi A1 go-no-go samples", () => {
  it("exports app-consumable named and default arrays", () => {
    expect(punjabiA1GoNoGoSamples).toBe(namedGoNoGoSamples);
    expect(Array.isArray(punjabiA1GoNoGoSamples)).toBe(true);
  });

  it("is compact and covers the required go-no-go domains", () => {
    expect(punjabiA1GoNoGoSamples.length).toBeGreaterThanOrEqual(12);
    expect(punjabiA1GoNoGoSamples.length).toBeLessThanOrEqual(14);

    const requiredDomains: PunjabiA1GoNoGoDomain[] = [
      "greetings",
      "identity",
      "family",
      "numbers",
    ...
    const domains = new Set(punjabiA1GoNoGoSamples.map((item) => item.domain));
  ...
  it("includes go-no-go, closure-validation, pre-integration, readiness-check, qa, and selector styles", () => {
    const requiredStyles: PunjabiA1GoNoGoStyle[] = [
      "go_no_go",
      "closure_validation",
      "pre_integration",
      "readiness_check",
    ...
    const styles = new Set(punjabiA1GoNoGoSamples.map((item) => item.style));
  ...
  it("uses Gurmukhi primary with bilingual support and review links", () => {
    for (const item of punjabiA1GoNoGoSamples) {
      expect(item.id).toMatch(/^pa_a1_go_nogo_/);
      ...
      expect(item.go_nogo_check_vi.trim().length).toBeGreaterThan(0);
      expect(item.go_nogo_check_en.trim().length).toBeGreaterThan(0);
  ...
  it("includes traps, romanization bridge, and Canada-practical release items", () => {
    const traps = punjabiA1GoNoGoSamples.flatMap((item) => (item.trap ? [item.trap] : []));
    const trapAudiences = new Set(traps.map((trap) => trap.audience));
    const canadaItems = punjabiA1GoNoGoSamples.filter((item) => item.canada_practical);
    const romanizationBridge = punjabiA1GoNoGoSamples.find((item) => item.domain === "romanization_bridge");
  ...
  it("verifies beginner readiness across the required go-no-go situations", () => {
    const allText = JSON.stringify(punjabiA1GoNoGoSamples);
    ...
    expect(allText).toMatch(/go-no-go|Go\/no-go|Sẵn sàng|Ready/i);
  });

  it("mentions Shahmukhi only as awareness and avoids disallowed claims and systems", () => {
    const allText = `${goNoGoScriptAwareness} ${JSON.stringify(punjabiA1GoNoGoSamples)}`;

    expect(goNoGoScriptAwareness).toContain("Shahmukhi");
    expect(goNoGoScriptAwareness).toContain("awareness only");
