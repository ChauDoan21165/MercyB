// Punjabi C2 owner-acceptance sample guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_OWNER_ACCEPTANCE_SAMPLES_DISCLAIMER,
  c2OwnerAcceptanceSamples,
  c2OwnerAcceptanceSamplesByContext,
  c2OwnerAcceptanceSamplesByFocus,
  type PunjabiC2OwnerAcceptanceContext,
  type PunjabiC2OwnerAcceptanceFocus,
  type PunjabiC2OwnerAcceptanceSample,
} from "@/languages/punjabi/c2OwnerAcceptanceSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2OwnerAcceptanceFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2OwnerAcceptanceContext[] = ["public", "professional", "community"];

describe("Punjabi C2 owner-acceptance samples - coverage", () => {
  it("ships a compact app-consumable owner-acceptance set", () => {
    expect(c2OwnerAcceptanceSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2OwnerAcceptanceSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required owner-acceptance focus", () => {
    const seen = new Set(c2OwnerAcceptanceSamples.map((item) => item.focus));
  ...
  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2OwnerAcceptanceSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
      expect(c2OwnerAcceptanceSamplesByContext(context).every((item) => item.context === context)).toBe(true);
    }
  });

  it("has unique ids and focus filter", () => {
    const ids = c2OwnerAcceptanceSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2OwnerAcceptanceSamplesByFocus(focus);
...
describe("Punjabi C2 owner-acceptance samples - bilingual integrity", () => {
  it("each item has VI+EN scenario and owner-acceptance goal", () => {
    for (const item of c2OwnerAcceptanceSamples) {
      ...
      expect(item.acceptance_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.acceptance_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
    }
  });
  ...
  it("each sample and phrase has Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2OwnerAcceptanceSamples) {
      ...
      expect(item.owner_acceptance_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.owner_acceptance_phrases) {
  ...
  it("includes owner-acceptance, final-acceptance, ship-candidate, and pre-integration styles", () => {
    const styles = new Set(c2OwnerAcceptanceSamples.map((item) => item.style));
  ...
  it("each item carries owner-acceptance checks", () => {
    for (const item of c2OwnerAcceptanceSamples) {
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2OwnerAcceptanceSamples.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2OwnerAcceptanceSamples.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 owner-acceptance samples - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_OWNER_ACCEPTANCE_SAMPLES_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_OWNER_ACCEPTANCE_SAMPLES_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_OWNER_ACCEPTANCE_SAMPLES_DISCLAIMER.vi} ${C2_OWNER_ACCEPTANCE_SAMPLES_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2OwnerAcceptanceSamples).toLowerCase();
...
const _typecheck: PunjabiC2OwnerAcceptanceSample[] = c2OwnerAcceptanceSamples;
void _typecheck;
