...
import {
  punjabiB1ReceiptSamples,
  punjabiB1ReceiptScope,
  type PunjabiReceiptLine,
  type PunjabiB1ReceiptFocus,
} from "../b1ReceiptSamples";

const GURMUKHI_RE = /[\u0A00-\u0A7F]/;
const LATIN_RE = /[a-z]/i;

const REQUIRED_FOCUSES: PunjabiB1ReceiptFocus[] = [
  "explain_situation",
  "retell_event",
  "clarify_next_steps",
  "service_recovery",
...
function expectLine(line: PunjabiReceiptLine) {
...
describe("Punjabi B1 receipt samples - batch", () => {
  it("has a compact useful receipt pack", () => {
    expect(punjabiB1ReceiptSamples.length).toBeGreaterThanOrEqual(8);
    expect(punjabiB1ReceiptSamples.length).toBeLessThanOrEqual(10);
  });

  it("has unique ids and marks every card B1", () => {
    const ids = punjabiB1ReceiptSamples.map((card) => card.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const card of punjabiB1ReceiptSamples) {
      expect(card.level).toBe("B1");
    }
  });

  it("covers the required receipt focus areas", () => {
    const seen = new Set(punjabiB1ReceiptSamples.map((card) => card.focus));
...
describe("Punjabi B1 receipt samples - learner content", () => {
  for (const card of punjabiB1ReceiptSamples) {
    describe(card.id, () => {
      it("has bilingual receipt framing and Canada context", () => {
        expect(card.title_en.trim().length).toBeGreaterThan(0);
        expect(card.title_vi.trim().length).toBeGreaterThan(0);
        expect(card.receiptPurpose_en.trim().length).toBeGreaterThan(0);
        expect(card.receiptPurpose_vi.trim().length).toBeGreaterThan(0);
        expect(card.canadaContext.trim().length).toBeGreaterThan(0);
        expect(card.receiptNote_en.trim().length).toBeGreaterThan(0);
        expect(card.receiptNote_vi.trim().length).toBeGreaterThan(0);
      });
      ...
      it("includes learner traps and keeps receipt repair language concrete", () => {
        ...
        if (card.focus === "service_recovery" || card.focus === "issue_resolution") {
          expect(`${card.receiptPurpose_en} ${card.receiptPurpose_vi} ${card.qa.prompt_en}`).toMatch(
            /correct|refund|repair|replacement|adjust/i,
          );
        }
      });

      it("keeps register-safe and signoff items clearly receipt-ready", () => {
        if (card.focus !== "register_safe_repair" && card.focus !== "receipt_signoff") return;

        const serialized = JSON.stringify(card);
        expect(serialized).toMatch(/receipt/i);
...
describe("Punjabi B1 receipt scope", () => {
  it("states deferred native review and Shahmukhi awareness", () => {
    const serialized = JSON.stringify({
      scope: punjabiB1ReceiptScope,
      samples: punjabiB1ReceiptSamples,
    });
