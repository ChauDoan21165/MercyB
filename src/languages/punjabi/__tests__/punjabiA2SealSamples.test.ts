// src/languages/punjabi/__tests__/punjabiA2SealSamples.test.ts
//
// Structural guards for Punjabi A2 seal samples. This is Wave 52 only, not
// A11 integration. These verify app-consumable learner content only; native
// review is deferred.

import { describe, expect, it } from "vitest";

import { a2SealSamples } from "@/languages/punjabi/a2SealSamples";
...
describe("Punjabi A2 seal samples - batch shape", () => {
  it("ships compact seal samples for all required scenarios", () => {
    expect(a2SealSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2SealSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2SealSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2SealSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2SealSamples) {
...
describe("Punjabi A2 seal samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2SealSamples) {
      ...
      expect(nonEmpty(item.seal_goal_vi)).toBe(true);
      expect(nonEmpty(item.seal_goal_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2SealSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers pre-A11-seal, pre-A11-snapshot, pre-A11-closure, pre-merge, CI-readiness, and pre-integration styles", () => {
    const styles = new Set(a2SealSamples.map((item) => item.style));
