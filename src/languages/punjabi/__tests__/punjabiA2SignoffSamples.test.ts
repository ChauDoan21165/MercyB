// src/languages/punjabi/__tests__/punjabiA2SignoffSamples.test.ts
//
// Structural guards for Punjabi A2 signoff samples. This is Wave 53 only,
// not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2SignoffSamples } from "@/languages/punjabi/a2SignoffSamples";
...
describe("Punjabi A2 signoff samples - batch shape", () => {
  it("ships compact signoff samples for all required scenarios", () => {
    expect(a2SignoffSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2SignoffSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2SignoffSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2SignoffSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2SignoffSamples) {
...
describe("Punjabi A2 signoff samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2SignoffSamples) {
      ...
      expect(nonEmpty(item.signoff_goal_vi)).toBe(true);
      expect(nonEmpty(item.signoff_goal_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2SignoffSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers pre-A11-signoff, pre-A11-seal, pre-A11-snapshot, pre-merge, CI-readiness, and pre-integration styles", () => {
    const styles = new Set(a2SignoffSamples.map((item) => item.style));
    expect(styles.has("pre_a11_signoff")).toBe(true);
    expect(styles.has("pre_a11_seal")).toBe(true);
    expect(styles.has("pre_a11_snapshot")).toBe(true);
