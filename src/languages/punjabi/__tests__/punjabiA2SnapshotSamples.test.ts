// src/languages/punjabi/__tests__/punjabiA2SnapshotSamples.test.ts
//
// Structural guards for Punjabi A2 snapshot samples. This is Wave 51 only,
// not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2SnapshotSamples } from "@/languages/punjabi/a2SnapshotSamples";
...
describe("Punjabi A2 snapshot samples - batch shape", () => {
  it("ships compact snapshot samples for all required scenarios", () => {
    expect(a2SnapshotSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2SnapshotSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2SnapshotSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2SnapshotSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2SnapshotSamples) {
...
describe("Punjabi A2 snapshot samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2SnapshotSamples) {
      ...
      expect(nonEmpty(item.snapshot_goal_vi)).toBe(true);
      expect(nonEmpty(item.snapshot_goal_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2SnapshotSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers pre-A11-snapshot, pre-A11-closure, pre-merge, CI-readiness, and pre-integration styles", () => {
    const styles = new Set(a2SnapshotSamples.map((item) => item.style));
    expect(styles.has("pre_a11_snapshot")).toBe(true);
    expect(styles.has("pre_a11_closure")).toBe(true);
