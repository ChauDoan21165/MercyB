// src/languages/punjabi/__tests__/punjabiA2ReleaseCandidateSamples.test.ts
//
// Structural guards for Punjabi A2 release-candidate samples. This is Wave 37
// only, not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2ReleaseCandidateSamples } from "@/languages/punjabi/a2ReleaseCandidateSamples";
...
const STYLES = ["release_candidate", "closure_validation", "pre_integration"] as const;

describe("Punjabi A2 release-candidate samples - batch shape", () => {
  it("ships compact samples for all required practical-readiness scenarios", () => {
    expect(a2ReleaseCandidateSamples.length).toBeGreaterThanOrEqual(11);
    expect(a2ReleaseCandidateSamples.length).toBeLessThanOrEqual(13);

    const present = new Set(a2ReleaseCandidateSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2ReleaseCandidateSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2ReleaseCandidateSamples) {
...
describe("Punjabi A2 release-candidate samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2ReleaseCandidateSamples) {
      ...
      expect(nonEmpty(item.readiness_goal_vi)).toBe(true);
      expect(nonEmpty(item.readiness_goal_en)).toBe(true);
      expect(nonEmpty(item.final_check_vi)).toBe(true);
      expect(nonEmpty(item.final_check_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2ReleaseCandidateSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers release-candidate, closure-validation, and pre-integration styles", () => {
    const styles = new Set(a2ReleaseCandidateSamples.map((item) => item.style));
