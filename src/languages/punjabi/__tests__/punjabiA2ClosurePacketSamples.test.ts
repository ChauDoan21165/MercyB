// src/languages/punjabi/__tests__/punjabiA2ClosurePacketSamples.test.ts
//
// Structural guards for Punjabi A2 closure packet samples. This is Wave 50
// only, not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2ClosurePacketSamples } from "@/languages/punjabi/a2ClosurePacketSamples";
...
const STYLES = ["pre_a11_closure", "pre_merge", "ci_readiness", "pre_integration"] as const;

describe("Punjabi A2 closure packet samples - batch shape", () => {
  it("ships compact closure samples for all required scenarios", () => {
    expect(a2ClosurePacketSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2ClosurePacketSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2ClosurePacketSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2ClosurePacketSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2ClosurePacketSamples) {
...
describe("Punjabi A2 closure packet samples - learner contract", () => {
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2ClosurePacketSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers pre-A11-closure, pre-merge, CI-readiness, and pre-integration styles", () => {
    const styles = new Set(a2ClosurePacketSamples.map((item) => item.style));
