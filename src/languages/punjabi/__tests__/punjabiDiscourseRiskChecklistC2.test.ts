// Punjabi C2 discourse risk checklist guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_DISCOURSE_RISK_CHECKLIST_DISCLAIMER,
  discourseRiskChecklistC2,
  discourseRiskChecklistC2ByFocus,
  type PunjabiC2DiscourseRiskChecklist,
  type PunjabiC2RiskFocus,
} from "@/languages/punjabi/discourseRiskChecklistC2";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2RiskFocus[] = [
...
describe("Punjabi C2 discourse risk checklist - coverage", () => {
  it("ships a compact app-consumable checklist set", () => {
    expect(discourseRiskChecklistC2.length).toBeGreaterThanOrEqual(10);
    expect(discourseRiskChecklistC2.length).toBeLessThanOrEqual(12);
  });

  it("covers every required discourse risk focus", () => {
    const seen = new Set(discourseRiskChecklistC2.map((item) => item.focus));
  ...
  it("has unique non-empty ids", () => {
    const ids = discourseRiskChecklistC2.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it("discourseRiskChecklistC2ByFocus returns only matching items", () => {
    for (const focus of REQUIRED_FOCUSES) {
      const subset = discourseRiskChecklistC2ByFocus(focus);
...
describe("Punjabi C2 discourse risk checklist - bilingual integrity", () => {
  it("each item has VI+EN scenario and risk goal", () => {
    for (const item of discourseRiskChecklistC2) {
      ...
      expect(item.risk_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.risk_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
    }
  });
  ...
  it("each checklist phrase has Gurmukhi, romanization, VI, and EN", () => {
    for (const item of discourseRiskChecklistC2) {
      expect(item.checklist_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.checklist_phrases) {
  ...
  it("includes final-stability, boundary, checklist, and regression styles", () => {
    const styles = new Set(discourseRiskChecklistC2.map((item) => item.style));
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(discourseRiskChecklistC2.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(8);
    const canadaItems = discourseRiskChecklistC2.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(3);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 discourse risk checklist - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_DISCOURSE_RISK_CHECKLIST_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_DISCOURSE_RISK_CHECKLIST_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_DISCOURSE_RISK_CHECKLIST_DISCLAIMER.vi} ${C2_DISCOURSE_RISK_CHECKLIST_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(discourseRiskChecklistC2).toLowerCase();
...
const _typecheck: PunjabiC2DiscourseRiskChecklist[] = discourseRiskChecklistC2;
void _typecheck;
