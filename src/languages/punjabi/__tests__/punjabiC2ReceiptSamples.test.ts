...
import {
  C2_RECEIPT_SAMPLES_DISCLAIMER,
  c2ReceiptSamples,
  c2ReceiptSamplesByFocus,
  c2ReceiptSamplesByStyle,
  type PunjabiC2ReceiptContext,
  type PunjabiC2ReceiptFocus,
  type PunjabiC2ReceiptSample,
  type PunjabiC2ReceiptStyle,
} from "@/languages/punjabi/c2ReceiptSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2ReceiptFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2ReceiptContext[] = ["public", "professional", "community"];
const REQUIRED_STYLES: PunjabiC2ReceiptStyle[] = [
  "pre_a11_receipt",
  "ledger_receipt",
  "archive_receipt",
  "pre_integration",
...
describe("Punjabi C2 receipt samples - coverage", () => {
  it("ships a compact app-consumable receipt sample set", () => {
    expect(c2ReceiptSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2ReceiptSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required receipt focus", () => {
    const seen = new Set(c2ReceiptSamples.map((item) => item.focus));
    for (const focus of REQUIRED_FOCUSES) {
      expect(seen.has(focus), `missing focus ${focus}`).toBe(true);
      expect(c2ReceiptSamplesByFocus(focus).every((item) => item.focus === focus)).toBe(true);
    }
  });

  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2ReceiptSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
    }
  });

  it("covers pre-A11 receipt, ledger receipt, archive receipt, and pre-integration styles", () => {
    const seen = new Set(c2ReceiptSamples.map((item) => item.style));
    for (const style of REQUIRED_STYLES) {
      expect(seen.has(style), `missing style ${style}`).toBe(true);
      expect(c2ReceiptSamplesByStyle(style).every((item) => item.style === style)).toBe(true);
    }
  });

  it("has unique ids and stable filters", () => {
    const ids = c2ReceiptSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
  });
});

describe("Punjabi C2 receipt samples - bilingual integrity", () => {
  it("each item has VI+EN scenario, receipt goal, and usable sample strings", () => {
    for (const item of c2ReceiptSamples) {
      ...
      expect(item.receipt_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.receipt_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
  ...
  it("each receipt phrase carries Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2ReceiptSamples) {
      expect(item.receipt_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.receipt_phrases) {
  ...
  it("each sample carries checks, learner traps, and Canada-practical examples where useful", () => {
    const canadaSamples = c2ReceiptSamples.filter((item) => item.canada_practical);
    expect(canadaSamples.length).toBeGreaterThanOrEqual(3);

    for (const item of c2ReceiptSamples) {
      expect(item.receipt_checks.length, `${item.id} checks`).toBeGreaterThanOrEqual(1);
      for (const check of item.receipt_checks) {
...
describe("Punjabi C2 receipt samples - scope framing", () => {
  it("states deferred native review and Shahmukhi awareness", () => {
    const blob = JSON.stringify({
      disclaimer: C2_RECEIPT_SAMPLES_DISCLAIMER,
      samples: c2ReceiptSamples,
    }).toLowerCase();
  ...
  it("keeps forbidden integration and scoring concepts out", () => {
    const blob = JSON.stringify(c2ReceiptSamples);
    expect(blob).not.toMatch(/audio|pronunciation scoring|Azure|auth|billing|RLS|Supabase|CI config/i);
  });
});

const _typecheck: PunjabiC2ReceiptSample[] = c2ReceiptSamples;
void _typecheck;
