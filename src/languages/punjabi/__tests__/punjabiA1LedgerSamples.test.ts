...
import punjabiA1LedgerSamples, {
  ledgerScriptAwareness,
  punjabiA1LedgerSamples as namedLedgerSamples,
  type PunjabiA1LedgerDomain,
  type PunjabiA1LedgerStyle,
} from "@/languages/punjabi/a1LedgerSamples";

const GURMUKHI_SCRIPT = /[\u0A00-\u0A7F]/;
const LATIN = /[a-zA-Z]/;

describe("Punjabi A1 ledger samples", () => {
  it("exports app-consumable named and default arrays", () => {
    expect(punjabiA1LedgerSamples).toBe(namedLedgerSamples);
    expect(Array.isArray(punjabiA1LedgerSamples)).toBe(true);
  });

  it("is compact and covers the required ledger domains", () => {
    expect(punjabiA1LedgerSamples.length).toBeGreaterThanOrEqual(12);
    expect(punjabiA1LedgerSamples.length).toBeLessThanOrEqual(14);

    const requiredDomains: PunjabiA1LedgerDomain[] = [
      "greetings",
      "identity",
      "family",
      "numbers",
    ...
    const domains = new Set(punjabiA1LedgerSamples.map((item) => item.domain));
  ...
  it("includes pre-A11-ledger, pre-snapshot, pre-signoff, pre-seal, closure-packet, pre-merge, pre-integration, qa, and readiness styles", () => {
    const requiredStyles: PunjabiA1LedgerStyle[] = [
      "pre_a11_ledger",
      "pre_snapshot",
      "pre_signoff",
      "pre_seal",
    ...
    const styles = new Set(punjabiA1LedgerSamples.map((item) => item.style));
  ...
  it("uses Gurmukhi primary with bilingual support and review links", () => {
    for (const item of punjabiA1LedgerSamples) {
      expect(item.id).toMatch(/^pa_a1_ledger_/);
      ...
      expect(item.ledger_check_vi.trim().length).toBeGreaterThan(0);
      expect(item.ledger_check_en.trim().length).toBeGreaterThan(0);
  ...
  it("includes traps, romanization bridge, and Canada-practical ledger items", () => {
    const traps = punjabiA1LedgerSamples.flatMap((item) => (item.trap ? [item.trap] : []));
    const trapAudiences = new Set(traps.map((trap) => trap.audience));
    const canadaItems = punjabiA1LedgerSamples.filter((item) => item.canada_practical);
    const romanizationBridge = punjabiA1LedgerSamples.find((item) => item.domain === "romanization_bridge");
  ...
  it("verifies beginner readiness across required ledger situations", () => {
    const allText = JSON.stringify(punjabiA1LedgerSamples);
    ...
    expect(allText).toMatch(/ledger|snapshot|signoff|seal|pre_a11_ledger|pre_snapshot|pre_signoff|pre_seal|closure_packet|pre_merge|pre-integration|Sẵn sàng|Ready/i);
  });

  it("mentions Shahmukhi only as awareness and avoids disallowed claims and systems", () => {
    const allText = `${ledgerScriptAwareness} ${JSON.stringify(punjabiA1LedgerSamples)}`;

    expect(ledgerScriptAwareness).toContain("Shahmukhi");
    expect(ledgerScriptAwareness).toContain("awareness only");
