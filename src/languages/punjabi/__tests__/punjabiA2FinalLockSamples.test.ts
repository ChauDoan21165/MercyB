// src/languages/punjabi/__tests__/punjabiA2FinalLockSamples.test.ts
//
// Structural guards for Punjabi A2 final-lock samples. This is Wave 42 only,
// not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2FinalLockSamples } from "@/languages/punjabi/a2FinalLockSamples";
...
const STYLES = ["final_lock", "owner_acceptance", "final_acceptance", "pre_integration"] as const;

describe("Punjabi A2 final-lock samples - batch shape", () => {
  it("ships compact samples for all required freeze-readiness scenarios", () => {
    expect(a2FinalLockSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2FinalLockSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2FinalLockSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2FinalLockSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2FinalLockSamples) {
...
describe("Punjabi A2 final-lock samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2FinalLockSamples) {
      ...
      expect(nonEmpty(item.lock_goal_vi)).toBe(true);
      expect(nonEmpty(item.lock_goal_en)).toBe(true);
      expect(nonEmpty(item.freeze_ready_vi)).toBe(true);
      expect(nonEmpty(item.freeze_ready_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2FinalLockSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers final-lock, owner-acceptance, final-acceptance, and pre-integration styles", () => {
    const styles = new Set(a2FinalLockSamples.map((item) => item.style));
