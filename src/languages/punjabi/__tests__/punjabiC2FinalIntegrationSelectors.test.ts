// Punjabi C2 final integration selector guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_FINAL_INTEGRATION_SELECTORS_DISCLAIMER,
  c2FinalIntegrationSelectors,
  c2FinalIntegrationSelectorsByFocus,
  type PunjabiC2FinalIntegrationSelector,
  type PunjabiC2FinalSelectorFocus,
} from "@/languages/punjabi/c2FinalIntegrationSelectors";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2FinalSelectorFocus[] = [
...
describe("Punjabi C2 final integration selectors - coverage", () => {
  it("ships a compact app-consumable selector set", () => {
    expect(c2FinalIntegrationSelectors.length).toBeGreaterThanOrEqual(10);
    expect(c2FinalIntegrationSelectors.length).toBeLessThanOrEqual(12);
  });

  it("covers every required selector focus", () => {
    const seen = new Set(c2FinalIntegrationSelectors.map((item) => item.focus));
  ...
  it("has unique non-empty ids", () => {
    const ids = c2FinalIntegrationSelectors.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it("c2FinalIntegrationSelectorsByFocus returns only matching items", () => {
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2FinalIntegrationSelectorsByFocus(focus);
...
describe("Punjabi C2 final integration selectors - bilingual integrity", () => {
  it("each item has VI+EN scenario and selector goal", () => {
    for (const item of c2FinalIntegrationSelectors) {
      ...
      expect(item.selector_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.selector_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
    }
  });
  ...
  it("each selector phrase has Gurmukhi, romanization, VI, and EN", () => {
    for (const item of c2FinalIntegrationSelectors) {
      expect(item.selector_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.selector_phrases) {
  ...
  it("includes selector, pre-integration, final-readiness, and regression styles", () => {
    const styles = new Set(c2FinalIntegrationSelectors.map((item) => item.style));
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2FinalIntegrationSelectors.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(8);
    const canadaItems = c2FinalIntegrationSelectors.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(3);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 final integration selectors - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_FINAL_INTEGRATION_SELECTORS_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_FINAL_INTEGRATION_SELECTORS_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_FINAL_INTEGRATION_SELECTORS_DISCLAIMER.vi} ${C2_FINAL_INTEGRATION_SELECTORS_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2FinalIntegrationSelectors).toLowerCase();
...
const _typecheck: PunjabiC2FinalIntegrationSelector[] = c2FinalIntegrationSelectors;
void _typecheck;
