// Punjabi C2 can-do statement guards. These validate app-consumable structure
// and scope, not certification, official placement, or native-level authority.

import { describe, expect, it } from "vitest";

import {
  C2_CAN_DO_STATEMENTS_DISCLAIMER,
  canDoStatementsC2,
  canDoStatementsC2ByFocus,
  type PunjabiC2CanDoFocus,
  type PunjabiC2CanDoStatement,
} from "@/languages/punjabi/canDoStatementsC2";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2CanDoFocus[] = [
...
describe("Punjabi C2 can-do statements — coverage", () => {
  it("ships a compact app-consumable can-do pack", () => {
    expect(canDoStatementsC2.length).toBeGreaterThanOrEqual(8);
    expect(canDoStatementsC2.length).toBeLessThanOrEqual(14);
  });

  it("covers every required C2 can-do focus", () => {
    const seen = new Set(canDoStatementsC2.map((statement) => statement.focus));
  ...
  it("has unique non-empty ids", () => {
    const ids = canDoStatementsC2.map((statement) => statement.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it("canDoStatementsC2ByFocus returns only matching statements", () => {
    for (const focus of REQUIRED_FOCUSES) {
      const subset = canDoStatementsC2ByFocus(focus);
...
describe("Punjabi C2 can-do statements — bilingual integrity", () => {
  it("each statement has VI+EN title, can-do statement, and learner context", () => {
    for (const statement of canDoStatementsC2) {
  ...
  it("includes can-do checkpoint and readiness style items", () => {
    for (const statement of canDoStatementsC2) {
    ...
    const statuses = new Set(canDoStatementsC2.map((statement) => statement.readiness.status_if_missing));
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(canDoStatementsC2.filter((statement) => statement.learner_trap).length).toBeGreaterThanOrEqual(5);
    const canadaStatements = canDoStatementsC2.filter((statement) => statement.canada_practical);
...
describe("Punjabi C2 can-do statements — scope framing", () => {
  it("exposes self-study, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_CAN_DO_STATEMENTS_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_CAN_DO_STATEMENTS_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_CAN_DO_STATEMENTS_DISCLAIMER.vi} ${C2_CAN_DO_STATEMENTS_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, or official placement in statements", () => {
    const blob = JSON.stringify(canDoStatementsC2).toLowerCase();
...
const _typecheck: PunjabiC2CanDoStatement[] = canDoStatementsC2;
void _typecheck;
