// Punjabi C2 final regression samples guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_FINAL_REGRESSION_SAMPLES_DISCLAIMER,
  c2FinalRegressionSamples,
  c2FinalRegressionSamplesByFocus,
  type PunjabiC2FinalRegressionSample,
  type PunjabiC2RegressionFocus,
} from "@/languages/punjabi/c2FinalRegressionSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2RegressionFocus[] = [
...
describe("Punjabi C2 final regression samples - coverage", () => {
  it("ships a compact app-consumable regression set", () => {
    expect(c2FinalRegressionSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2FinalRegressionSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required regression focus", () => {
    const seen = new Set(c2FinalRegressionSamples.map((item) => item.focus));
  ...
  it("has unique non-empty ids", () => {
    const ids = c2FinalRegressionSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it("c2FinalRegressionSamplesByFocus returns only matching items", () => {
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2FinalRegressionSamplesByFocus(focus);
...
describe("Punjabi C2 final regression samples - bilingual integrity", () => {
  it("each item has VI+EN scenario and regression goal", () => {
    for (const item of c2FinalRegressionSamples) {
      ...
      expect(item.regression_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.regression_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
    }
  });
  ...
  it("each regression phrase has Gurmukhi, romanization, VI, and EN", () => {
    for (const item of c2FinalRegressionSamples) {
      expect(item.regression_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.regression_phrases) {
  ...
  it("includes final-regression, sanity, pre-integration, and regression styles", () => {
    const styles = new Set(c2FinalRegressionSamples.map((item) => item.style));
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2FinalRegressionSamples.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2FinalRegressionSamples.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 final regression samples - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_FINAL_REGRESSION_SAMPLES_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_FINAL_REGRESSION_SAMPLES_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_FINAL_REGRESSION_SAMPLES_DISCLAIMER.vi} ${C2_FINAL_REGRESSION_SAMPLES_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2FinalRegressionSamples).toLowerCase();
...
const _typecheck: PunjabiC2FinalRegressionSample[] = c2FinalRegressionSamples;
void _typecheck;
