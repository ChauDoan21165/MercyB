...
import punjabiA1IntegrationDryRunSet, {
  integrationDryRunScriptAwareness,
  punjabiA1IntegrationDryRunSet as namedIntegrationDryRunSet,
  type PunjabiA1IntegrationDryRunDomain,
  type PunjabiA1IntegrationDryRunStyle,
} from "@/languages/punjabi/a1IntegrationDryRunSet";

const GURMUKHI_SCRIPT = /[\u0A00-\u0A7F]/;
const LATIN = /[a-zA-Z]/;

describe("Punjabi A1 integration dry run set", () => {
  it("exports app-consumable named and default arrays", () => {
    expect(punjabiA1IntegrationDryRunSet).toBe(namedIntegrationDryRunSet);
    expect(Array.isArray(punjabiA1IntegrationDryRunSet)).toBe(true);
  });

  it("is compact and covers the required dry-run domains", () => {
    expect(punjabiA1IntegrationDryRunSet.length).toBeGreaterThanOrEqual(9);
    expect(punjabiA1IntegrationDryRunSet.length).toBeLessThanOrEqual(14);

    const requiredDomains: PunjabiA1IntegrationDryRunDomain[] = [
      "greetings",
      "identity",
      "family",
      "numbers",
    ...
    const domains = new Set(punjabiA1IntegrationDryRunSet.map((item) => item.domain));
  ...
  it("includes dry-run, pre-integration, final-readiness, selector, and quality-gate styles", () => {
    const requiredStyles: PunjabiA1IntegrationDryRunStyle[] = [
      "dry_run",
      "pre_integration",
      "final_readiness",
      "selector",
    ...
    const styles = new Set(punjabiA1IntegrationDryRunSet.map((item) => item.style));
  ...
  it("uses Gurmukhi primary with bilingual support and review links", () => {
    for (const item of punjabiA1IntegrationDryRunSet) {
      expect(item.id).toMatch(/^pa_a1_dryrun_/);
      ...
      expect(item.why_dry_run_vi.trim().length).toBeGreaterThan(0);
      expect(item.why_dry_run_en.trim().length).toBeGreaterThan(0);
  ...
  it("includes traps and Canada-practical selector items", () => {
    const traps = punjabiA1IntegrationDryRunSet.flatMap((item) => item.trap ? [item.trap] : []);
    const trapAudiences = new Set(traps.map((trap) => trap.audience));
    const canadaItems = punjabiA1IntegrationDryRunSet.filter((item) => item.canada_practical);
  ...
  it("proves the A1 dry-run set across key survival phrases", () => {
    const allText = JSON.stringify(punjabiA1IntegrationDryRunSet);
  ...
  it("mentions Shahmukhi only as awareness and avoids disallowed claims/systems", () => {
    const allText = `${integrationDryRunScriptAwareness} ${JSON.stringify(punjabiA1IntegrationDryRunSet)}`;

    expect(integrationDryRunScriptAwareness).toContain("Shahmukhi");
    expect(integrationDryRunScriptAwareness).toContain("awareness only");
