// Punjabi C2 Pipeline-readiness sample guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_PIPELINE_READINESS_SAMPLES_DISCLAIMER,
  c2PipelineReadinessSamples,
  c2PipelineReadinessSamplesByContext,
  c2PipelineReadinessSamplesByFocus,
  type PunjabiC2PipelineReadinessContext,
  type PunjabiC2PipelineReadinessFocus,
  type PunjabiC2PipelineReadinessSample,
} from "@/languages/punjabi/c2PipelineReadinessSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2PipelineReadinessFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2PipelineReadinessContext[] = ["public", "professional", "community"];

describe("Punjabi C2 Pipeline-readiness samples - coverage", () => {
  it("ships a compact app-consumable Pipeline-readiness sample set", () => {
    expect(c2PipelineReadinessSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2PipelineReadinessSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required Pipeline-readiness focus", () => {
    const seen = new Set(c2PipelineReadinessSamples.map((item) => item.focus));
  ...
  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2PipelineReadinessSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
      expect(c2PipelineReadinessSamplesByContext(context).every((item) => item.context === context)).toBe(true);
    }
  });

  it("has unique ids and focus filter", () => {
    const ids = c2PipelineReadinessSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2PipelineReadinessSamplesByFocus(focus);
...
describe("Punjabi C2 Pipeline-readiness samples - bilingual integrity", () => {
  it("each item has VI+EN scenario and Pipeline-readiness goal", () => {
    for (const item of c2PipelineReadinessSamples) {
  ...
  it("each sample and phrase has Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2PipelineReadinessSamples) {
      ...
      expect(item.pipeline_readiness_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.pipeline_readiness_phrases) {
  ...
  it("includes Pipeline-readiness, CI-readiness, MR-readiness, and pre-integration styles", () => {
    const styles = new Set(c2PipelineReadinessSamples.map((item) => item.style));
  ...
  it("each item carries Pipeline-readiness checks", () => {
    for (const item of c2PipelineReadinessSamples) {
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2PipelineReadinessSamples.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2PipelineReadinessSamples.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 Pipeline-readiness samples - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_PIPELINE_READINESS_SAMPLES_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_PIPELINE_READINESS_SAMPLES_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_PIPELINE_READINESS_SAMPLES_DISCLAIMER.vi} ${C2_PIPELINE_READINESS_SAMPLES_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2PipelineReadinessSamples).toLowerCase();
...
const _typecheck: PunjabiC2PipelineReadinessSample[] = c2PipelineReadinessSamples;
void _typecheck;
