// Punjabi C2 pre-merge sample guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_PRE_MERGE_SAMPLES_DISCLAIMER,
  c2PreMergeSamples,
  c2PreMergeSamplesByContext,
  c2PreMergeSamplesByFocus,
  type PunjabiC2PreMergeContext,
  type PunjabiC2PreMergeFocus,
  type PunjabiC2PreMergeSample,
} from "@/languages/punjabi/c2PreMergeSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2PreMergeFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2PreMergeContext[] = ["public", "professional", "community"];

describe("Punjabi C2 pre-merge samples - coverage", () => {
  it("ships a compact app-consumable pre-merge sample set", () => {
    expect(c2PreMergeSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2PreMergeSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required pre-merge focus", () => {
    const seen = new Set(c2PreMergeSamples.map((item) => item.focus));
  ...
  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2PreMergeSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
      expect(c2PreMergeSamplesByContext(context).every((item) => item.context === context)).toBe(true);
    }
  });

  it("has unique ids and focus filter", () => {
    const ids = c2PreMergeSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2PreMergeSamplesByFocus(focus);
...
describe("Punjabi C2 pre-merge samples - bilingual integrity", () => {
  it("each item has VI+EN scenario and pre-merge goal", () => {
    for (const item of c2PreMergeSamples) {
  ...
  it("each sample and phrase has Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2PreMergeSamples) {
      ...
      expect(item.pre_merge_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.pre_merge_phrases) {
  ...
  it("includes pre-merge, runner-readiness, pipeline-readiness, and pre-integration styles", () => {
    const styles = new Set(c2PreMergeSamples.map((item) => item.style));
  ...
  it("each item carries pre-merge checks", () => {
    for (const item of c2PreMergeSamples) {
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2PreMergeSamples.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2PreMergeSamples.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 pre-merge samples - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_PRE_MERGE_SAMPLES_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_PRE_MERGE_SAMPLES_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_PRE_MERGE_SAMPLES_DISCLAIMER.vi} ${C2_PRE_MERGE_SAMPLES_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2PreMergeSamples).toLowerCase();
...
const _typecheck: PunjabiC2PreMergeSample[] = c2PreMergeSamples;
void _typecheck;
