// src/languages/punjabi/__tests__/punjabiA2FinalFreezeSamples.test.ts
//
// Structural guards for Punjabi A2 final-freeze samples. This is Wave 43 only,
// not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2FinalFreezeSamples } from "@/languages/punjabi/a2FinalFreezeSamples";
...
const STYLES = ["final_freeze", "final_lock", "owner_acceptance", "pre_integration"] as const;

describe("Punjabi A2 final-freeze samples - batch shape", () => {
  it("ships compact samples for all required freeze-stability scenarios", () => {
    expect(a2FinalFreezeSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2FinalFreezeSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2FinalFreezeSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2FinalFreezeSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2FinalFreezeSamples) {
...
describe("Punjabi A2 final-freeze samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2FinalFreezeSamples) {
      ...
      expect(nonEmpty(item.freeze_goal_vi)).toBe(true);
      expect(nonEmpty(item.freeze_goal_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2FinalFreezeSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers final-freeze, final-lock, owner-acceptance, and pre-integration styles", () => {
    const styles = new Set(a2FinalFreezeSamples.map((item) => item.style));
