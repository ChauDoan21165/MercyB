// src/languages/punjabi/__tests__/punjabiA2OwnerAcceptanceSamples.test.ts
//
// Structural guards for Punjabi A2 owner-acceptance samples. This is Wave 41
// only, not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2OwnerAcceptanceSamples } from "@/languages/punjabi/a2OwnerAcceptanceSamples";
...
const STYLES = ["owner_acceptance", "final_acceptance", "ship_candidate", "pre_integration"] as const;

describe("Punjabi A2 owner-acceptance samples - batch shape", () => {
  it("ships compact samples for all required practical learner acceptance scenarios", () => {
    expect(a2OwnerAcceptanceSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2OwnerAcceptanceSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2OwnerAcceptanceSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2OwnerAcceptanceSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2OwnerAcceptanceSamples) {
...
describe("Punjabi A2 owner-acceptance samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2OwnerAcceptanceSamples) {
      ...
      expect(nonEmpty(item.owner_goal_vi)).toBe(true);
      expect(nonEmpty(item.owner_goal_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2OwnerAcceptanceSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers owner-acceptance, final-acceptance, ship-candidate, and pre-integration styles", () => {
    const styles = new Set(a2OwnerAcceptanceSamples.map((item) => item.style));
