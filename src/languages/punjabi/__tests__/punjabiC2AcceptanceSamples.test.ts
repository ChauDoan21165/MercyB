// Punjabi C2 acceptance sample guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_ACCEPTANCE_SAMPLES_DISCLAIMER,
  c2AcceptanceSamples,
  c2AcceptanceSamplesByContext,
  c2AcceptanceSamplesByFocus,
  type PunjabiC2AcceptanceContext,
  type PunjabiC2AcceptanceFocus,
  type PunjabiC2AcceptanceSample,
} from "@/languages/punjabi/c2AcceptanceSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2AcceptanceFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2AcceptanceContext[] = ["public", "professional", "community"];

describe("Punjabi C2 acceptance samples - coverage", () => {
  it("ships a compact app-consumable acceptance set", () => {
    expect(c2AcceptanceSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2AcceptanceSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required acceptance focus", () => {
    const seen = new Set(c2AcceptanceSamples.map((item) => item.focus));
  ...
  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2AcceptanceSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
      expect(c2AcceptanceSamplesByContext(context).every((item) => item.context === context)).toBe(true);
    }
  });

  it("has unique ids and focus filter", () => {
    const ids = c2AcceptanceSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2AcceptanceSamplesByFocus(focus);
...
describe("Punjabi C2 acceptance samples - bilingual integrity", () => {
  it("each item has VI+EN scenario and ship goal", () => {
    for (const item of c2AcceptanceSamples) {
  ...
  it("each sample and phrase has Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2AcceptanceSamples) {
      ...
      expect(item.acceptance_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.acceptance_phrases) {
  ...
  it("includes acceptance, go-no-go, release-candidate, and pre-integration styles", () => {
    const styles = new Set(c2AcceptanceSamples.map((item) => item.style));
  ...
  it("each item carries acceptance checks", () => {
    for (const item of c2AcceptanceSamples) {
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2AcceptanceSamples.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2AcceptanceSamples.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 acceptance samples - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_ACCEPTANCE_SAMPLES_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_ACCEPTANCE_SAMPLES_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_ACCEPTANCE_SAMPLES_DISCLAIMER.vi} ${C2_ACCEPTANCE_SAMPLES_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2AcceptanceSamples).toLowerCase();
...
const _typecheck: PunjabiC2AcceptanceSample[] = c2AcceptanceSamples;
void _typecheck;
