// src/languages/punjabi/__tests__/punjabiA2PreMergeSamples.test.ts
//
// Structural guards for Punjabi A2 pre-merge samples. This is Wave 49 only,
// not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2PreMergeSamples } from "@/languages/punjabi/a2PreMergeSamples";
...
const STYLES = ["pre_merge", "pre_a11_checksum", "runner_readiness", "pre_integration"] as const;

describe("Punjabi A2 pre-merge samples - batch shape", () => {
  it("ships compact pre-merge samples for all required scenarios", () => {
    expect(a2PreMergeSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2PreMergeSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2PreMergeSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2PreMergeSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2PreMergeSamples) {
...
describe("Punjabi A2 pre-merge samples - learner contract", () => {
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2PreMergeSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers pre-merge, pre-A11-checksum, runner-readiness, and pre-integration styles", () => {
    const styles = new Set(a2PreMergeSamples.map((item) => item.style));
