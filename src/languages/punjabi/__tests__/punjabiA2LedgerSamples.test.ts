// src/languages/punjabi/__tests__/punjabiA2LedgerSamples.test.ts
//
// Structural guards for Punjabi A2 ledger samples. This is Wave 55 only,
// not A11 integration. Native review is deferred.

import { describe, expect, it } from "vitest";

import { a2LedgerSamples } from "@/languages/punjabi/a2LedgerSamples";
...
describe("Punjabi A2 ledger samples - batch shape", () => {
  it("ships compact ledger samples for all required scenarios", () => {
    expect(a2LedgerSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2LedgerSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2LedgerSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2LedgerSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2LedgerSamples) {
...
describe("Punjabi A2 ledger samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2LedgerSamples) {
      ...
      expect(nonEmpty(item.ledger_goal_vi)).toBe(true);
      expect(nonEmpty(item.ledger_goal_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2LedgerSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers ledger, signoff, copy, pre-merge, QA, pre-integration, and readiness styles", () => {
    const styles = new Set(a2LedgerSamples.map((item) => item.style));
    expect(styles.has("pre_a11_ledger")).toBe(true);
    expect(styles.has("signoff")).toBe(true);
    expect(styles.has("ledger_copy")).toBe(true);
