// Punjabi C2 CI-readiness sample guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_CI_READINESS_SAMPLES_DISCLAIMER,
  c2CiReadinessSamples,
  c2CiReadinessSamplesByContext,
  c2CiReadinessSamplesByFocus,
  type PunjabiC2CiReadinessContext,
  type PunjabiC2CiReadinessFocus,
  type PunjabiC2CiReadinessSample,
} from "@/languages/punjabi/c2CiReadinessSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2CiReadinessFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2CiReadinessContext[] = ["public", "professional", "community"];

describe("Punjabi C2 CI-readiness samples - coverage", () => {
  it("ships a compact app-consumable CI-readiness sample set", () => {
    expect(c2CiReadinessSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2CiReadinessSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required CI-readiness focus", () => {
    const seen = new Set(c2CiReadinessSamples.map((item) => item.focus));
  ...
  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2CiReadinessSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
      expect(c2CiReadinessSamplesByContext(context).every((item) => item.context === context)).toBe(true);
    }
  });

  it("has unique ids and focus filter", () => {
    const ids = c2CiReadinessSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2CiReadinessSamplesByFocus(focus);
...
describe("Punjabi C2 CI-readiness samples - bilingual integrity", () => {
  it("each item has VI+EN scenario and CI-readiness goal", () => {
    for (const item of c2CiReadinessSamples) {
  ...
  it("each sample and phrase has Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2CiReadinessSamples) {
      ...
      expect(item.ci_readiness_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.ci_readiness_phrases) {
  ...
  it("includes CI-readiness, MR-readiness, final-freeze, and pre-integration styles", () => {
    const styles = new Set(c2CiReadinessSamples.map((item) => item.style));
  ...
  it("each item carries CI-readiness checks", () => {
    for (const item of c2CiReadinessSamples) {
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2CiReadinessSamples.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2CiReadinessSamples.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 CI-readiness samples - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_CI_READINESS_SAMPLES_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_CI_READINESS_SAMPLES_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_CI_READINESS_SAMPLES_DISCLAIMER.vi} ${C2_CI_READINESS_SAMPLES_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2CiReadinessSamples).toLowerCase();
...
const _typecheck: PunjabiC2CiReadinessSample[] = c2CiReadinessSamples;
void _typecheck;
