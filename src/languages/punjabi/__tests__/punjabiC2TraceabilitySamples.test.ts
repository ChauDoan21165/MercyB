...
import {
  C2_TRACEABILITY_SAMPLES_DISCLAIMER,
  c2TraceabilitySamples,
  c2TraceabilitySamplesByFocus,
  c2TraceabilitySamplesByStyle,
  type PunjabiC2TraceabilityContext,
  type PunjabiC2TraceabilityFocus,
  type PunjabiC2TraceabilitySample,
  type PunjabiC2TraceabilityStyle,
} from "@/languages/punjabi/c2TraceabilitySamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2TraceabilityFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2TraceabilityContext[] = ["public", "professional", "community"];
const REQUIRED_STYLES: PunjabiC2TraceabilityStyle[] = [
...
describe("Punjabi C2 traceability samples - coverage", () => {
  it("ships a compact app-consumable traceability sample set", () => {
    expect(c2TraceabilitySamples.length).toBeGreaterThanOrEqual(8);
    expect(c2TraceabilitySamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required traceability focus", () => {
    const seen = new Set(c2TraceabilitySamples.map((item) => item.focus));
    for (const focus of REQUIRED_FOCUSES) {
      expect(seen.has(focus), `missing focus ${focus}`).toBe(true);
      expect(c2TraceabilitySamplesByFocus(focus).every((item) => item.focus === focus)).toBe(true);
    }
  });

  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2TraceabilitySamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
    }
  });

  it("covers pre-A11 traceability, evidence receipt trace, completion record trace, and pre-integration styles", () => {
    const seen = new Set(c2TraceabilitySamples.map((item) => item.style));
    for (const style of REQUIRED_STYLES) {
      expect(seen.has(style), `missing style ${style}`).toBe(true);
      expect(c2TraceabilitySamplesByStyle(style).every((item) => item.style === style)).toBe(true);
    }
  });

  it("has unique ids and stable filters", () => {
    const ids = c2TraceabilitySamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
  });
});

describe("Punjabi C2 traceability samples - bilingual integrity", () => {
  it("each item has VI+EN scenario, traceability goal, and usable sample strings", () => {
    for (const item of c2TraceabilitySamples) {
      ...
      expect(item.traceability_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.traceability_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
  ...
  it("each traceability phrase carries Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2TraceabilitySamples) {
      expect(item.traceability_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.traceability_phrases) {
  ...
  it("each sample carries checks, learner traps, and Canada-practical examples where useful", () => {
    const canadaSamples = c2TraceabilitySamples.filter((item) => item.canada_practical);
    expect(canadaSamples.length).toBeGreaterThanOrEqual(3);

    for (const item of c2TraceabilitySamples) {
      expect(item.traceability_checks.length, `${item.id} checks`).toBeGreaterThanOrEqual(1);
      for (const check of item.traceability_checks) {
...
describe("Punjabi C2 traceability samples - scope framing", () => {
  it("states deferred native review and Shahmukhi awareness", () => {
    const blob = JSON.stringify({
      disclaimer: C2_TRACEABILITY_SAMPLES_DISCLAIMER,
      samples: c2TraceabilitySamples,
    }).toLowerCase();
  ...
  it("keeps forbidden integration and scoring concepts out", () => {
    const blob = JSON.stringify(c2TraceabilitySamples);
    expect(blob).not.toMatch(/audio|pronunciation scoring|Azure|auth|billing|RLS|Supabase|CI config/i);
  });
});

const _typecheck: PunjabiC2TraceabilitySample[] = c2TraceabilitySamples;
void _typecheck;
