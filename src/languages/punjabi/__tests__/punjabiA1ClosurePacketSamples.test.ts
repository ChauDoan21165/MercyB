...
import punjabiA1ClosurePacketSamples, {
  closurePacketScriptAwareness,
  punjabiA1ClosurePacketSamples as namedClosurePacketSamples,
  type PunjabiA1ClosurePacketDomain,
  type PunjabiA1ClosurePacketStyle,
} from "@/languages/punjabi/a1ClosurePacketSamples";

const GURMUKHI_SCRIPT = /[\u0A00-\u0A7F]/;
const LATIN = /[a-zA-Z]/;

describe("Punjabi A1 closure-packet samples", () => {
  it("exports app-consumable named and default arrays", () => {
    expect(punjabiA1ClosurePacketSamples).toBe(namedClosurePacketSamples);
    expect(Array.isArray(punjabiA1ClosurePacketSamples)).toBe(true);
  });

  it("is compact and covers the required closure-packet domains", () => {
    expect(punjabiA1ClosurePacketSamples.length).toBeGreaterThanOrEqual(12);
    expect(punjabiA1ClosurePacketSamples.length).toBeLessThanOrEqual(14);

    const requiredDomains: PunjabiA1ClosurePacketDomain[] = [
      "greetings",
      "identity",
      "family",
      "numbers",
    ...
    const domains = new Set(punjabiA1ClosurePacketSamples.map((item) => item.domain));
  ...
  it("includes pre-A11-closure, pre-merge, CI-readiness, pre-integration, qa, and readiness styles", () => {
    const requiredStyles: PunjabiA1ClosurePacketStyle[] = [
      "pre_a11_closure",
      "pre_merge",
      "ci_readiness",
      "pre_integration",
    ...
    const styles = new Set(punjabiA1ClosurePacketSamples.map((item) => item.style));
  ...
  it("uses Gurmukhi primary with bilingual support and review links", () => {
    for (const item of punjabiA1ClosurePacketSamples) {
      expect(item.id).toMatch(/^pa_a1_closure_packet_/);
      ...
      expect(item.closure_packet_check_vi.trim().length).toBeGreaterThan(0);
      expect(item.closure_packet_check_en.trim().length).toBeGreaterThan(0);
  ...
  it("includes traps, romanization bridge, and Canada-practical closure-packet items", () => {
    const traps = punjabiA1ClosurePacketSamples.flatMap((item) => (item.trap ? [item.trap] : []));
    const trapAudiences = new Set(traps.map((trap) => trap.audience));
    const canadaItems = punjabiA1ClosurePacketSamples.filter((item) => item.canada_practical);
    const romanizationBridge = punjabiA1ClosurePacketSamples.find((item) => item.domain === "romanization_bridge");
  ...
  it("verifies beginner readiness across required closure-packet situations", () => {
    const allText = JSON.stringify(punjabiA1ClosurePacketSamples);
    ...
    expect(allText).toMatch(/closure-packet|pre_a11_closure|pre_merge|ci_readiness|pre-integration|Sẵn sàng|Ready/i);
  });

  it("mentions Shahmukhi only as awareness and avoids disallowed claims and systems", () => {
    const allText = `${closurePacketScriptAwareness} ${JSON.stringify(punjabiA1ClosurePacketSamples)}`;

    expect(closurePacketScriptAwareness).toContain("Shahmukhi");
    expect(closurePacketScriptAwareness).toContain("awareness only");
