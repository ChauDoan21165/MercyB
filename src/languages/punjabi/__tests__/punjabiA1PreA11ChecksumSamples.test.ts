...
import punjabiA1PreA11ChecksumSamples, {
  preA11ChecksumScriptAwareness,
  punjabiA1PreA11ChecksumSamples as namedPreA11ChecksumSamples,
  type PunjabiA1PreA11ChecksumDomain,
  type PunjabiA1PreA11ChecksumStyle,
} from "@/languages/punjabi/a1PreA11ChecksumSamples";

const GURMUKHI_SCRIPT = /[\u0A00-\u0A7F]/;
const LATIN = /[a-zA-Z]/;

describe("Punjabi A1 pre-A11-checksum samples", () => {
  it("exports app-consumable named and default arrays", () => {
    expect(punjabiA1PreA11ChecksumSamples).toBe(namedPreA11ChecksumSamples);
    expect(Array.isArray(punjabiA1PreA11ChecksumSamples)).toBe(true);
  });

  it("is compact and covers the required pre-A11-checksum domains", () => {
    expect(punjabiA1PreA11ChecksumSamples.length).toBeGreaterThanOrEqual(12);
    expect(punjabiA1PreA11ChecksumSamples.length).toBeLessThanOrEqual(14);

    const requiredDomains: PunjabiA1PreA11ChecksumDomain[] = [
      "greetings",
      "identity",
      "family",
      "numbers",
    ...
    const domains = new Set(punjabiA1PreA11ChecksumSamples.map((item) => item.domain));
  ...
  it("includes pre-A11-checksum, runner-readiness, pipeline-readiness, pre-integration, qa, and readiness styles", () => {
    const requiredStyles: PunjabiA1PreA11ChecksumStyle[] = [
      "pre_a11_checksum",
      "runner_readiness",
      "pipeline_readiness",
      "pre_integration",
    ...
    const styles = new Set(punjabiA1PreA11ChecksumSamples.map((item) => item.style));
  ...
  it("uses Gurmukhi primary with bilingual support and review links", () => {
    for (const item of punjabiA1PreA11ChecksumSamples) {
      expect(item.id).toMatch(/^pa_a1_pre_a11_checksum_/);
      ...
      expect(item.pre_a11_checksum_check_vi.trim().length).toBeGreaterThan(0);
      expect(item.pre_a11_checksum_check_en.trim().length).toBeGreaterThan(0);
  ...
  it("includes traps, romanization bridge, and Canada-practical pre-A11-checksum items", () => {
    const traps = punjabiA1PreA11ChecksumSamples.flatMap((item) => (item.trap ? [item.trap] : []));
    const trapAudiences = new Set(traps.map((trap) => trap.audience));
    const canadaItems = punjabiA1PreA11ChecksumSamples.filter((item) => item.canada_practical);
    const romanizationBridge = punjabiA1PreA11ChecksumSamples.find((item) => item.domain === "romanization_bridge");
  ...
  it("verifies beginner readiness across required pre-A11-checksum situations", () => {
    const allText = JSON.stringify(punjabiA1PreA11ChecksumSamples);
    ...
    expect(allText).toMatch(/pre-A11-checksum|pre_a11_checksum|runner_readiness|pipeline_readiness|pre-integration|Sẵn sàng|Ready/i);
  });

  it("mentions Shahmukhi only as awareness and avoids disallowed claims and systems", () => {
    const allText = `${preA11ChecksumScriptAwareness} ${JSON.stringify(punjabiA1PreA11ChecksumSamples)}`;

    expect(preA11ChecksumScriptAwareness).toContain("Shahmukhi");
    expect(preA11ChecksumScriptAwareness).toContain("awareness only");
