// src/languages/punjabi/__tests__/punjabiA2ShipCandidateSamples.test.ts
//
// Structural guards for Punjabi A2 ship-candidate samples. This is Wave 39
// only, not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2ShipCandidateSamples } from "@/languages/punjabi/a2ShipCandidateSamples";
...
const STYLES = ["ship_candidate", "go_no_go", "release_candidate", "pre_integration"] as const;

describe("Punjabi A2 ship-candidate samples - batch shape", () => {
  it("ships compact samples for all required practical-readiness scenarios", () => {
    expect(a2ShipCandidateSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2ShipCandidateSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2ShipCandidateSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2ShipCandidateSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2ShipCandidateSamples) {
...
describe("Punjabi A2 ship-candidate samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2ShipCandidateSamples) {
      ...
      expect(nonEmpty(item.ship_goal_vi)).toBe(true);
      expect(nonEmpty(item.ship_goal_en)).toBe(true);
      expect(nonEmpty(item.readiness_signal_vi)).toBe(true);
      expect(nonEmpty(item.readiness_signal_en)).toBe(true);
      expect(nonEmpty(item.hold_signal_vi)).toBe(true);
      expect(nonEmpty(item.hold_signal_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2ShipCandidateSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers ship-candidate, go/no-go, release-candidate, and pre-integration styles", () => {
    const styles = new Set(a2ShipCandidateSamples.map((item) => item.style));
