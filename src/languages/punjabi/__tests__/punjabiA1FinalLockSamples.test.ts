...
import punjabiA1FinalLockSamples, {
  finalLockScriptAwareness,
  punjabiA1FinalLockSamples as namedFinalLockSamples,
  type PunjabiA1FinalLockDomain,
  type PunjabiA1FinalLockStyle,
} from "@/languages/punjabi/a1FinalLockSamples";

const GURMUKHI_SCRIPT = /[\u0A00-\u0A7F]/;
const LATIN = /[a-zA-Z]/;

describe("Punjabi A1 final-lock samples", () => {
  it("exports app-consumable named and default arrays", () => {
    expect(punjabiA1FinalLockSamples).toBe(namedFinalLockSamples);
    expect(Array.isArray(punjabiA1FinalLockSamples)).toBe(true);
  });

  it("is compact and covers the required final-lock domains", () => {
    expect(punjabiA1FinalLockSamples.length).toBeGreaterThanOrEqual(12);
    expect(punjabiA1FinalLockSamples.length).toBeLessThanOrEqual(14);

    const requiredDomains: PunjabiA1FinalLockDomain[] = [
      "greetings",
      "identity",
      "family",
      "numbers",
    ...
    const domains = new Set(punjabiA1FinalLockSamples.map((item) => item.domain));
  ...
  it("includes final-lock, owner-acceptance, final-acceptance, pre-integration, qa, and readiness styles", () => {
    const requiredStyles: PunjabiA1FinalLockStyle[] = [
      "final_lock",
      "owner_acceptance",
      "final_acceptance",
      "pre_integration",
    ...
    const styles = new Set(punjabiA1FinalLockSamples.map((item) => item.style));
  ...
  it("uses Gurmukhi primary with bilingual support and review links", () => {
    for (const item of punjabiA1FinalLockSamples) {
      expect(item.id).toMatch(/^pa_a1_final_lock_/);
      ...
      expect(item.final_lock_check_vi.trim().length).toBeGreaterThan(0);
      expect(item.final_lock_check_en.trim().length).toBeGreaterThan(0);
  ...
  it("includes traps, romanization bridge, and Canada-practical final-lock items", () => {
    const traps = punjabiA1FinalLockSamples.flatMap((item) => (item.trap ? [item.trap] : []));
    const trapAudiences = new Set(traps.map((trap) => trap.audience));
    const canadaItems = punjabiA1FinalLockSamples.filter((item) => item.canada_practical);
    const romanizationBridge = punjabiA1FinalLockSamples.find((item) => item.domain === "romanization_bridge");
  ...
  it("verifies beginner readiness across required final-lock situations", () => {
    const allText = JSON.stringify(punjabiA1FinalLockSamples);
    ...
    expect(allText).toMatch(/final-lock|final_lock|owner-acceptance|pre-integration|Sẵn sàng|Ready/i);
  });

  it("mentions Shahmukhi only as awareness and avoids disallowed claims and systems", () => {
    const allText = `${finalLockScriptAwareness} ${JSON.stringify(punjabiA1FinalLockSamples)}`;

    expect(finalLockScriptAwareness).toContain("Shahmukhi");
    expect(finalLockScriptAwareness).toContain("awareness only");
