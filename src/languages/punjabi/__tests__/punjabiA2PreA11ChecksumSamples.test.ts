// src/languages/punjabi/__tests__/punjabiA2PreA11ChecksumSamples.test.ts
//
// Structural guards for Punjabi A2 pre-A11 checksum samples. This is Wave 48
// only, not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2PreA11ChecksumSamples } from "@/languages/punjabi/a2PreA11ChecksumSamples";
...
const STYLES = ["pre_a11_checksum", "runner_readiness", "pipeline_readiness", "pre_integration"] as const;

describe("Punjabi A2 pre-A11 checksum samples - batch shape", () => {
  it("ships compact checksum samples for all required scenarios", () => {
    expect(a2PreA11ChecksumSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2PreA11ChecksumSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2PreA11ChecksumSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2PreA11ChecksumSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2PreA11ChecksumSamples) {
...
describe("Punjabi A2 pre-A11 checksum samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2PreA11ChecksumSamples) {
      ...
      expect(nonEmpty(item.checksum_goal_vi)).toBe(true);
      expect(nonEmpty(item.checksum_goal_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2PreA11ChecksumSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers pre-A11-checksum, runner-readiness, pipeline-readiness, and pre-integration styles", () => {
    const styles = new Set(a2PreA11ChecksumSamples.map((item) => item.style));
