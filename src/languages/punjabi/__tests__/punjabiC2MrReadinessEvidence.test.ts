// Punjabi C2 MR-readiness evidence guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_MR_READINESS_EVIDENCE_DISCLAIMER,
  c2MrReadinessEvidence,
  c2MrReadinessEvidenceByContext,
  c2MrReadinessEvidenceByFocus,
  type PunjabiC2MrReadinessContext,
  type PunjabiC2MrReadinessFocus,
  type PunjabiC2MrReadinessEvidenceItem,
} from "@/languages/punjabi/c2MrReadinessEvidence";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2MrReadinessFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2MrReadinessContext[] = ["public", "professional", "community"];

describe("Punjabi C2 MR-readiness evidence - coverage", () => {
  it("ships a compact app-consumable MR-readiness evidence set", () => {
    expect(c2MrReadinessEvidence.length).toBeGreaterThanOrEqual(8);
    expect(c2MrReadinessEvidence.length).toBeLessThanOrEqual(12);
  });

  it("covers every required MR-readiness focus", () => {
    const seen = new Set(c2MrReadinessEvidence.map((item) => item.focus));
  ...
  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2MrReadinessEvidence.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
      expect(c2MrReadinessEvidenceByContext(context).every((item) => item.context === context)).toBe(true);
    }
  });

  it("has unique ids and focus filter", () => {
    const ids = c2MrReadinessEvidence.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2MrReadinessEvidenceByFocus(focus);
...
describe("Punjabi C2 MR-readiness evidence - bilingual integrity", () => {
  it("each item has VI+EN scenario and MR-readiness goal", () => {
    for (const item of c2MrReadinessEvidence) {
      ...
      expect(item.evidence_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.evidence_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
    }
  });
  ...
  it("each sample and phrase has Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2MrReadinessEvidence) {
      ...
      expect(item.mr_readiness_evidence_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.mr_readiness_evidence_phrases) {
  ...
  it("includes MR-readiness, final-freeze, final-lock, and pre-integration styles", () => {
    const styles = new Set(c2MrReadinessEvidence.map((item) => item.style));
  ...
  it("each item carries MR-readiness checks", () => {
    for (const item of c2MrReadinessEvidence) {
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2MrReadinessEvidence.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2MrReadinessEvidence.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 MR-readiness evidence - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_MR_READINESS_EVIDENCE_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_MR_READINESS_EVIDENCE_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_MR_READINESS_EVIDENCE_DISCLAIMER.vi} ${C2_MR_READINESS_EVIDENCE_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2MrReadinessEvidence).toLowerCase();
...
const _typecheck: PunjabiC2MrReadinessEvidenceItem[] = c2MrReadinessEvidence;
void _typecheck;
