// Punjabi C2 final-validation set guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_FINAL_VALIDATION_SET_DISCLAIMER,
  c2FinalValidationSet,
  c2FinalValidationSetByContext,
  c2FinalValidationSetByFocus,
  type PunjabiC2FinalValidationContext,
  type PunjabiC2FinalValidationFocus,
  type PunjabiC2FinalValidationSample,
} from "@/languages/punjabi/c2FinalValidationSet";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2FinalValidationFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2FinalValidationContext[] = ["public", "professional", "community"];

describe("Punjabi C2 final-validation set - coverage", () => {
  it("ships a compact app-consumable final-validation set", () => {
    expect(c2FinalValidationSet.length).toBeGreaterThanOrEqual(8);
    expect(c2FinalValidationSet.length).toBeLessThanOrEqual(12);
  });

  it("covers every required final-validation focus", () => {
    const seen = new Set(c2FinalValidationSet.map((item) => item.focus));
  ...
  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2FinalValidationSet.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
      expect(c2FinalValidationSetByContext(context).every((item) => item.context === context)).toBe(true);
    }
  });

  it("has unique ids and focus filter", () => {
    const ids = c2FinalValidationSet.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2FinalValidationSetByFocus(focus);
...
describe("Punjabi C2 final-validation set - bilingual integrity", () => {
  it("each item has VI+EN scenario and validation goal", () => {
    for (const item of c2FinalValidationSet) {
      ...
      expect(item.validation_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.validation_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
    }
  });
  ...
  it("each sample and phrase has Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2FinalValidationSet) {
      ...
      expect(item.validation_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.validation_phrases) {
  ...
  it("includes final-validation, cross-check, pre-integration, and readiness styles", () => {
    const styles = new Set(c2FinalValidationSet.map((item) => item.style));
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2FinalValidationSet.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2FinalValidationSet.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 final-validation set - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_FINAL_VALIDATION_SET_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_FINAL_VALIDATION_SET_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_FINAL_VALIDATION_SET_DISCLAIMER.vi} ${C2_FINAL_VALIDATION_SET_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2FinalValidationSet).toLowerCase();
...
const _typecheck: PunjabiC2FinalValidationSample[] = c2FinalValidationSet;
void _typecheck;
