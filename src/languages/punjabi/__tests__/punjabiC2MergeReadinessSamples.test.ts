// Punjabi C2 merge-readiness sample guards. These validate app-consumable
// structure and scope, not certification, official placement, or native review.

import { describe, expect, it } from "vitest";

import {
  C2_MERGE_READINESS_SAMPLES_DISCLAIMER,
  c2MergeReadinessSamples,
  c2MergeReadinessSamplesByFocus,
  type PunjabiC2MergeReadinessFocus,
  type PunjabiC2MergeReadinessSample,
} from "@/languages/punjabi/c2MergeReadinessSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2MergeReadinessFocus[] = [
...
describe("Punjabi C2 merge-readiness samples - coverage", () => {
  it("ships a compact app-consumable merge-readiness set", () => {
    expect(c2MergeReadinessSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2MergeReadinessSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required merge-readiness focus", () => {
    const seen = new Set(c2MergeReadinessSamples.map((item) => item.focus));
  ...
  it("has unique non-empty ids", () => {
    const ids = c2MergeReadinessSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it("c2MergeReadinessSamplesByFocus returns only matching items", () => {
    for (const focus of REQUIRED_FOCUSES) {
      const subset = c2MergeReadinessSamplesByFocus(focus);
...
describe("Punjabi C2 merge-readiness samples - bilingual integrity", () => {
  it("each item has VI+EN scenario and readiness goal", () => {
    for (const item of c2MergeReadinessSamples) {
  ...
  it("includes merge-readiness, final-regression, pre-integration, and readiness styles", () => {
    const styles = new Set(c2MergeReadinessSamples.map((item) => item.style));
  ...
  it("includes learner traps and Canada-practical examples where useful", () => {
    expect(c2MergeReadinessSamples.filter((item) => item.learner_trap).length).toBeGreaterThanOrEqual(6);
    const canadaItems = c2MergeReadinessSamples.filter((item) => item.canada_practical);
    expect(canadaItems.length).toBeGreaterThanOrEqual(2);
    expect(JSON.stringify(canadaItems).toLowerCase()).toContain("canada");
  });
});

describe("Punjabi C2 merge-readiness samples - scope framing", () => {
  it("exposes study-support, deferred-review, Shahmukhi-awareness disclaimer", () => {
    expect(C2_MERGE_READINESS_SAMPLES_DISCLAIMER.vi.length).toBeGreaterThan(0);
    expect(C2_MERGE_READINESS_SAMPLES_DISCLAIMER.en.length).toBeGreaterThan(0);
    const disclaimer =
      `${C2_MERGE_READINESS_SAMPLES_DISCLAIMER.vi} ${C2_MERGE_READINESS_SAMPLES_DISCLAIMER.en}`.toLowerCase();
  ...
  it("does not claim native review, certification, official placement, or legal authority", () => {
    const blob = JSON.stringify(c2MergeReadinessSamples).toLowerCase();
...
const _typecheck: PunjabiC2MergeReadinessSample[] = c2MergeReadinessSamples;
void _typecheck;
