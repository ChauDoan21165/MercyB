...
import {
  C2_CATALOG_SAMPLES_DISCLAIMER,
  c2CatalogSamples,
  c2CatalogSamplesByFocus,
  c2CatalogSamplesByStyle,
  type PunjabiC2CatalogContext,
  type PunjabiC2CatalogFocus,
  type PunjabiC2CatalogSample,
  type PunjabiC2CatalogStyle,
} from "@/languages/punjabi/c2CatalogSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2CatalogFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2CatalogContext[] = ["public", "professional", "community"];
const REQUIRED_STYLES: PunjabiC2CatalogStyle[] = [
  "pre_a11_catalog",
  "bundle_catalog",
  "receipt_catalog",
  "pre_integration",
...
describe("Punjabi C2 catalog samples - coverage", () => {
  it("ships a compact app-consumable catalog sample set", () => {
    expect(c2CatalogSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2CatalogSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required catalog focus", () => {
    const seen = new Set(c2CatalogSamples.map((item) => item.focus));
    for (const focus of REQUIRED_FOCUSES) {
      expect(seen.has(focus), `missing focus ${focus}`).toBe(true);
      expect(c2CatalogSamplesByFocus(focus).every((item) => item.focus === focus)).toBe(true);
    }
  });

  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2CatalogSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
    }
  });

  it("covers pre-A11 catalog, bundle catalog, receipt catalog, and pre-integration styles", () => {
    const seen = new Set(c2CatalogSamples.map((item) => item.style));
    for (const style of REQUIRED_STYLES) {
      expect(seen.has(style), `missing style ${style}`).toBe(true);
      expect(c2CatalogSamplesByStyle(style).every((item) => item.style === style)).toBe(true);
    }
  });

  it("has unique ids and stable filters", () => {
    const ids = c2CatalogSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
  });
});

describe("Punjabi C2 catalog samples - bilingual integrity", () => {
  it("each item has VI+EN scenario, catalog goal, and usable sample strings", () => {
    for (const item of c2CatalogSamples) {
      ...
      expect(item.catalog_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.catalog_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
  ...
  it("each catalog phrase carries Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2CatalogSamples) {
      expect(item.catalog_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.catalog_phrases) {
  ...
  it("each sample carries checks, learner traps, and Canada-practical examples where useful", () => {
    const canadaSamples = c2CatalogSamples.filter((item) => item.canada_practical);
    expect(canadaSamples.length).toBeGreaterThanOrEqual(3);

    for (const item of c2CatalogSamples) {
      expect(item.catalog_checks.length, `${item.id} checks`).toBeGreaterThanOrEqual(1);
      for (const check of item.catalog_checks) {
...
describe("Punjabi C2 catalog samples - scope framing", () => {
  it("states deferred native review and Shahmukhi awareness", () => {
    const blob = JSON.stringify({
      disclaimer: C2_CATALOG_SAMPLES_DISCLAIMER,
      samples: c2CatalogSamples,
    }).toLowerCase();
  ...
  it("keeps forbidden integration and scoring concepts out", () => {
    const blob = JSON.stringify(c2CatalogSamples);
    expect(blob).not.toMatch(/audio|pronunciation scoring|Azure|auth|billing|RLS|Supabase|CI config/i);
  });
});

const _typecheck: PunjabiC2CatalogSample[] = c2CatalogSamples;
void _typecheck;
