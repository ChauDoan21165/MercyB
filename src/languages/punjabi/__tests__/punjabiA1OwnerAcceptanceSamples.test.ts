...
import punjabiA1OwnerAcceptanceSamples, {
  ownerAcceptanceScriptAwareness,
  punjabiA1OwnerAcceptanceSamples as namedOwnerAcceptanceSamples,
  type PunjabiA1OwnerAcceptanceDomain,
  type PunjabiA1OwnerAcceptanceStyle,
} from "@/languages/punjabi/a1OwnerAcceptanceSamples";

const GURMUKHI_SCRIPT = /[\u0A00-\u0A7F]/;
const LATIN = /[a-zA-Z]/;

describe("Punjabi A1 owner-acceptance samples", () => {
  it("exports app-consumable named and default arrays", () => {
    expect(punjabiA1OwnerAcceptanceSamples).toBe(namedOwnerAcceptanceSamples);
    expect(Array.isArray(punjabiA1OwnerAcceptanceSamples)).toBe(true);
  });

  it("is compact and covers the required owner-acceptance domains", () => {
    expect(punjabiA1OwnerAcceptanceSamples.length).toBeGreaterThanOrEqual(12);
    expect(punjabiA1OwnerAcceptanceSamples.length).toBeLessThanOrEqual(14);

    const requiredDomains: PunjabiA1OwnerAcceptanceDomain[] = [
      "greetings",
      "identity",
      "family",
      "numbers",
    ...
    const domains = new Set(punjabiA1OwnerAcceptanceSamples.map((item) => item.domain));
  ...
  it("includes owner-acceptance, final-acceptance, ship-candidate, pre-integration, qa, and selector styles", () => {
    const requiredStyles: PunjabiA1OwnerAcceptanceStyle[] = [
      "owner_acceptance",
      "final_acceptance",
      "ship_candidate",
      "pre_integration",
    ...
    const styles = new Set(punjabiA1OwnerAcceptanceSamples.map((item) => item.style));
  ...
  it("uses Gurmukhi primary with bilingual support and review links", () => {
    for (const item of punjabiA1OwnerAcceptanceSamples) {
      expect(item.id).toMatch(/^pa_a1_owner_acceptance_/);
      ...
      expect(item.owner_acceptance_check_vi.trim().length).toBeGreaterThan(0);
      expect(item.owner_acceptance_check_en.trim().length).toBeGreaterThan(0);
  ...
  it("includes traps, romanization bridge, and Canada-practical owner-acceptance items", () => {
    const traps = punjabiA1OwnerAcceptanceSamples.flatMap((item) => (item.trap ? [item.trap] : []));
    const trapAudiences = new Set(traps.map((trap) => trap.audience));
    const canadaItems = punjabiA1OwnerAcceptanceSamples.filter((item) => item.canada_practical);
    const romanizationBridge = punjabiA1OwnerAcceptanceSamples.find((item) => item.domain === "romanization_bridge");
  ...
  it("verifies beginner readiness across the required owner-acceptance situations", () => {
    const allText = JSON.stringify(punjabiA1OwnerAcceptanceSamples);
    ...
    expect(allText).toMatch(/owner-acceptance|final_acceptance|Sẵn sàng|Ready/i);
  });

  it("mentions Shahmukhi only as awareness and avoids disallowed claims and systems", () => {
    const allText = `${ownerAcceptanceScriptAwareness} ${JSON.stringify(punjabiA1OwnerAcceptanceSamples)}`;

    expect(ownerAcceptanceScriptAwareness).toContain("Shahmukhi");
    expect(ownerAcceptanceScriptAwareness).toContain("awareness only");
