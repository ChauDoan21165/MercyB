...
import {
  C2_EVIDENCE_RECEIPT_SAMPLES_DISCLAIMER,
  c2EvidenceReceiptSamples,
  c2EvidenceReceiptSamplesByFocus,
  c2EvidenceReceiptSamplesByStyle,
  type PunjabiC2EvidenceReceiptContext,
  type PunjabiC2EvidenceReceiptFocus,
  type PunjabiC2EvidenceReceiptSample,
  type PunjabiC2EvidenceReceiptStyle,
} from "@/languages/punjabi/c2EvidenceReceiptSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2EvidenceReceiptFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2EvidenceReceiptContext[] = ["public", "professional", "community"];
const REQUIRED_STYLES: PunjabiC2EvidenceReceiptStyle[] = [
...
describe("Punjabi C2 evidence receipt samples - coverage", () => {
  it("ships a compact app-consumable evidence receipt sample set", () => {
    expect(c2EvidenceReceiptSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2EvidenceReceiptSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required evidence receipt focus", () => {
    const seen = new Set(c2EvidenceReceiptSamples.map((item) => item.focus));
    for (const focus of REQUIRED_FOCUSES) {
      expect(seen.has(focus), `missing focus ${focus}`).toBe(true);
      expect(c2EvidenceReceiptSamplesByFocus(focus).every((item) => item.focus === focus)).toBe(true);
    }
  });

  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2EvidenceReceiptSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
    }
  });

  it("covers pre-A11 evidence receipt, completion record receipt, inventory seal receipt, and pre-integration styles", () => {
    const seen = new Set(c2EvidenceReceiptSamples.map((item) => item.style));
    for (const style of REQUIRED_STYLES) {
      expect(seen.has(style), `missing style ${style}`).toBe(true);
      expect(c2EvidenceReceiptSamplesByStyle(style).every((item) => item.style === style)).toBe(true);
    }
  });

  it("has unique ids and stable filters", () => {
    const ids = c2EvidenceReceiptSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
  });
});

describe("Punjabi C2 evidence receipt samples - bilingual integrity", () => {
  it("each item has VI+EN scenario, evidence receipt goal, and usable sample strings", () => {
    for (const item of c2EvidenceReceiptSamples) {
      ...
      expect(item.evidence_receipt_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.evidence_receipt_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
  ...
  it("each evidence receipt phrase carries Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2EvidenceReceiptSamples) {
      expect(item.evidence_receipt_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.evidence_receipt_phrases) {
  ...
  it("each sample carries checks, learner traps, and Canada-practical examples where useful", () => {
    const canadaSamples = c2EvidenceReceiptSamples.filter((item) => item.canada_practical);
    expect(canadaSamples.length).toBeGreaterThanOrEqual(3);

    for (const item of c2EvidenceReceiptSamples) {
      expect(item.evidence_receipt_checks.length, `${item.id} checks`).toBeGreaterThanOrEqual(1);
      for (const check of item.evidence_receipt_checks) {
...
describe("Punjabi C2 evidence receipt samples - scope framing", () => {
  it("states deferred native review and Shahmukhi awareness", () => {
    const blob = JSON.stringify({
      disclaimer: C2_EVIDENCE_RECEIPT_SAMPLES_DISCLAIMER,
      samples: c2EvidenceReceiptSamples,
    }).toLowerCase();
  ...
  it("keeps forbidden integration and scoring concepts out", () => {
    const blob = JSON.stringify(c2EvidenceReceiptSamples);
    expect(blob).not.toMatch(/audio|pronunciation scoring|Azure|auth|billing|RLS|Supabase|CI config/i);
  });
});

const _typecheck: PunjabiC2EvidenceReceiptSample[] = c2EvidenceReceiptSamples;
void _typecheck;
