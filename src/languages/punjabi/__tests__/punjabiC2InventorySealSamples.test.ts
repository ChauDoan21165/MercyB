...
import {
  C2_INVENTORY_SEAL_SAMPLES_DISCLAIMER,
  c2InventorySealSamples,
  c2InventorySealSamplesByFocus,
  c2InventorySealSamplesByStyle,
  type PunjabiC2InventorySealContext,
  type PunjabiC2InventorySealFocus,
  type PunjabiC2InventorySealSample,
  type PunjabiC2InventorySealStyle,
} from "@/languages/punjabi/c2InventorySealSamples";

const GURMUKHI_RANGE = /[\u0A00-\u0A7F]/;
const hasGurmukhi = (s: string) => GURMUKHI_RANGE.test(s);

const REQUIRED_FOCUSES: PunjabiC2InventorySealFocus[] = [
...
const REQUIRED_CONTEXTS: PunjabiC2InventorySealContext[] = ["public", "professional", "community"];
const REQUIRED_STYLES: PunjabiC2InventorySealStyle[] = [
...
describe("Punjabi C2 inventory seal samples - coverage", () => {
  it("ships a compact app-consumable inventory seal sample set", () => {
    expect(c2InventorySealSamples.length).toBeGreaterThanOrEqual(8);
    expect(c2InventorySealSamples.length).toBeLessThanOrEqual(12);
  });

  it("covers every required inventory seal focus", () => {
    const seen = new Set(c2InventorySealSamples.map((item) => item.focus));
    for (const focus of REQUIRED_FOCUSES) {
      expect(seen.has(focus), `missing focus ${focus}`).toBe(true);
      expect(c2InventorySealSamplesByFocus(focus).every((item) => item.focus === focus)).toBe(true);
    }
  });

  it("covers public, professional, and community contexts", () => {
    const seen = new Set(c2InventorySealSamples.map((item) => item.context));
    for (const context of REQUIRED_CONTEXTS) {
      expect(seen.has(context), `missing context ${context}`).toBe(true);
    }
  });

  it("covers pre-A11 inventory seal, catalog seal, bundle seal, and pre-integration styles", () => {
    const seen = new Set(c2InventorySealSamples.map((item) => item.style));
    for (const style of REQUIRED_STYLES) {
      expect(seen.has(style), `missing style ${style}`).toBe(true);
      expect(c2InventorySealSamplesByStyle(style).every((item) => item.style === style)).toBe(true);
    }
  });

  it("has unique ids and stable filters", () => {
    const ids = c2InventorySealSamples.map((item) => item.id);
    expect(ids.every((id) => id.length > 0)).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
  });
});

describe("Punjabi C2 inventory seal samples - bilingual integrity", () => {
  it("each item has VI+EN scenario, inventory seal goal, and usable sample strings", () => {
    for (const item of c2InventorySealSamples) {
      ...
      expect(item.inventory_seal_goal_vi.length, `${item.id} goal_vi`).toBeGreaterThan(0);
      expect(item.inventory_seal_goal_en.length, `${item.id} goal_en`).toBeGreaterThan(0);
  ...
  it("each inventory seal phrase carries Gurmukhi, romanization, Vietnamese, and English", () => {
    for (const item of c2InventorySealSamples) {
      expect(item.inventory_seal_phrases.length, `${item.id} phrases`).toBeGreaterThanOrEqual(2);
      for (const phrase of item.inventory_seal_phrases) {
  ...
  it("each sample carries checks, learner traps, and Canada-practical examples where useful", () => {
    const canadaSamples = c2InventorySealSamples.filter((item) => item.canada_practical);
    expect(canadaSamples.length).toBeGreaterThanOrEqual(3);

    for (const item of c2InventorySealSamples) {
      expect(item.inventory_seal_checks.length, `${item.id} checks`).toBeGreaterThanOrEqual(1);
      for (const check of item.inventory_seal_checks) {
...
describe("Punjabi C2 inventory seal samples - scope framing", () => {
  it("states deferred native review and Shahmukhi awareness", () => {
    const blob = JSON.stringify({
      disclaimer: C2_INVENTORY_SEAL_SAMPLES_DISCLAIMER,
      samples: c2InventorySealSamples,
    }).toLowerCase();
  ...
  it("keeps forbidden integration and scoring concepts out", () => {
    const blob = JSON.stringify(c2InventorySealSamples);
    expect(blob).not.toMatch(/audio|pronunciation scoring|Azure|auth|billing|RLS|Supabase|CI config/i);
  });
});

const _typecheck: PunjabiC2InventorySealSample[] = c2InventorySealSamples;
void _typecheck;
