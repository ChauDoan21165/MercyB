...
import punjabiA1ReleaseCandidateSamples, {
  releaseCandidateScriptAwareness,
  punjabiA1ReleaseCandidateSamples as namedReleaseCandidateSamples,
  type PunjabiA1ReleaseCandidateDomain,
  type PunjabiA1ReleaseCandidateStyle,
} from "@/languages/punjabi/a1ReleaseCandidateSamples";

const GURMUKHI_SCRIPT = /[\u0A00-\u0A7F]/;
const LATIN = /[a-zA-Z]/;

describe("Punjabi A1 release-candidate samples", () => {
  it("exports app-consumable named and default arrays", () => {
    expect(punjabiA1ReleaseCandidateSamples).toBe(namedReleaseCandidateSamples);
    expect(Array.isArray(punjabiA1ReleaseCandidateSamples)).toBe(true);
  });

  it("is compact and covers the required release-candidate domains", () => {
    expect(punjabiA1ReleaseCandidateSamples.length).toBeGreaterThanOrEqual(12);
    expect(punjabiA1ReleaseCandidateSamples.length).toBeLessThanOrEqual(14);

    const requiredDomains: PunjabiA1ReleaseCandidateDomain[] = [
      "greetings",
      "identity",
      "family",
      "numbers",
    ...
    const domains = new Set(punjabiA1ReleaseCandidateSamples.map((item) => item.domain));
  ...
  it("includes release-candidate, closure-validation, pre-integration, readiness-check, qa, and selector styles", () => {
    const requiredStyles: PunjabiA1ReleaseCandidateStyle[] = [
      "release_candidate",
      "closure_validation",
      "pre_integration",
      "readiness_check",
    ...
    const styles = new Set(punjabiA1ReleaseCandidateSamples.map((item) => item.style));
  ...
  it("uses Gurmukhi primary with bilingual support and review links", () => {
    for (const item of punjabiA1ReleaseCandidateSamples) {
      expect(item.id).toMatch(/^pa_a1_release_/);
      ...
      expect(item.release_check_vi.trim().length).toBeGreaterThan(0);
      expect(item.release_check_en.trim().length).toBeGreaterThan(0);
  ...
  it("includes traps, romanization bridge, and Canada-practical release items", () => {
    const traps = punjabiA1ReleaseCandidateSamples.flatMap((item) => (item.trap ? [item.trap] : []));
    const trapAudiences = new Set(traps.map((trap) => trap.audience));
    const canadaItems = punjabiA1ReleaseCandidateSamples.filter((item) => item.canada_practical);
    const romanizationBridge = punjabiA1ReleaseCandidateSamples.find((item) => item.domain === "romanization_bridge");
  ...
  it("verifies beginner readiness across the required release-candidate situations", () => {
    const allText = JSON.stringify(punjabiA1ReleaseCandidateSamples);
    ...
    expect(allText).toMatch(/release|candidate|Sẵn sàng|Ready/i);
  });

  it("mentions Shahmukhi only as awareness and avoids disallowed claims and systems", () => {
    const allText = `${releaseCandidateScriptAwareness} ${JSON.stringify(punjabiA1ReleaseCandidateSamples)}`;

    expect(releaseCandidateScriptAwareness).toContain("Shahmukhi");
    expect(releaseCandidateScriptAwareness).toContain("awareness only");
