...
import {
  c1FinalLockSamples,
  c1FinalLockSamplesScriptAwareness,
} from "../c1FinalLockSamples";
...
describe("Punjabi C1 final-lock samples - app data contract", () => {
  it("contains a compact final-lock pack with stable ids", () => {
    expect(c1FinalLockSamples.length).toBeGreaterThanOrEqual(8);
    expect(c1FinalLockSamples.length).toBeLessThanOrEqual(12);

    const ids = c1FinalLockSamples.map((sample) => sample.id);
    expect(ids.every((id) => id.startsWith("pa_c1_final_lock_"))).toBe(true);
    expect(new Set(ids).size).toBe(ids.length);
    expect(c1FinalLockSamples.every((sample) => sample.level === "C1")).toBe(true);
  });

  it("covers all Wave 42 final-lock areas", () => {
    const present = new Set(c1FinalLockSamples.map((sample) => sample.area));
    for (const area of REQUIRED_AREAS) {
      expect(present.has(area)).toBe(true);
    }
  });

  it("includes final-lock, owner-acceptance, final-acceptance, and pre-integration modes", () => {
    const modes = new Set(c1FinalLockSamples.map((sample) => sample.mode));
...
describe("Punjabi C1 final-lock samples - bilingual Gurmukhi contract", () => {
  it("uses Gurmukhi samples with romanization, Vietnamese, and English support", () => {
    for (const sample of c1FinalLockSamples) {
  ...
  it("includes final-lock checks, owner acceptance checks, pre-integration notes, Canada examples, and learner traps", () => {
    for (const sample of c1FinalLockSamples) {
      expect(sample.final_lock_checks_vi.length).toBeGreaterThanOrEqual(3);
      expect(sample.final_lock_checks_en.length).toBeGreaterThanOrEqual(3);
...
describe("Punjabi C1 final-lock samples - script and scope", () => {
  it("mentions Shahmukhi only as awareness, not learner sample content", () => {
    expect(c1FinalLockSamplesScriptAwareness.vi).toContain("Gurmukhi");
    expect(c1FinalLockSamplesScriptAwareness.en).toContain("Gurmukhi");
    expect(c1FinalLockSamplesScriptAwareness.vi).toContain("Shahmukhi");
    expect(c1FinalLockSamplesScriptAwareness.en).toContain("Shahmukhi");

    const learnerPunjabi = c1FinalLockSamples.flatMap((sample) => [
      sample.title_pa,
      sample.sample.pa,
  ...
  it("keeps Wave 42 scoped away from A11 integration and native-review claims", () => {
    const allText = JSON.stringify(c1FinalLockSamples);
