// src/languages/punjabi/__tests__/punjabiA2PipelineReadinessSamples.test.ts
//
// Structural guards for Punjabi A2 pipeline-readiness samples. This is Wave 46
// only, not A11 integration. These verify app-consumable learner content only;
// native review is deferred.

import { describe, expect, it } from "vitest";

import { a2PipelineReadinessSamples } from "@/languages/punjabi/a2PipelineReadinessSamples";
...
const STYLES = ["pipeline_readiness", "ci_readiness", "mr_readiness", "pre_integration"] as const;

describe("Punjabi A2 pipeline-readiness samples - batch shape", () => {
  it("ships compact test-stable samples for all required scenarios", () => {
    expect(a2PipelineReadinessSamples.length).toBeGreaterThanOrEqual(12);
    expect(a2PipelineReadinessSamples.length).toBeLessThanOrEqual(14);

    const present = new Set(a2PipelineReadinessSamples.map((item) => item.scenario));
  ...
  it("has unique ids and valid scenarios/styles", () => {
    const ids = a2PipelineReadinessSamples.map((item) => item.id);
    expect(new Set(ids).size).toBe(ids.length);

    for (const item of a2PipelineReadinessSamples) {
...
describe("Punjabi A2 pipeline-readiness samples - learner contract", () => {
  it("uses Gurmukhi lines with romanization and bilingual meaning", () => {
    for (const item of a2PipelineReadinessSamples) {
      ...
      expect(nonEmpty(item.pipeline_goal_vi)).toBe(true);
      expect(nonEmpty(item.pipeline_goal_en)).toBe(true);
  ...
  it("keeps Shahmukhi as awareness only and includes Canada-practical coverage", () => {
    ...
    const canadaItems = a2PipelineReadinessSamples.filter((item) => item.canada_practical_vi || item.canada_practical_en);
  ...
  it("covers pipeline-readiness, CI-readiness, MR-readiness, and pre-integration styles", () => {
    const styles = new Set(a2PipelineReadinessSamples.map((item) => item.style));
