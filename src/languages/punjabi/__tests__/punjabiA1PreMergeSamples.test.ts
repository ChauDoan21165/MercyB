...
import punjabiA1PreMergeSamples, {
  preMergeScriptAwareness,
  punjabiA1PreMergeSamples as namedPreMergeSamples,
  type PunjabiA1PreMergeDomain,
  type PunjabiA1PreMergeStyle,
} from "@/languages/punjabi/a1PreMergeSamples";

const GURMUKHI_SCRIPT = /[\u0A00-\u0A7F]/;
const LATIN = /[a-zA-Z]/;

describe("Punjabi A1 pre-merge samples", () => {
  it("exports app-consumable named and default arrays", () => {
    expect(punjabiA1PreMergeSamples).toBe(namedPreMergeSamples);
    expect(Array.isArray(punjabiA1PreMergeSamples)).toBe(true);
  });

  it("is compact and covers the required pre-merge domains", () => {
    expect(punjabiA1PreMergeSamples.length).toBeGreaterThanOrEqual(12);
    expect(punjabiA1PreMergeSamples.length).toBeLessThanOrEqual(14);

    const requiredDomains: PunjabiA1PreMergeDomain[] = [
      "greetings",
      "identity",
      "family",
      "numbers",
    ...
    const domains = new Set(punjabiA1PreMergeSamples.map((item) => item.domain));
  ...
  it("includes pre-merge, pre-A11-checksum, runner-readiness, pre-integration, qa, and readiness styles", () => {
    const requiredStyles: PunjabiA1PreMergeStyle[] = [
      "pre_merge",
      "pre_a11_checksum",
      "runner_readiness",
      "pre_integration",
    ...
    const styles = new Set(punjabiA1PreMergeSamples.map((item) => item.style));
  ...
  it("uses Gurmukhi primary with bilingual support and review links", () => {
    for (const item of punjabiA1PreMergeSamples) {
      expect(item.id).toMatch(/^pa_a1_pre_merge_/);
      ...
      expect(item.pre_merge_check_vi.trim().length).toBeGreaterThan(0);
      expect(item.pre_merge_check_en.trim().length).toBeGreaterThan(0);
  ...
  it("includes traps, romanization bridge, and Canada-practical pre-merge items", () => {
    const traps = punjabiA1PreMergeSamples.flatMap((item) => (item.trap ? [item.trap] : []));
    const trapAudiences = new Set(traps.map((trap) => trap.audience));
    const canadaItems = punjabiA1PreMergeSamples.filter((item) => item.canada_practical);
    const romanizationBridge = punjabiA1PreMergeSamples.find((item) => item.domain === "romanization_bridge");
  ...
  it("verifies beginner readiness across required pre-merge situations", () => {
    const allText = JSON.stringify(punjabiA1PreMergeSamples);
    ...
    expect(allText).toMatch(/pre-merge|pre_merge|pre_a11_checksum|runner_readiness|pre-integration|Sẵn sàng|Ready/i);
  });

  it("mentions Shahmukhi only as awareness and avoids disallowed claims and systems", () => {
    const allText = `${preMergeScriptAwareness} ${JSON.stringify(punjabiA1PreMergeSamples)}`;

    expect(preMergeScriptAwareness).toContain("Shahmukhi");
    expect(preMergeScriptAwareness).toContain("awareness only");
