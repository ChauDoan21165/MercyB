// src/languages/punjabi/b1AuditTrailSamples.ts
//
// Punjabi B1 audit trail samples for Vietnamese-speaking and English-speaking
...
export type PunjabiB1AuditTrailFocus =
  | "explain_situation"
  | "retell_event"
  | "clarify_next_steps"
  | "service_recovery"
...
export type PunjabiAuditTrailLine = {
  pa: string;
  romanization: string;
  en: string;
  vi: string;
...
export type PunjabiAuditTrailTrap = {
  trap_en: string;
  trap_vi: string;
  better: PunjabiAuditTrailLine;
};

export type PunjabiAuditTrailQA = {
  prompt_en: string;
  prompt_vi: string;
  modelAnswer: PunjabiAuditTrailLine;
  followUpQuestion: PunjabiAuditTrailLine;
};

export type PunjabiB1AuditTrailCard = {
  id: string;
  level: "B1";
  focus: PunjabiB1AuditTrailFocus;
  title_en: string;
  title_vi: string;
  auditTrailPurpose_en: string;
  auditTrailPurpose_vi: string;
  canadaContext: string;
  checkpointCriteria_en: string[];
  checkpointCriteria_vi: string[];
  usefulLanguage: PunjabiAuditTrailLine[];
  qa: PunjabiAuditTrailQA;
  commonTraps: PunjabiAuditTrailTrap[];
  auditTrailNote_en: string;
  auditTrailNote_vi: string;
};

export type PunjabiB1AuditTrailScope = {
  level: "B1";
  note_en: string;
  note_vi: string;
  awareness_en: string;
...
export const punjabiB1AuditTrailScope: PunjabiB1AuditTrailScope = {
  level: "B1",
  note_en:
    "Native review is deferred. This audit-trail pack keeps stable independent communication samples for later A11 use after traceability, evidence receipt, and pre-integration checks.",
  note_vi:
    "Việc native review được hoãn lại. Bộ audit trail này giữ các mẫu giao tiếp độc lập ổn định để dùng cho A11 sau này, sau truy vết, biên nhận bằng chứng và kiểm tra trước tích hợp.",
  awareness_en:
...
export const punjabiB1AuditTrailSamples: PunjabiB1AuditTrailCard[] = [
  {
    id: "pa-b1-audit-trail-01-explain-situation",
    level: "B1",
    focus: "explain_situation",
    title_en: "Explain a situation clearly",
    title_vi: "Giải thích tình huống rõ ràng",
    auditTrailPurpose_en: "Keep a stable sample for explaining an access problem, delay, or disruption without overexplaining.",
    auditTrailPurpose_vi: "Giữ mẫu ổn định để giải thích vấn đề truy cập, sự chậm trễ hoặc gián đoạn mà không giải thích dài dòng.",
    ...
    auditTrailNote_en: "Recorded as an audit-trail-ready situation-explanation pattern for later review and reuse.",
    auditTrailNote_vi: "Được lưu như mẫu giải thích tình huống đơn giản để xem lại và dùng lại sau.",
  },
  {
    id: "pa-b1-audit-trail-02-retell-event",
    level: "B1",
    focus: "retell_event",
    title_en: "Retell an event in order",
    title_vi: "Kể lại sự việc theo thứ tự",
    auditTrailPurpose_en: "Keep a short, stable sequence sample for late arrivals, transit delays, and schedule changes.",
    auditTrailPurpose_vi: "Giữ mẫu trình tự ngắn, ổn định cho việc đến muộn, trễ xe và thay đổi lịch.",
    ...
    auditTrailNote_en: "Recorded as an audit-trail-ready sequence model for messages, appointments, and quick follow-ups.",
    auditTrailNote_vi: "Được lưu như mẫu trình tự cho tin nhắn, cuộc hẹn và theo dõi nhanh.",
  },
  {
    id: "pa-b1-audit-trail-03-clarify-next-steps",
    level: "B1",
    focus: "clarify_next_steps",
    title_en: "Clarify next steps",
    title_vi: "Làm rõ bước tiếp theo",
    auditTrailPurpose_en: "Keep a polite clarification sample for forms, deadlines, email follow-up, and written confirmation.",
    auditTrailPurpose_vi: "Giữ mẫu làm rõ lịch sự cho biểu mẫu, hạn chót, theo dõi email và xác nhận bằng văn bản.",
    ...
    auditTrailNote_en: "Recorded for safe clarification and written follow-up practice.",
    auditTrailNote_vi: "Được lưu để luyện làm rõ an toàn và theo dõi bằng văn bản.",
  },
  {
    id: "pa-b1-audit-trail-04-service-recovery",
    level: "B1",
    focus: "service_recovery",
    title_en: "Recover after a service mistake",
    title_vi: "Khắc phục sau lỗi dịch vụ",
    auditTrailPurpose_en: "Keep a repair sample for wrong charges, missing bookings, or service errors that need correction.",
    auditTrailPurpose_vi: "Giữ mẫu sửa chữa cho phí sai, đặt chỗ bị thiếu hoặc lỗi dịch vụ cần được điều chỉnh.",
    ...
    auditTrailNote_en: "Recorded as a service-recovery script for correction, timing, and reference tracking.",
    auditTrailNote_vi: "Được lưu như kịch bản khắc phục dịch vụ cho sửa lỗi, thời gian xử lý và theo dõi tham chiếu.",
  },
  {
    id: "pa-b1-audit-trail-05-issue-resolution",
    level: "B1",
    focus: "issue_resolution",
    title_en: "Resolve a practical issue",
    title_vi: "Giải quyết một vấn đề thực tế",
    auditTrailPurpose_en: "Keep a repair-and-resolution sample for housing, delivery, and workplace issues that need a concrete fix.",
    auditTrailPurpose_vi: "Giữ mẫu sửa chữa và giải quyết cho vấn đề nhà ở, giao hàng và công việc cần giải pháp cụ thể.",
    ...
    auditTrailNote_en: "Recorded for practical fix language across housing, delivery, and workplace settings.",
    auditTrailNote_vi: "Được lưu cho ngôn ngữ sửa lỗi thực tế ở bối cảnh nhà ở, giao hàng và công việc.",
  },
  {
    id: "pa-b1-audit-trail-06-follow-up-message",
    level: "B1",
    focus: "follow_up_message",
    title_en: "Send a follow-up message",
    title_vi: "Gửi tin nhắn theo dõi",
    auditTrailPurpose_en: "Keep a clean follow-up sample for email, text, and short written summaries after a conversation.",
    auditTrailPurpose_vi: "Giữ mẫu theo dõi gọn cho email, tin nhắn và tóm tắt ngắn sau cuộc trò chuyện.",
    ...
    auditTrailNote_en: "Recorded as a concise follow-up template for written records and reminders.",
    auditTrailNote_vi: "Được lưu như mẫu theo dõi ngắn gọn cho hồ sơ viết và lời nhắc.",
  },
  {
    id: "pa-b1-audit-trail-07-workplace-housing-school-community",
    level: "B1",
    ...
    auditTrailPurpose_en: "Keep one flexible sample for cross-setting coordination when the same issue follows you across places.",
    auditTrailPurpose_vi: "Giữ một mẫu linh hoạt cho phối hợp nhiều bối cảnh khi cùng một vấn đề đi theo bạn qua nhiều nơi.",
    ...
    auditTrailNote_en: "Recorded as a cross-setting coordination pattern for Canadian daily life.",
    auditTrailNote_vi: "Được lưu như mẫu phối hợp nhiều bối cảnh cho đời sống hằng ngày ở Canada.",
  },
  {
    id: "pa-b1-audit-trail-08-register-safe-repair",
    level: "B1",
    focus: "register_safe_repair",
    title_en: "Repair a too-direct message",
    title_vi: "Sửa một câu quá trực diện",
    auditTrailPurpose_en: "Keep a register-safe repair sample so a blunt line can be turned into a polite request.",
    auditTrailPurpose_vi: "Giữ mẫu sửa mức độ lịch sự để một câu quá thẳng có thể đổi thành yêu cầu lịch sự.",
    ...
    auditTrailNote_en: "Recorded for register repair, where a direct line becomes a usable polite request.",
    auditTrailNote_vi: "Được lưu cho sửa mức độ lịch sự, nơi một câu trực diện trở thành yêu cầu lịch sự có thể dùng được.",
  },
  {
    id: "pa-b1-audit-trail-09-audit-trail-signoff",
    level: "B1",
    focus: "audit_trail_signoff",
    title_en: "Close and seal the audit trail sample",
    title_vi: "Kết và đóng gói mẫu audit trail",
    auditTrailPurpose_en: "Keep a pre-A11 audit-trail signoff sample that mentions traceability, evidence receipt, pre-integration sealing, handoff, and stable reuse.",
    auditTrailPurpose_vi: "Giữ mẫu signoff audit trail trước A11 có nhắc đến truy vết, biên nhận bằng chứng, đóng gói trước tích hợp, bàn giao và dùng lại ổn định.",
    canadaContext: "Useful when preparing a final note for a team lead, teacher, landlord, or coordinator in Canada.",
    checkpointCriteria_en: ["Say the sample is ready.", "Name the audit trail or handoff step.", "Ask for the final confirmation."],
    checkpointCriteria_vi: ["Nói mẫu đã sẵn sàng.", "Nêu bước audit trail hoặc bàn giao.", "Xin xác nhận cuối cùng."],
    usefulLanguage: [
      { pa: "ਇਹ ਨਮੂਨਾ ਆਡਿਟ ਟ੍ਰੇਲ ਲਈ ਤਿਆਰ ਹੈ।", romanization: "eh namuna audit trail lai tayar hai.", en: "This sample is ready for the audit trail.", vi: "Mẫu này đã sẵn sàng cho audit trail." },
    ...
    qa: {
      ...
      modelAnswer: {
        pa: "ਇਹ ਨਮੂਨਾ ਆਡਿਟ ਟ੍ਰੇਲ ਲਈ ਤਿਆਰ ਹੈ। ਕਿਰਪਾ ਕਰਕੇ ਸਾਈਨ-ਆਫ਼ ਕਰੋ ਜੀ ਅਤੇ ਅਗਲਾ ਪੜਾਅ ਸ਼ੁਰੂ ਕਰੋ।",
        romanization: "eh namuna audit trail lai tayar hai. kirpa karke sign-off karo ji ate agla parra shuru karo.",
        en: "This sample is ready for the audit trail. Please sign off and start the next stage.",
        vi: "Mẫu này đã sẵn sàng cho audit trail. Vui lòng xác nhận hoàn tất và bắt đầu giai đoạn tiếp theo.",
      },
      followUpQuestion: {
    ...
    commonTraps: [
      {
        trap_en: "Treating the audit-trail note like a live operational instruction.",
        trap_vi: "Xem ghi chú audit trail như hướng dẫn vận hành trực tiếp.",
        better: {
          pa: "ਇਹ ਆਡਿਟ ਟ੍ਰੇਲ ਨੋਟ ਹੈ, ਲਾਈਵ ਹਦਾਇਤ ਨਹੀਂ।",
          romanization: "eh audit trail note hai, live hadait nahin.",
          en: "This is an audit-trail note, not a live instruction.",
          vi: "Đây là ghi chú audit trail, không phải hướng dẫn trực tiếp.",
        },
      },
    ],
    auditTrailNote_en: "Recorded as the pre-A11 handoff and seal point for this compact B1 audit-trail pack.",
    auditTrailNote_vi: "Được lưu như điểm bàn giao và đóng gói trước A11 cho bộ audit trail B1 ngắn gọn này.",
  },
];
