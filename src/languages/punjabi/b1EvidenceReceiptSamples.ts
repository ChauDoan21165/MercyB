// src/languages/punjabi/b1EvidenceReceiptSamples.ts
//
// Punjabi B1 evidence receipt samples for Vietnamese-speaking and English-speaking
...
export type PunjabiB1EvidenceReceiptFocus =
  | "explain_situation"
  | "retell_event"
  | "clarify_next_steps"
  | "service_recovery"
...
export type PunjabiEvidenceReceiptLine = {
  pa: string;
  romanization: string;
  en: string;
  vi: string;
...
export type PunjabiEvidenceReceiptTrap = {
  trap_en: string;
  trap_vi: string;
  better: PunjabiEvidenceReceiptLine;
};

export type PunjabiEvidenceReceiptQA = {
  prompt_en: string;
  prompt_vi: string;
  modelAnswer: PunjabiEvidenceReceiptLine;
  followUpQuestion: PunjabiEvidenceReceiptLine;
};

export type PunjabiB1EvidenceReceiptCard = {
  id: string;
  level: "B1";
  focus: PunjabiB1EvidenceReceiptFocus;
  title_en: string;
  title_vi: string;
  evidenceReceiptPurpose_en: string;
  evidenceReceiptPurpose_vi: string;
  canadaContext: string;
  checkpointCriteria_en: string[];
  checkpointCriteria_vi: string[];
  usefulLanguage: PunjabiEvidenceReceiptLine[];
  qa: PunjabiEvidenceReceiptQA;
  commonTraps: PunjabiEvidenceReceiptTrap[];
  evidenceReceiptNote_en: string;
  evidenceReceiptNote_vi: string;
};

export type PunjabiB1EvidenceReceiptScope = {
  level: "B1";
  note_en: string;
  note_vi: string;
  awareness_en: string;
...
export const punjabiB1EvidenceReceiptScope: PunjabiB1EvidenceReceiptScope = {
  level: "B1",
  note_en:
    "Native review is deferred. This evidence receipt keeps stable independent communication samples for later A11 use after completion record, inventory seal, and pre-integration checks.",
  note_vi:
...
export const punjabiB1EvidenceReceiptSamples: PunjabiB1EvidenceReceiptCard[] = [
  {
    id: "pa-b1-evidence-receipt-01-explain-situation",
    level: "B1",
    focus: "explain_situation",
    title_en: "Explain a situation clearly",
    title_vi: "Giải thích tình huống rõ ràng",
    evidenceReceiptPurpose_en: "Keep a stable sample for explaining an access problem, delay, or disruption without overexplaining.",
    evidenceReceiptPurpose_vi: "Giữ mẫu ổn định để giải thích vấn đề truy cập, sự chậm trễ hoặc gián đoạn mà không giải thích dài dòng.",
    ...
    evidenceReceiptNote_en: "Recorded as an evidence-receipt-ready situation-explanation pattern for later review and reuse.",
    evidenceReceiptNote_vi: "Được lưu như mẫu giải thích tình huống đơn giản để xem lại và dùng lại sau.",
  },
  {
    id: "pa-b1-evidence-receipt-02-retell-event",
    level: "B1",
    focus: "retell_event",
    title_en: "Retell an event in order",
    title_vi: "Kể lại sự việc theo thứ tự",
    evidenceReceiptPurpose_en: "Keep a short, stable sequence sample for late arrivals, transit delays, and schedule changes.",
    evidenceReceiptPurpose_vi: "Giữ mẫu trình tự ngắn, ổn định cho việc đến muộn, trễ xe và thay đổi lịch.",
    ...
    evidenceReceiptNote_en: "Recorded as an evidence-receipt-ready sequence model for messages, appointments, and quick follow-ups.",
    evidenceReceiptNote_vi: "Được lưu như mẫu trình tự cho tin nhắn, cuộc hẹn và theo dõi nhanh.",
  },
  {
    id: "pa-b1-evidence-receipt-03-clarify-next-steps",
    level: "B1",
    focus: "clarify_next_steps",
    title_en: "Clarify next steps",
    title_vi: "Làm rõ bước tiếp theo",
    evidenceReceiptPurpose_en: "Keep a polite clarification sample for forms, deadlines, email follow-up, and written confirmation.",
    evidenceReceiptPurpose_vi: "Giữ mẫu làm rõ lịch sự cho biểu mẫu, hạn chót, theo dõi email và xác nhận bằng văn bản.",
    ...
    evidenceReceiptNote_en: "Recorded for safe clarification and written follow-up practice.",
    evidenceReceiptNote_vi: "Được lưu để luyện làm rõ an toàn và theo dõi bằng văn bản.",
  },
  {
    id: "pa-b1-evidence-receipt-04-service-recovery",
    level: "B1",
    focus: "service_recovery",
    title_en: "Recover after a service mistake",
    title_vi: "Khắc phục sau lỗi dịch vụ",
    evidenceReceiptPurpose_en: "Keep a repair sample for wrong charges, missing bookings, or service errors that need correction.",
    evidenceReceiptPurpose_vi: "Giữ mẫu sửa chữa cho phí sai, đặt chỗ bị thiếu hoặc lỗi dịch vụ cần được điều chỉnh.",
    ...
    evidenceReceiptNote_en: "Recorded as a service-recovery script for correction, timing, and reference tracking.",
    evidenceReceiptNote_vi: "Được lưu như kịch bản khắc phục dịch vụ cho sửa lỗi, thời gian xử lý và theo dõi tham chiếu.",
  },
  {
    id: "pa-b1-evidence-receipt-05-issue-resolution",
    level: "B1",
    focus: "issue_resolution",
    title_en: "Resolve a practical issue",
    title_vi: "Giải quyết một vấn đề thực tế",
    evidenceReceiptPurpose_en: "Keep a repair-and-resolution sample for housing, delivery, and workplace issues that need a concrete fix.",
    evidenceReceiptPurpose_vi: "Giữ mẫu sửa chữa và giải quyết cho vấn đề nhà ở, giao hàng và công việc cần giải pháp cụ thể.",
    ...
    evidenceReceiptNote_en: "Recorded for practical fix language across housing, delivery, and workplace settings.",
    evidenceReceiptNote_vi: "Được lưu cho ngôn ngữ sửa lỗi thực tế ở bối cảnh nhà ở, giao hàng và công việc.",
  },
  {
    id: "pa-b1-evidence-receipt-06-follow-up-message",
    level: "B1",
    focus: "follow_up_message",
    title_en: "Send a follow-up message",
    title_vi: "Gửi tin nhắn theo dõi",
    evidenceReceiptPurpose_en: "Keep a clean follow-up sample for email, text, and short written summaries after a conversation.",
    evidenceReceiptPurpose_vi: "Giữ mẫu theo dõi gọn cho email, tin nhắn và tóm tắt ngắn sau cuộc trò chuyện.",
    ...
    evidenceReceiptNote_en: "Recorded as a concise follow-up template for written records and reminders.",
    evidenceReceiptNote_vi: "Được lưu như mẫu theo dõi ngắn gọn cho hồ sơ viết và lời nhắc.",
  },
  {
    id: "pa-b1-evidence-receipt-07-workplace-housing-school-community",
    level: "B1",
    ...
    evidenceReceiptPurpose_en: "Keep one flexible sample for cross-setting coordination when the same issue follows you across places.",
    evidenceReceiptPurpose_vi: "Giữ một mẫu linh hoạt cho phối hợp nhiều bối cảnh khi cùng một vấn đề đi theo bạn qua nhiều nơi.",
    ...
    evidenceReceiptNote_en: "Recorded as a cross-setting coordination pattern for Canadian daily life.",
    evidenceReceiptNote_vi: "Được lưu như mẫu phối hợp nhiều bối cảnh cho đời sống hằng ngày ở Canada.",
  },
  {
    id: "pa-b1-evidence-receipt-08-register-safe-repair",
    level: "B1",
    focus: "register_safe_repair",
    title_en: "Repair a too-direct message",
    title_vi: "Sửa một câu quá trực diện",
    evidenceReceiptPurpose_en: "Keep a register-safe repair sample so a blunt line can be turned into a polite request.",
    evidenceReceiptPurpose_vi: "Giữ mẫu sửa mức độ lịch sự để một câu quá thẳng có thể đổi thành yêu cầu lịch sự.",
    ...
    evidenceReceiptNote_en: "Recorded for register repair, where a direct line becomes a usable polite request.",
    evidenceReceiptNote_vi: "Được lưu cho sửa mức độ lịch sự, nơi một câu trực diện trở thành yêu cầu lịch sự có thể dùng được.",
  },
  {
    id: "pa-b1-evidence-receipt-09-evidence-receipt-signoff",
    level: "B1",
    focus: "evidence_receipt_signoff",
    title_en: "Close and seal the evidence receipt",
    title_vi: "Kết và đóng gói mẫu lưu trữ",
    evidenceReceiptPurpose_en: "Keep a pre-A11 evidence-receipt signoff sample that mentions completion record, inventory seal, pre-integration sealing, handoff, and stable reuse.",
    evidenceReceiptPurpose_vi: "Giữ mẫu signoff biên nhận bằng chứng trước A11 có nhắc đến hồ sơ hoàn tất, kiểm kê đóng gói, đóng gói trước tích hợp, bàn giao và dùng lại ổn định.",
    canadaContext: "Useful when preparing a final note for a team lead, teacher, landlord, or coordinator in Canada.",
    checkpointCriteria_en: ["Say the sample is ready.", "Name the evidence receipt or handoff step.", "Ask for the final confirmation."],
    checkpointCriteria_vi: ["Nói mẫu đã sẵn sàng.", "Nêu bước biên nhận bằng chứng hoặc bàn giao.", "Xin xác nhận cuối cùng."],
    usefulLanguage: [
      { pa: "ਇਹ ਨਮੂਨਾ ਐਵੀਡੈਂਸ ਰਸੀਦ ਲਈ ਤਿਆਰ ਹੈ।", romanization: "eh namuna evidence receipt lai tayar hai.", en: "This sample is ready for the evidence receipt.", vi: "Mẫu này đã sẵn sàng cho biên nhận bằng chứng." },
    ...
    qa: {
      ...
      modelAnswer: {
        pa: "ਇਹ ਨਮੂਨਾ ਐਵੀਡੈਂਸ ਰਸੀਦ ਲਈ ਤਿਆਰ ਹੈ। ਕਿਰਪਾ ਕਰਕੇ ਸਾਈਨ-ਆਫ਼ ਕਰੋ ਜੀ ਅਤੇ ਅਗਲਾ ਪੜਾਅ ਸ਼ੁਰੂ ਕਰੋ।",
        romanization: "eh namuna evidence receipt lai tayar hai. kirpa karke sign-off karo ji ate agla parra shuru karo.",
        en: "This sample is ready for the evidence receipt. Please sign off and start the next stage.",
        vi: "Mẫu này đã sẵn sàng cho biên nhận bằng chứng. Vui lòng xác nhận hoàn tất và bắt đầu giai đoạn tiếp theo.",
      },
      followUpQuestion: {
    ...
    commonTraps: [
      {
        trap_en: "Treating the evidence receipt note like a live operational instruction.",
        trap_vi: "Xem ghi chú biên nhận bằng chứng như hướng dẫn vận hành trực tiếp.",
        better: {
          pa: "ਇਹ ਐਵੀਡੈਂਸ ਰਸੀਦ ਨੋਟ ਹੈ, ਲਾਈਵ ਹਦਾਇਤ ਨਹੀਂ।",
          romanization: "eh evidence receipt note hai, live hadait nahin.",
          en: "This is an evidence receipt note, not a live instruction.",
          vi: "Đây là ghi chú biên nhận bằng chứng, không phải hướng dẫn trực tiếp.",
        },
      },
    ],
    evidenceReceiptNote_en: "Recorded as the pre-A11 handoff and seal point for this compact B1 evidence receipt pack.",
    evidenceReceiptNote_vi: "Được lưu như điểm bàn giao và đóng gói trước A11 cho bộ B1 ngắn gọn này.",
  },
];
