// Punjabi B2 final handoff set for upper-intermediate integration prep.
...
export type PunjabiB2FinalHandoffSetFocus = PunjabiConsistencyReviewB2Focus;
export type PunjabiB2FinalHandoffSetTopic = PunjabiConsistencyReviewB2Topic;

export type PunjabiB2FinalHandoffSetEntry = {
  id: string;
  level: "B2";
  handoffFocus: PunjabiB2FinalHandoffSetFocus;
  topic: PunjabiB2FinalHandoffSetTopic;
  handoffPrompt_gurmukhi: string;
  handoffPrompt_romanization: string;
  handoffPrompt_vi: string;
  handoffPrompt_en: string;
  ...
  handoffNotes_vi: string[];
  handoffNotes_en: string[];
...
const handoffByFocus: Record<
  PunjabiB2FinalHandoffSetFocus,
  {
    preVi: string[];
    preEn: string[];
    readinessVi: string[];
...
> = {
  structured_opinion: {
    ...
    notesVi: ["Dùng cho public issue", "Giữ logic xuyên suốt"],
...
const handoffPromptByFocus: Record<
  PunjabiB2FinalHandoffSetFocus,
  { vi: string; en: string }
> = {
  structured_opinion: {
    vi: "Chọn một ý kiến có stance, reason và conclusion rõ để bàn giao cho giai đoạn tiếp theo.",
    en: "Select an opinion with a clear stance, reason, and conclusion for the next stage.",
  },
  comparison: {
    vi: "Chọn một so sánh giữ được hai phía và tradeoff để bàn giao.",
    en: "Select a comparison that keeps both sides and the tradeoff for handoff.",
  },
  counterpoint: {
...
export const punjabiB2FinalHandoffSet: PunjabiB2FinalHandoffSetEntry[] =
  punjabiConsistencyReviewB2.map((item: PunjabiConsistencyReviewB2Item, index: number) => {
    const handoff = handoffByFocus[item.consistencyFocus];
    const prompt = handoffPromptByFocus[item.consistencyFocus];
    ...
    return {
      id: item.id.replace("consistency", "handoff"),
      level: "B2",
      handoffFocus: item.consistencyFocus,
      topic: item.topic,
      handoffPrompt_gurmukhi: `${item.consistencyPrompt_gurmukhi} ਫਾਈਨਲ ਹਵਾਲੇ ਲਈ ਵਰਤੋ।`,
      handoffPrompt_romanization: `${item.consistencyPrompt_romanization} final hawale lai varto.`,
      handoffPrompt_vi: `${item.consistencyPrompt_vi} Dùng để bàn giao cho giai đoạn cuối.`,
      handoffPrompt_en: `${item.consistencyPrompt_en} Use it for the final handoff stage.`,
      ...
      preIntegration_vi: handoff.preVi,
      preIntegration_en: handoff.preEn,
      finalReadiness_vi: handoff.readinessVi,
      finalReadiness_en: handoff.readinessEn,
      handoffNotes_vi: handoff.notesVi,
      handoffNotes_en: handoff.notesEn,
      selectorGuard_vi: handoff.guardVi,
      selectorGuard_en: handoff.guardEn,
...
export default punjabiB2FinalHandoffSet;
