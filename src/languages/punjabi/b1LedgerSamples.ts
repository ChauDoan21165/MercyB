// src/languages/punjabi/b1LedgerSamples.ts
//
// Punjabi B1 ledger samples for Vietnamese-speaking and English-speaking
...
export type PunjabiB1LedgerFocus =
  | "explain_situation"
  | "retell_event"
  | "clarify_next_steps"
  | "service_recovery"
...
export type PunjabiLedgerTrap = {
  trap_en: string;
  trap_vi: string;
  better: PunjabiLedgerLine;
};

export type PunjabiLedgerQA = {
  prompt_en: string;
  prompt_vi: string;
  modelAnswer: PunjabiLedgerLine;
  followUpQuestion: PunjabiLedgerLine;
};

export type PunjabiB1LedgerCard = {
  id: string;
  level: "B1";
  focus: PunjabiB1LedgerFocus;
  title_en: string;
  title_vi: string;
  ledgerPurpose_en: string;
  ledgerPurpose_vi: string;
  canadaContext: string;
  checkpointCriteria_en: string[];
  checkpointCriteria_vi: string[];
  usefulLanguage: PunjabiLedgerLine[];
  qa: PunjabiLedgerQA;
  commonTraps: PunjabiLedgerTrap[];
  ledgerNote_en: string;
  ledgerNote_vi: string;
};

export type PunjabiB1LedgerScope = {
  level: "B1";
  note_en: string;
  note_vi: string;
  awareness_en: string;
...
export const punjabiB1LedgerScope: PunjabiB1LedgerScope = {
  level: "B1",
  note_en:
    "Native review is deferred. This ledger keeps stable independent communication samples for later A11 use.",
  note_vi:
...
export const punjabiB1LedgerSamples: PunjabiB1LedgerCard[] = [
  {
    id: "pa-b1-ledger-01-explain-situation",
    level: "B1",
    focus: "explain_situation",
    title_en: "Explain a situation clearly",
    title_vi: "Giải thích tình huống rõ ràng",
    ledgerPurpose_en: "Keep a stable sample for explaining an access problem, delay, or disruption without overexplaining.",
    ledgerPurpose_vi: "Giữ mẫu ổn định để giải thích vấn đề truy cập, sự chậm trễ hoặc gián đoạn mà không giải thích dài dòng.",
    ...
    ledgerNote_en: "Ledgerd as a plain situation-explanation pattern for later review and reuse.",
    ledgerNote_vi: "Được lưu như mẫu giải thích tình huống đơn giản để xem lại và dùng lại sau.",
  },
  {
    id: "pa-b1-ledger-02-retell-event",
    level: "B1",
    focus: "retell_event",
    title_en: "Retell an event in order",
    title_vi: "Kể lại sự việc theo thứ tự",
    ledgerPurpose_en: "Keep a short, stable sequence sample for late arrivals, transit delays, and schedule changes.",
    ledgerPurpose_vi: "Giữ mẫu trình tự ngắn, ổn định cho việc đến muộn, trễ xe và thay đổi lịch.",
    ...
    ledgerNote_en: "Ledgerd as a sequence model for messages, appointments, and quick follow-ups.",
    ledgerNote_vi: "Được lưu như mẫu trình tự cho tin nhắn, cuộc hẹn và theo dõi nhanh.",
  },
  {
    id: "pa-b1-ledger-03-clarify-next-steps",
    level: "B1",
    focus: "clarify_next_steps",
    title_en: "Clarify next steps",
    title_vi: "Làm rõ bước tiếp theo",
    ledgerPurpose_en: "Keep a polite clarification sample for forms, deadlines, email follow-up, and written confirmation.",
    ledgerPurpose_vi: "Giữ mẫu làm rõ lịch sự cho biểu mẫu, hạn chót, theo dõi email và xác nhận bằng văn bản.",
    ...
    ledgerNote_en: "Ledgerd for safe clarification and written follow-up practice.",
    ledgerNote_vi: "Được lưu để luyện làm rõ an toàn và theo dõi bằng văn bản.",
  },
  {
    id: "pa-b1-ledger-04-service-recovery",
    level: "B1",
    focus: "service_recovery",
    title_en: "Recover after a service mistake",
    title_vi: "Khắc phục sau lỗi dịch vụ",
    ledgerPurpose_en: "Keep a repair sample for wrong charges, missing bookings, or service errors that need correction.",
    ledgerPurpose_vi: "Giữ mẫu sửa chữa cho phí sai, đặt chỗ bị thiếu hoặc lỗi dịch vụ cần được điều chỉnh.",
    ...
    ledgerNote_en: "Ledgerd as a service-recovery script for correction, timing, and reference tracking.",
    ledgerNote_vi: "Được lưu như kịch bản khắc phục dịch vụ cho sửa lỗi, thời gian xử lý và theo dõi tham chiếu.",
  },
  {
    id: "pa-b1-ledger-05-issue-resolution",
    level: "B1",
    focus: "issue_resolution",
    title_en: "Resolve a practical issue",
    title_vi: "Giải quyết một vấn đề thực tế",
    ledgerPurpose_en: "Keep a repair-and-resolution sample for housing, delivery, and workplace issues that need a concrete fix.",
    ledgerPurpose_vi: "Giữ mẫu sửa chữa và giải quyết cho vấn đề nhà ở, giao hàng và công việc cần giải pháp cụ thể.",
    ...
    ledgerNote_en: "Ledgerd for practical fix language across housing, delivery, and workplace settings.",
    ledgerNote_vi: "Được lưu cho ngôn ngữ sửa lỗi thực tế ở bối cảnh nhà ở, giao hàng và công việc.",
  },
  {
    id: "pa-b1-ledger-06-follow-up-message",
    level: "B1",
    focus: "follow_up_message",
    title_en: "Send a follow-up message",
    title_vi: "Gửi tin nhắn theo dõi",
    ledgerPurpose_en: "Keep a clean follow-up sample for email, text, and short written summaries after a conversation.",
    ledgerPurpose_vi: "Giữ mẫu theo dõi gọn cho email, tin nhắn và tóm tắt ngắn sau cuộc trò chuyện.",
    ...
    ledgerNote_en: "Ledgerd as a concise follow-up template for written records and reminders.",
    ledgerNote_vi: "Được lưu như mẫu theo dõi ngắn gọn cho hồ sơ viết và lời nhắc.",
  },
  {
    id: "pa-b1-ledger-07-workplace-housing-school-community",
    level: "B1",
    ...
    ledgerPurpose_en: "Keep one flexible sample for cross-setting coordination when the same issue follows you across places.",
    ledgerPurpose_vi: "Giữ một mẫu linh hoạt cho phối hợp nhiều bối cảnh khi cùng một vấn đề đi theo bạn qua nhiều nơi.",
    ...
    ledgerNote_en: "Ledgerd as a cross-setting coordination pattern for Canadian daily life.",
    ledgerNote_vi: "Được lưu như mẫu phối hợp nhiều bối cảnh cho đời sống hằng ngày ở Canada.",
  },
  {
    id: "pa-b1-ledger-08-register-safe-repair",
    level: "B1",
    focus: "register_safe_repair",
    title_en: "Repair a too-direct message",
    title_vi: "Sửa một câu quá trực diện",
    ledgerPurpose_en: "Keep a register-safe repair sample so a blunt line can be turned into a polite request.",
    ledgerPurpose_vi: "Giữ mẫu sửa mức độ lịch sự để một câu quá thẳng có thể đổi thành yêu cầu lịch sự.",
    ...
    ledgerNote_en: "Ledgerd for register repair, where a direct line becomes a usable polite request.",
    ledgerNote_vi: "Được lưu cho sửa mức độ lịch sự, nơi một câu trực diện trở thành yêu cầu lịch sự có thể dùng được.",
  },
  {
    id: "pa-b1-ledger-09-ledger-signoff",
    level: "B1",
    focus: "ledger_signoff",
    title_en: "Close and seal the ledger sample",
    title_vi: "Kết và đóng gói mẫu lưu trữ",
    ledgerPurpose_en: "Keep a pre-A11 ledger-signoff sample that mentions sealing, handoff, and stable reuse.",
    ledgerPurpose_vi: "Giữ mẫu signoff trước A11 có nhắc đến đóng gói, bàn giao và dùng lại ổn định.",
    canadaContext: "Useful when preparing a final note for a team lead, teacher, landlord, or coordinator in Canada.",
    checkpointCriteria_en: ["Say the sample is ready.", "Name the ledger or handoff step.", "Ask for the final confirmation."],
    checkpointCriteria_vi: ["Nói mẫu đã sẵn sàng.", "Nêu bước lưu trữ hoặc bàn giao.", "Xin xác nhận cuối cùng."],
    usefulLanguage: [
      { pa: "ਇਹ ਨਮੂਨਾ ਆਰਕਾਈਵ ਲਈ ਤਿਆਰ ਹੈ।", romanization: "eh namuna ledger lai tayar hai.", en: "This sample is ready for the ledger.", vi: "Mẫu này đã sẵn sàng để lưu trữ." },
    ...
    qa: {
      ...
      modelAnswer: {
        ...
        romanization: "eh namuna ledger lai tayar hai. kirpa karke sign-off karo ji ate agla parra shuru karo.",
        en: "This sample is ready for the ledger. Please sign off and start the next stage.",
    ...
    commonTraps: [
      {
        trap_en: "Treating the ledger note like a live operational instruction.",
        trap_vi: "Xem ghi chú lưu trữ như hướng dẫn vận hành trực tiếp.",
        better: {
          pa: "ਇਹ ਆਰਕਾਈਵ ਨੋਟ ਹੈ, ਲਾਈਵ ਹਦਾਇਤ ਨਹੀਂ।",
          romanization: "eh ledger note hai, live hadait nahin.",
          en: "This is an ledger note, not a live instruction.",
          vi: "Đây là ghi chú lưu trữ, không phải hướng dẫn trực tiếp.",
        },
      },
    ],
    ledgerNote_en: "Ledgerd as the pre-A11 handoff and seal point for this compact B1 pack.",
    ledgerNote_vi: "Được lưu như điểm bàn giao và đóng gói trước A11 cho bộ B1 ngắn gọn này.",
  },
];
