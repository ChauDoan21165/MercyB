// FILE: RoomGridVIP3II.tsx
// PATH: src/_legacy_next_pages/RoomGridVIP3II.tsx
// NOTE: Surgical fix — gate must allow VIP3+ (VIP9 passes), and match useUserAccess() API.
...
import { ROOM_GRID_CLASS } from "@/lib/constants";
...
const RoomGridVIP3II = () => {
  ...
  // ✅ FIX: match the real hook API (used elsewhere): isLoading + canAccessTier()
  const { isAdmin, isHighAdmin, isLoading: accessLoading, canAccessTier } = useUserAccess();

  // ✅ FIX: VIP3II is a VIP3 specialization block -> VIP3 and VIP9 should pass.
  // If canAccessTier is strict equality, including vip9 here prevents VIP9 users from being blocked.
  const hasAccess = canAccessTier("vip3") || canAccessTier("vip9") || isHighAdmin || isAdmin;
  ...
  return (
    <div className="min-h-screen">
      <ColorfulMercyBladeHeader subtitle="VIP3 II - Core Specialization" showBackButton={true} />

      <div
        className="min-h-screen"
        style={{ background: "linear-gradient(135deg, hsl(340, 70%, 95%), hsl(320, 70%, 95%))" }}
      >
          ...
          <div className="mb-8 space-y-4">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-gray-600">
                {loading
                  ? "Loading..."
                  : `${vip3IIRooms.length} specialized core rooms — Sexuality, Finance, Psychology, Philosophy`}
              </p>
            </div>
          </div>
          ...
          <Card className="mb-8 bg-white/80 backdrop-blur shadow-lg border-2 border-rose-200">
            <div className="p-6 space-y-6">
              <div className="flex items-start gap-3">
                <div
                  className="flex-shrink-0 w-12 h-12 rounded-full bg-gradient-to-br from-rose-500 to-pink-600 flex items-center justify-center text-white font-bold shadow-lg"
                  aria-hidden="true"
                >
                  VIP3 II
                </div>
                <div className="flex-1 space-y-4">
                  <div>
                    ...
                    <p className="text-gray-800 leading-relaxed mb-4">
                      {highlightTextByRules(
                        "VIP3 II contains large and sensitive topics that require dedicated space: Sexuality & Sex Education, Finance & Money Psychology, Schizophrenia Mastery, Emotional Well-being, Heavy Psychological Topics, and Large Philosophical Clusters. These are CORE topics, not English lessons. VIP3 users have full access to this specialization block.",
                        false
                      )}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-gray-800 leading-relaxed">
                      {highlightTextByRules(
                        "VIP3 II chứa các chủ đề lớn và nhạy cảm cần không gian riêng: Tình Dục & Giáo Dục Giới Tính, Tài Chính & Tâm Lý Tiền Bạc, Làm Chủ Tâm Thần Phân Liệt, Sức Khỏe Cảm Xúc, Các Chủ Đề Tâm Lý Nặng, và Các Cụm Triết Học Lớn. Đây là các chủ đề CỐT LÕI, không phải bài học tiếng Anh. Người dùng VIP3 có quyền truy cập đầy đủ vào khối chuyên biệt này.",
                        true
                      )}
                    </p>
                  </div>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 flex-1 ml-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-gray-800 mb-1">Back to VIP3 Main Collection</h3>
                    <p className="text-gray-600">Core Premium Rooms • Diverse Topics</p>
                    <p className="text-sm text-gray-500 mt-1">Quay lại Bộ sưu tập VIP3 Chính • Chủ đề Đa dạng</p>
                  </div>
                </div>
              </div>
            </Card>
                ...
                return (
                  <Card
                    ...
                    className={`relative p-3 transition-all duration-300 cursor-pointer group animate-fade-in ${
                      hasData ? "hover:scale-110 hover:shadow-xl hover:z-10" : "opacity-60 cursor-not-allowed"
                    }`}
                    style={{
                      background: "white",
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <div className="space-y-1">
                        <p className="text-xs font-bold leading-tight line-clamp-2 text-foreground">{room.title_en}</p>
                        <p className="text-[10px] leading-tight line-clamp-2 text-muted-foreground">{room.title_vi}</p>
                      </div>
                    </div>
