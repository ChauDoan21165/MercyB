/**
 * MercyBlade Blue — Welcome
 * Path: src/_legacy_next_pages/Welcome.tsx
 * Version: MB-BLUE-94.14.13 — 2025-12-25 (+0700)
 *
 * CHANGE (94.14.13):
 * - Remove legacy Supabase import "@/lib/supabaseClient".
 * - Use canonical client "@/lib/supabaseClient".
 * - Keep auth timeline single-source via useAuth().
 * - Keep profile lookup + UsernameSetup gating unchanged.
 * - Logout routes to /logout (NUCLEAR logout).
 */

import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { Check, LogOut, User, ArrowRight } from "lucide-react";
...
const Welcome = () => {
  ...
  const { isAdmin, tier, canAccessVIP1, canAccessVIP2, canAccessVIP3, canAccessVIP4 } =
    useUserAccess();
  ...
  // Profile loader (runs when auth user changes)
  useEffect(() => {
    let alive = true;

    const loadProfile = async () => {
      // While auth is loading, don't do anything.
      if (isLoading) return;
      ...
      try {
        ...
        if (!error && profileData) {
          ...
          // Only show username setup if username is null/empty AND user hasn't skipped
          const hasUsername =
            profileData.username && String(profileData.username).trim().length > 0;
  ...
  const handleLogout = async () => {
    // Always use NUCLEAR logout page to avoid half-cleared auth state.
    navigate("/logout");
  };
  ...
  const tiers = [
    ...
    },
    {
      ...
      features: {
        ...
        vi: [
          "Người dùng có thể yêu cầu một chủ đề tùy chỉnh",
          "Truy cập tự do 1 phòng/ngày",
          "🤖 Nội dung tạo bởi AI",
        ],
      },
      popular: true,
    },
    {
      ...
      features: {
        ...
        vi: [
          "Người dùng có thể yêu cầu hai chủ đề tùy chỉnh",
          "Truy cập tự do 2 phòng/ngày",
          "🤖 Nội dung tạo bởi AI",
        ],
      },
    },
    {
      ...
      features: {
        ...
        vi: [
          "Người dùng có thể yêu cầu ba chủ đề tùy chỉnh",
      ...
      <div className="min-h-screen" style={{ background: "hsl(var(--page-welcome))" }}>
        ...
        <div className="container mx-auto px-4 py-12 max-w-7xl">
              ...
              <div className="p-8 space-y-6">
                <div className="space-y-3">
                  <p className="text-base leading-relaxed text-foreground/90">
                    Mercy Blade is your compassionate companion on the journey of holistic
                    development — from physical health, mental well-being, relationships,
                    finances, to personal growth. The app provides concise, evidence-based, and
                    easy-to-apply guidance to help you recover from burnout, build healthy
                    habits, manage chronic conditions, and nurture confidence and inner peace.
                    Whether you're looking to reduce stress, improve health, or develop yourself,
                    Mercy Blade walks alongside you in a space full of understanding,
                    encouragement, and non-judgment. (After some time using the app, we can help
                    you find the most compatible learning buddy and potentially your soulmate.)
                  </p>
                  <p className="text-base leading-relaxed text-muted-foreground">
                    Mercy Blade là người bạn đồng hành đầy lòng trắc ẩn trên hành trình phát triển
                    toàn diện của bạn — từ sức khỏe thể chất, tinh thần, mối quan hệ, tài chính
                    cho đến sự trưởng thành cá nhân. Ứng dụng cung cấp những hướng dẫn ngắn gọn,
                    dựa trên bằng chứng và dễ áp dụng, giúp bạn phục hồi sau kiệt sức, xây dựng
                    thói quen lành mạnh, quản lý bệnh mạn tính, cũng như nuôi dưỡng sự tự tin và
                    bình an nội tâm. Dù bạn đang tìm cách giảm căng thẳng, cải thiện sức khỏe hay
                    phát triển bản thân, Mercy Blade luôn đồng hành trong một không gian đầy thấu
                    hiểu, khích lệ và không phán xét. (Sau một thời gian sử dụng ứng dụng, chúng
                    tôi có thể giúp bạn tìm được người bạn học tập phù hợp nhất và có thể là tri
                    kỷ của bạn.)
                  </p>
                </div>
              </div>
          ...
          <div className="text-center mt-16 space-y-2">
            <p className="text-sm text-muted-foreground">
              Learn English vocabulary through real-world content about health, life, and personal
              development
            </p>
            <p className="text-xs text-muted-foreground">
          ...
          <div className="space-y-8 mt-16">
            <div className="text-center space-y-2">
              ...
              <p className="text-lg text-muted-foreground">Chọn Gói Phù Hợp Với Bạn</p>
            </div>
                ...
                >
                  <div className="space-y-4 flex flex-col flex-grow">
                    <div className="space-y-1">
                      <h3 className="text-xl font-bold text-foreground">{tierItem.name.en}</h3>
                      <p className="text-sm text-muted-foreground">{tierItem.name.vi}</p>
                    </div>

                    <div className="flex items-baseline gap-1">
                      <span className="text-4xl font-bold text-primary">
                        {isYearly ? tierItem.priceYearly : tierItem.priceMonthly}
                      </span>
                      {tierItem.priceMonthly !== "$0" && (
                    ...
                    {isYearly && tierItem.priceMonthly !== "$0" && (
                      <p className="text-xs text-muted-foreground">Save 33% vs monthly / Tiết kiệm 33%</p>
                    )}

                    <div className="space-y-3 pt-4 flex-grow">
                      {tierItem.features.en.map((feature, idx) => (
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            size="icon"
                            disabled={tierItem.requiresAuth && !user}
                            onClick={() => {
                              if (index === 0) {
                                if (user) navigate("/rooms");
                                else toast("Please sign in first / Vui lòng đăng nhập trước");
                              } else {
                                navigate(`/subscribe?tier=${tierItem.name.en.toLowerCase()}`);
                              }
                            }}
                          >
          ...
          <Card className="max-w-4xl mx-auto mt-12 p-3 bg-primary/5 dark:bg-primary/10 border-primary/20">
            <div className="space-y-1.5 text-xs text-muted-foreground leading-relaxed">
              <p>
                This app provides general wellness guidance and educational content. It is NOT a
                substitute for professional medical, psychological, or financial advice. If you are
                experiencing a medical emergency or mental health crisis, please contact emergency
                services immediately.
              </p>
              <p>
                Ứng dụng này cung cấp hướng dẫn sức khỏe tổng thể và nội dung giáo dục. Nó KHÔNG thay
                thế lời khuyên y tế, tâm lý hoặc tài chính chuyên nghiệp. Nếu bạn đang gặp cấp cứu y
                tế hoặc khủng hoảng sức khỏe tâm thần, vui lòng liên hệ dịch vụ cấp cứu ngay lập tức.
              </p>
            </div>
          </Card>
        </div>
