...
import { shoppingSpeakTopics } from "@/lib/tutor/speakTopics/shopping";

// Covers the D4-deepened shopping theme (12 topics).
...
describe("shopping speak topics — D4 metadata", () => {
  ...
  it("topic ids are unique within the theme", () => {
    const ids = shoppingSpeakTopics.map((t) => t.id);
