...
// Step 11 — Vietlish corpus WAVE 9 (F1 Content Factory) schema guard.
...
describe("step11 vietlish corpus wave 9 (F1 review queue)", () => {
  it("is the F1 wave-9 review queue, not wired into the engine", () => {
    expect(data.corpus).toBe("step11-vietlish-corpus-d4-wave9");
  ...
  it("holds exactly 200 entries and a matching count field", () => {
    expect(data.entries).toHaveLength(180);
