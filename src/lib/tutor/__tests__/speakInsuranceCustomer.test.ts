...
import { insuranceCustomerSpeakTopics } from "@/lib/tutor/speakTopics/insuranceCustomerEnglish";

// Covers the D4-deepened insurance customer theme (6 topics).
...
describe("insurance customer speak topics — D4 metadata", () => {
  const libraryIds = new Set(SPEAK_TOPIC_LIBRARY.map((t) => t.id));

  it("ships 6 topics and auto-registers all in the library", () => {
    expect(insuranceCustomerSpeakTopics).toHaveLength(6);
    for (const topic of insuranceCustomerSpeakTopics) {
      expect(libraryIds.has(topic.id), `${topic.id} not in library`).toBe(true);
      expect(topic.category).toBe("insurance-customer");
    }
  });
  ...
  it("topic ids are unique within the theme", () => {
    const ids = insuranceCustomerSpeakTopics.map((t) => t.id);
