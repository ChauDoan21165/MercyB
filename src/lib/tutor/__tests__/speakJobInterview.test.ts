...
import { jobInterviewSpeakTopics } from "@/lib/tutor/speakTopics/jobInterview";

// Covers the D4-deepened jobInterview theme (10 topics).
...
describe("jobInterview speak topics — D4 metadata", () => {
  const libraryIds = new Set(SPEAK_TOPIC_LIBRARY.map((t) => t.id));

  it("ships 10 topics and auto-registers all in the library", () => {
    expect(jobInterviewSpeakTopics).toHaveLength(10);
    for (const topic of jobInterviewSpeakTopics) {
  ...
  it("topic ids are unique within the theme", () => {
    const ids = jobInterviewSpeakTopics.map((t) => t.id);
