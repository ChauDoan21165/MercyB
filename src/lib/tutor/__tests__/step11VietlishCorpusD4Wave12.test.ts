...
// Step 11 — Vietlish corpus WAVE 12 (A5 pool) schema guard.
// Review-queue only; not wired into the live engine. Dedupe is asserted
// case-insensitively vs. live VIETLISH_CORPUS and all pending waves 1-11.
...
describe("step11 vietlish corpus wave 12 (A5 pool review queue)", () => {
  it("is the A5 wave-12 review queue, not wired into the engine", () => {
    expect(data.corpus).toBe("step11-vietlish-corpus-d4-wave12");
  ...
  it("every context sentence embeds the vietlish string, case-insensitively", () => {
    const offenders = data.entries.filter(
      (e) => !normalize(e.context).includes(normalize(e.vietlish)),
    );
    expect(offenders.map((e) => e.vietlish)).toEqual([]);
  });

  it("does not collide with live corpus or pending waves 1-11, case-insensitively", () => {
