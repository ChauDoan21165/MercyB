...
import { transportationSpeakTopics } from "@/lib/tutor/speakTopics/transportation";

// Covers the D4-deepened transportation theme (13 topics).
...
describe("transportation speak topics — D4 metadata", () => {
  const libraryIds = new Set(SPEAK_TOPIC_LIBRARY.map((t) => t.id));

  it("ships 13 topics and auto-registers all in the library", () => {
    expect(transportationSpeakTopics).toHaveLength(13);
    for (const topic of transportationSpeakTopics) {
      expect(libraryIds.has(topic.id), `${topic.id} not in library`).toBe(true);
      expect(topic.category).toBe("transportation");
    }
  });
  ...
  it("topic ids are unique within the theme", () => {
    const ids = transportationSpeakTopics.map((t) => t.id);
