...
import { speakTopics } from "@/lib/tutor/speakTopics/airportImmigration";

const VIETNAMESE_DIACRITIC_PATTERN = /[À-ɏḀ-ỿ]/u;

describe("Airport & immigration customer English speak topics", () => {
  it("ships the authored set and auto-registers in the speak topic library", () => {
    ...
    for (const topic of speakTopics) {
      ...
      expect(topic.category).toBe("airport");
    }
  });
