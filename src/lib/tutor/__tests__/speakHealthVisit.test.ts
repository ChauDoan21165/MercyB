...
import { healthVisitSpeakTopics } from "@/lib/tutor/speakTopics/healthVisit";

// Covers the D4-deepened healthVisit theme (13 topics).
...
describe("healthVisit speak topics — D4 metadata", () => {
  ...
  it("topic ids are unique within the theme", () => {
    const ids = healthVisitSpeakTopics.map((t) => t.id);
