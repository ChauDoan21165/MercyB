...
// Step 11 — Vietlish corpus WAVE 7 (A10) schema + dedup guard.
// Review-queue candidate file ONLY; NOT wired into the live engine. Authoring
// deduped against live corpus + all 6 merged waves (1546-item denylist);
...
describe("step11 vietlish corpus wave 7 (review queue)", () => {
  it("is the A10 wave-7 review queue, not wired into the engine", () => {
    expect(data.corpus).toBe("step11-vietlish-corpus-d4-wave7");
  ...
  it("holds exactly 54 entries and a matching count field", () => {
    expect(data.entries).toHaveLength(26);
