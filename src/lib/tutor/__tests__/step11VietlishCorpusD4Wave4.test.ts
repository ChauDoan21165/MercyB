...
// Step 11 — Vietlish corpus WAVE 4 (A10 / D4-2) schema guard.
// Locks invariants on the review-queue candidate file ONLY; it is NOT wired
// into the live engine. File-disjoint from A4's d4-wave-200 and A10's wave2;
// authoring deduped against live + wave1 + wave2 (749-item denylist). This
// test re-asserts the live-corpus half of that guarantee at runtime.
...
describe("step11 vietlish corpus wave 4 (D4-2 review queue)", () => {
  it("is the A10 wave-4 review queue, not wired into the engine", () => {
    expect(data.corpus).toBe("step11-vietlish-corpus-d4-wave4");
    expect(data.wiredIntoEngine).toBe(false);
    expect(data.status).toBe("review_not_wired");
  });

  it("holds exactly 182 entries and a matching count field", () => {
    // 200 authored; 1 cross-wave duplicate removed in the 2026-06-10 dedup pass
    ...
    expect(data.entries).toHaveLength(182);
    expect(data.count).toBe(182);
  });
