...
import { schoolParentSpeakTopics } from "@/lib/tutor/speakTopics/schoolParent";

// Covers the D4-deepened school-parent theme (12 topics).
...
describe("school-parent speak topics — D4 metadata", () => {
  const libraryIds = new Set(SPEAK_TOPIC_LIBRARY.map((t) => t.id));

  it("ships 12 topics and auto-registers all in the library", () => {
    expect(schoolParentSpeakTopics).toHaveLength(12);
    for (const topic of schoolParentSpeakTopics) {
      expect(libraryIds.has(topic.id), `${topic.id} not in library`).toBe(true);
      expect(topic.category).toBe("school-parent");
    }
  });
  ...
  it("topic ids are unique within the theme", () => {
    const ids = schoolParentSpeakTopics.map((t) => t.id);
