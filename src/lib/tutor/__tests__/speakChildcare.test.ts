...
import { childcareSpeakTopics } from "@/lib/tutor/speakTopics/childcare";

// Covers the D4-deepened childcare theme (7 topics).
...
describe("childcare speak topics — D4 metadata", () => {
  const libraryIds = new Set(SPEAK_TOPIC_LIBRARY.map((t) => t.id));

  it("ships 7 topics and auto-registers all in the library", () => {
    expect(childcareSpeakTopics).toHaveLength(7);
    for (const topic of childcareSpeakTopics) {
  ...
  it("topic ids are unique within the theme", () => {
    const ids = childcareSpeakTopics.map((t) => t.id);
