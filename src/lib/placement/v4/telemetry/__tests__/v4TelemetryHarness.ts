...
export function scenarioDenseUserTimeline(options?: {
  ...
  const modalities: PlacementV3Modality[] = ["reading", "writing", "speaking", "listening", "conversation"];
...
export function scenarioPlan(
  ...
  return makePlan({
    ...
    days: Array.from({ length: d }, (_, dayIdx) => ({
      ...
      isRecoveryDay: intens === "gentle" && dayIdx % 4 === 3,
    })),
      ...
      events
        .filter(
          (e): e is TelemetryEvent & { lessonId: string } =>
            "lessonId" in e && typeof (e as unknown as Record<string, unknown>).lessonId === "string",
        )
        .map((e) => e.lessonId),
    ),
  ].sort();
