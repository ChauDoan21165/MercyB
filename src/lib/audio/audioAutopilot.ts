  ...
  if (typeof window !== "undefined" && window.localStorage) {
    try {
      ...
      const merged: AutopilotStatusStore = {
        ...
        governanceFlags:
          status.governanceFlags.length > 0
...
export function rejectGovernanceReview(
  id: string,
  reviewedBy?: string,
  notes?: string
): boolean {
 ...
 */
export function writeAutopilotStatusToFile(
  status: AutopilotStatusStore,
  outputPath?: string
): void {
  console.log(
    "[Autopilot] Status ready to write:",
 ...
 */
export function writeAutopilotReportToFile(
 ...
 */
export function writeAutopilotChangeSetToFile(
  changeSet: AudioChangeSet,
  timestamp: string,
  outputPath?: string
): void {
 ...
 */
export function writeAutopilotHistoryToFile(
 ...
 */
export function writePendingGovernanceToFile(
  db: PendingGovernanceDB,
  outputPath?: string
): void {
...
// ============================================
// Type Compatibility Helpers (v4.6)
// ============================================

/**
 * Some audio subsystems historically disagreed on slug types (string vs number).
 * Keep this local + surgical: coerce numeric slugs into strings at the call boundary.
 * NO behavior change, only typing / normalization safety.
 */
type AnyEntryLike = {
  slug?: unknown;
  id?: unknown;
...
function coerceEntriesForAudioSubsystem<T extends AnyEntryLike>(
  entries: T[]
): Array<
  T & {
    slug?: string;
...
> {
  return (entries || []).map((e) => {
    const slugRaw = (e as any).slug;
    const nextSlug =
      typeof slugRaw === "number"
        ? String(slugRaw)
        : typeof slugRaw === "string"
          ? slugRaw
          : undefined;

    // Preserve original object shape, only coerce slug when needed.
    return { ...(e as any), slug: nextSlug };
  });
}

function emptyIntegritySummary(): IntegritySummary {
  return {
    totalRooms: 0,
    healthyRooms: 0,
    roomsWithIssues: 0,
...
function logChangeToLifecycle(
  ...
  if (
    change.type === "rename" ||
    change.type === "attach-orphan" ||
    change.type === "fix-json-ref"
  ) {
...
export async function runAutopilotCycle(
  ...
  try {
    ...
    if (
      fullConfig.maxRoomsPerRun &&
      targetRooms.length > fullConfig.maxRoomsPerRun
    ) {
    ...
    for (const room of targetRooms) {
      const roomResult = getCanonicalAudioForEntireRoom(
        room.roomId,
        // typing/compat: coerce numeric slugs to string for subsystems that require string
        coerceEntriesForAudioSubsystem(room.entries as any),
        storageFiles
      );
    ...
    for (const decision of decisions) {
      if (decision.decision === "block") {
        ...
        if (
          fullConfig.governanceMode === "strict" ||
          fullConfig.governanceMode === "assisted"
        ) {
      ...
      for (const change of getAllChanges(changeSet)) {
        if (isChangeApproved(change, decisions)) {
          const decision = decisions.find(
            (d) => d.operation.metadata?.changeId === change.id
          );
          const lifecycleResult = logChangeToLifecycle(
            change,
            decision,
            lifecycleDB
          );
    ...
    result.meetsThreshold =
      afterIntegrity >= (fullConfig.minIntegrityThreshold || 99);
  ...
  for (const room of roomResults) {
    for (const issue of room.allIssues) {
      const change: AudioChange = {
        id: `${room.roomId}-${issue.type}-${Date.now()}-${Math.random()
          .toString(36)
          .substr(2, 9)}`,
        roomId: room.roomId,
...
function calculateIssueConfidence(issue: {
  type: string;
  autoFixable: boolean;
}): number {
...
function isChangeApproved(
  ...
  return (
    ...
    decision?.decision === "governance-approve"
  );
}

function findChangeByDecision(
  changeSet: AudioChangeSet,
  decision: GovernanceDecision
): AudioChange | undefined {
...
export function generateAutopilotReport(
  ...
  config: AutopilotConfig,
  roomResults: GCERoomResult[] = [],
  integritySummary: IntegritySummary = emptyIntegritySummary()
): AutopilotReport {
...
export function updateAutopilotStatusStore(
  updates: Partial<AutopilotStatusStore>
): void {
