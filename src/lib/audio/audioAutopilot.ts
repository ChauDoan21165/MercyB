// FILE: autopilotEngine.ts
// PATH: src/lib/audio/autopilotEngine.ts
// END-OF-PROJECT SAFE PATCH (2026-02-24):
// - Do NOT change runtime behavior.
// - Fix TS-only break: IntegritySummary doesn't include roomsBelow80.
//   => Use a minimal default IntegritySummary shape.
...
import { matchAudioToEntry, batchMatchOrphans, type SemanticMatch } from "./semanticMatcher";
import {
  evaluateChangeSet,
  evaluateOperation,
  getSystemIntegrity,
  ...
  if (typeof window !== "undefined" && window.localStorage) {
    try {
      ...
      const merged: AutopilotStatusStore = {
        ...
        governanceFlags: status.governanceFlags.length > 0 ? status.governanceFlags : existing.governanceFlags,
      };
...
export async function runAutopilotCycle(
  ...
  try {
    ...
    for (const room of targetRooms) {
      const roomResult = getCanonicalAudioForEntireRoom(room.roomId, room.entries, storageFiles);
    ...
    for (const decision of decisions) {
      if (decision.decision === "block") {
        ...
        // In strict/assisted mode, add blocked items to pending review
    ...
    // =====================
    const afterIntegrity = fullConfig.mode === "apply" ? getSystemIntegrity(targetRooms, storageFiles) : beforeIntegrity;
...
function buildChangeSetFromResults(roomResults: GCERoomResult[], _storageFiles: Set<string>): AudioChangeSet {
...
function changeSetToOperations(changeSet: AudioChangeSet): GovernanceOperation[] {
  ...
  for (const change of getAllChanges(changeSet)) {
    operations.push({
      type: change.type === "attach-orphan" ? "attach-orphan" : change.type === "rename" ? "rename" : "update-json",
...
function applyApprovedChanges(changeSet: AudioChangeSet, decisions: GovernanceDecision[]): number {
  let applied = 0;
...
function countRoomsWithAppliedChanges(changeSet: AudioChangeSet, decisions: GovernanceDecision[]): number {
...
// ✅ FIX (TS-only): IntegritySummary type changed and no longer includes roomsBelow80.
// Provide a minimal, version-tolerant default that satisfies the *current* IntegritySummary type
// without changing runtime behavior.
const DEFAULT_INTEGRITY_SUMMARY = {
  totalRooms: 0,
  healthyRooms: 0,
  roomsWithIssues: 0,
  totalExpected: 0,
...
export function generateAutopilotReport(
  ...
  config: AutopilotConfig,
  roomResults: GCERoomResult[] = [],
  integritySummary: IntegritySummary = DEFAULT_INTEGRITY_SUMMARY
): AutopilotReport {
