 ...
 *
 * BUILD-SAFE FIX (2026-02-24):
 * - RoomEntry.id is typed as string in the app code (id?: string).
 * - This test previously used numeric ids (0/1/2) which breaks TS (TS2322).
 * - Keep the intent (“index-based match”) by using stringified indices ("0","1","2").
 */

import { describe, it, expect } from "vitest";
import { matchAudioToEntry, batchMatchOrphans, getMatchConfidence } from "../semanticMatcher";

describe("semanticMatcher", () => {
  describe("matchAudioToEntry", () => {
    const entries = [{ slug: "breathing-exercise" }, { slug: "anger-tips" }, { slug: "meditation" }];

    it("finds exact match", () => {
      const result = matchAudioToEntry("anger-room-breathing-exercise-en.mp3", "anger-room", entries);
    ...
    it("finds index-based match", () => {
      const result = matchAudioToEntry("anger-room-entry-2-en.mp3", "anger-room", [
        { id: "0" },
        { id: "1" },
        { id: "2" },
      ]);
    ...
    it("flags low confidence matches for human review", () => {
      const result = matchAudioToEntry("anger-room-random-name-en.mp3", "anger-room", entries);
  ...
  describe("batchMatchOrphans", () => {
    it("separates auto-repairs from human review", () => {
      ...
      const { autoRepairs, humanReview } = batchMatchOrphans(orphans, "test-room", entries);
    ...
    it("returns empty arrays for no orphans", () => {
      const { autoRepairs, humanReview } = batchMatchOrphans([], "test-room", [{ slug: "entry-1" }]);
  ...
  describe("getMatchConfidence", () => {
    it("returns 1 for identical strings", () => {
      expect(getMatchConfidence("test-room-entry-1-en.mp3", "test-room-entry-1-en.mp3")).toBe(1);
    });

    it("returns high score for similar strings", () => {
      const score = getMatchConfidence("test-room-entry-1-en.mp3", "test-room-entry-2-en.mp3");
      expect(score).toBeGreaterThan(0.8);
    });

    it("returns low score for different strings", () => {
      const score = getMatchConfidence("test-room-entry-1-en.mp3", "completely-different-room-vi.mp3");
      expect(score).toBeLessThan(0.5);
    });

    it("is case insensitive", () => {
      const score = getMatchConfidence("TEST-ROOM-entry-1-en.mp3", "test-room-entry-1-en.mp3");
      expect(score).toBe(1);
    });
  });
});