...
export interface SemanticMatch {
  filename: string;
  matchedEntry: { slug: string; index: number } | null;
  confidence: number;
...
/**
 * ✅ Canonical RoomEntry shape aligned with integrityMap.
 * Slug/id/artifact_id are string-only.
 */
...
function asSlug(entry: RoomEntry, fallbackIndex: number): string {
  const raw = entry.slug ?? entry.artifact_id ?? entry.id ?? String(fallbackIndex);
  return String(raw);
}
...
function findSlugMatch(slugPart: string, entries: RoomEntry[]): MatchResult | null {
  ...
  for (let i = 0; i < entries.length; i++) {
    const entry = entries[i];
    const entrySlug = normToken(asSlug(entry, i));
