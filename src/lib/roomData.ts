...
function generateRoomInfo(): RoomInfo[] {
  ...
  for (const [roomId, roomData] of Object.entries(roomDataMap)) {
    ...
    // ✅ IMPORTANT: tier MUST come from roomId (supports -vip3 and _vip3 etc)
    ...
    rooms.push({
      ...
      nameVi,
      tier,
      hasData: !!(roomData?.hasData || (Array.isArray(roomData?.entries) && roomData.entries.length > 0)),
    });
  }
  ...
  console.log(
    ...
    rooms.length,
    "Free rooms:",
    rooms.filter((r) => r.tier === "free").length
  );
