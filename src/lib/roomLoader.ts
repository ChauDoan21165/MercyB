...
export const loadMergedRoom = async (roomId: string, tier: string = 'free') => {
  ...
  // Regular rooms: URL and DB both use hyphens (nutrition-vip2)
  let dbRoomId = roomId;
  if (roomId.endsWith('_kids_l1')) {
    ...
    // Regular room: use roomId as-is (no conversion needed)
  ...
  try {
    ...
    if (dbRoom && !error) {
      ...
      if (hasEntries || hasKeywords) {
        ...
        // Extract keywords from entries to get proper en/vi pairing
        if (hasEntries) {
          const enList: string[] = [];
          const viList: string[] = [];
