  ...
  if (repairedRoom.entries && Array.isArray(repairedRoom.entries)) {
    repairedRoom.entries = repairedRoom.entries.map(
      (entry: RoomJson["entries"][number], index: number) => {
        if (entry?.slug && !RULES.SLUG.PATTERN.test(String(entry.slug))) {
          const fixedSlug = String(entry.slug)
            .toLowerCase()
            .replace(/_/g, "-")
            .replace(/[^a-z0-9-]/g, "")
            .replace(/-+/g, "-")
          ...
          if (fixedSlug !== entry.slug) {
            changesApplied.push(
              `Fixed slug in entry ${index}: "${entry.slug}" → "${fixedSlug}"`
            );
            return { ...entry, slug: fixedSlug } as RoomJson["entries"][number];
          }
        }
        return entry;
      }
    ...
    repairedRoom.entries = repairedRoom.entries.map(
      (entry: RoomJson["entries"][number], index: number) => {
        if (
          ...
          changesApplied.push(
            `Fixed audio in entry ${index}: "${entry.audio}" → "${fixedAudio}"`
          );
          return { ...entry, audio: fixedAudio } as RoomJson["entries"][number];
        }
        return entry;
      }
    );
...
function normalizeWhitespace(room: RoomJson, changes: string[]): RoomJson {
  ...
  if (normalized.entries && Array.isArray(normalized.entries)) {
    normalized.entries = normalized.entries.map(
      (entry: RoomJson["entries"][number], index: number) => {
        if (entry?.copy && entry.copy.en) {
          const cleaned = String(entry.copy.en)
        ...
        if (entry?.copy && entry.copy.vi) {
          const cleaned = String(entry.copy.vi)
            .replace(/\s+/g, " ")
            .replace(/\.\.\./g, "…")
            .trim();
...
function rebuildKeywordsIfMissing(room: RoomJson, changes: string[]): RoomJson {
  ...
  if (rebuilt.entries && Array.isArray(rebuilt.entries)) {
    rebuilt.entries = rebuilt.entries.map(
      (entry: RoomJson["entries"][number], index: number) => {
...
function extractKeywordsFromText(text: string, min: number, max: number): string[] {
  ...
  // NOTE: "min" kept for signature stability; currently unused by the simple heuristic.
