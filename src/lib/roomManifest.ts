/**
 * AUTO-GENERATED: Do not edit manually
 * Generated by: scripts/generate-room-registry.js
 * Run: node scripts/generate-room-registry.js
 */
export const PUBLIC_ROOM_MANIFEST: Record<string, string> = {
  "adhd-support-vip1": "data/ADHD_Support_vip1.json",
  "adhd-support-vip2": "data/ADHD_Support_vip2.json",
  "adhd-support-vip3": "data/ADHD_Support_vip3.json",
  "ai-free": "data/AI_free.json",
  "ai-vip1": "data/AI_vip1.json",
  "ai-vip2": "data/AI_vip2.json",
  "ai-vip3": "data/AI_vip3.json",
  "addiction-support-free": "data/Addiction_Support_free.json",
  "addiction-support-vip1": "data/Addiction_Support_vip1.json",
  "addiction-support-vip2": "data/Addiction_Support_vip2.json",
  "addiction-support-vip3": "data/Addiction_Support_vip3.json",
  "adhd-support-free": "data/Adhd_Support_free.json",
  "anxiety-relief-free": "data/Anxiety_Relief_free.json",
  "anxiety-relief-vip1": "data/Anxiety_Relief_vip1.json",
  "anxiety-relief-vip2": "data/Anxiety_Relief_vip2.json",
  "anxiety-relief-vip3": "data/Anxiety_Relief_vip3.json",
  "bipolar-support-free": "data/Bipolar_Support_free.json",
  "bipolar-support-vip1": "data/Bipolar_Support_vip1.json",
  "bipolar-support-vip2": "data/Bipolar_Support_vip2.json",
  "bipolar-support-vip3": "data/Bipolar_Support_vip3.json",
  "bridge-to-reality-vip4": "data/Bridge_to_Reality_vip4.json",
  "burnout-recovery-free": "data/Burnout_Recovery_free.json",
  "burnout-recovery-vip1": "data/Burnout_Recovery_vip1.json",
  "burnout-recovery-vip2": "data/Burnout_Recovery_vip2.json",
  "burnout-recovery-vip3": "data/Burnout_Recovery_vip3.json",
  "calm-body-calm-money-vip3-sub1": "data/Calm_Body,_Calm_Money_vip3_sub1.json",
  "calm-confident-sexual-health-vip3-sub3-sex": "data/Calm_Confident_Sexual_Health_vip3_sub3_sex.json",
  "career-community-vip4": "data/Career_Community_vip4.json",
  "career-consultant-free": "data/Career_Consultant_free.json",
  "choosing-your-path-vip4": "data/Choosing_Your_Path_vip4.json",
  "confidence-building-vip2": "data/Confidence_Building_vip2.json",
  "courage-to-begin-vip4": "data/Courage_To_Begin_vip4.json",
  "depression-support-vip1": "data/Depression_Support_vip1.json",
  "depression-support-vip3": "data/Depression_Support_vip3.json",
  "diverse-desires-&-belonging-vip3-sub5-sex": "data/Diverse_Desires_&_Belonging_vip3_sub5_sex.json",
  "eating-disorder-support-free": "data/Eating_Disorder_Support_free.json",
  "eating-disorder-support-vip1": "data/Eating_Disorder_Support_vip1.json",
  "eating-disorder-support-vip2": "data/Eating_Disorder_Support_vip2.json",
  "eating-disorder-support-vip3": "data/Eating_Disorder_Support_vip3.json",
  "english-writing-free": "data/english-writing-free.json",
  "english-specialization-mastery-vip3-ii": "data/english_specialization_mastery_vip3_ii.json",
  "english-specialization-mastery-module2-vip3-ii": "data/English_Specialization_Mastery_Module2_vip3_ii.json",
  "erotic-wisdom-for-life-vip3-sub-6-sex": "data/Erotic_Wisdom_for_Life_vip3_sub_6_sex.json",
  "finance-calm-money-clear-future-preview-free": "data/Finance_Calm_Money_Clear_Future_Preview_free.json",
  "finance-calm-money-sub1-nervous-system-vip3": "data/Finance_Calm_Money_Sub1_Nervous_System_vip3.json",
  "finance-calm-money-sub2-money-basics-vip3": "data/Finance_Calm_Money_Sub2_Money_Basics_vip3.json",
  "finance-calm-money-sub3-growing-money-vip3": "data/Finance_Calm_Money_Sub3_Growing_Money_vip3.json",
  "finance-calm-money-sub4-vip3": "data/Finance_Calm_Money_Sub4_vip3.json",
  "finance-calm-money-sub5-growing-bigger-vip3": "data/Finance_Calm_Money_Sub5_Growing_Bigger_vip3.json",
  "finance-calm-money-sub6-legacy-peace-vip3": "data/Finance_Calm_Money_Sub6_Legacy_Peace_vip3.json",
  "finance-glory-vip3": "data/Finance_Glory_vip3.json",
  "find-what-you-love-vip4": "data/Find_What_You_Love_vip4.json",
  "grief-healing-free": "data/Grief_Healing_free.json",
  "grammar-accelerator-vip1": "data/grammar_accelerator_vip1.json",
  "grammar-fluency-structure-vip2": "data/grammar_fluency_structure_vip2.json",
  "grammar-foundations-free": "data/grammar_foundations_free.json",
  "grammar-native-logic-vip3-ii": "data/grammar_native_logic_vip3_ii.json",
  "growing-bigger-when-ready-vip3-5-finance": "data/Growing_Bigger_When_Ready_vip3_5_fiance.json",
  "know-yourself-deeply-vip4": "data/Know_Yourself_Deeply_vip4.json",
  "legacy-and-impact-vip4": "data/Legacy_and_Impact_vip4.json",
  "legacy-&-long-term-peace-vip3-6-finance": "data/Legacy_&_Long_Term_Peace_vip3_6_finance.json",
  "loneliness-comfort-free": "data/Loneliness_Comfort_free.json",
  "lullabies-for-a-tired-heart-vip3": "data/Lullabies_For_A_Tired_Heart_vip3.json",
  "master-english-high-efficiency-vip1": "data/master_english_high_efficiency_vip1.json",
  "master-english-high-efficiency-vip2": "data/master_english_high_efficiency_vip2.json",
  "master-english-high-efficiency-vip3": "data/Master_English_High_Efficiency_vip3.json",
  "master-english-high-efficiency-vip3-ii": "data/master_english_high_efficiency_vip3_II.json",
  "master-english-grammar-high-roi-vip3": "data/master_english_grammar_high_roi_vip3.json",
  "meaning-of-life-free": "data/Meaning_Of_Life_free.json",
  "meaning-of-life-vip3": "data/Meaning_Of_Life_vip3.json",
  "mens-mental-health-free": "data/Mens_Mental_Health_free.json",
  "mens-mental-health-vip1": "data/Mens_Mental_Health_vip1.json",
  "mens-mental-health-vip2": "data/Mens_Mental_Health_vip2.json",
  "mens-mental-health-vip3": "data/Mens_Mental_Health_vip3.json",
  "mental-health-vip1": "data/Mental_Health_vip1.json",
  "mental-sharpness-vip3": "data/Mental_Sharpness_vip3.json",
  "mentors-who-light-the-way-vip4": "data/Mentors_Who_Light_the_Way_vip4.json",
  "mindfulness-vip1": "data/Mindfulness_vip1.json",
  "nutrition-free": "data/Nutrition_free.json",
  "nutrition-your-health-vip1": "data/Nutrition-Your_Health_vip1.json",
  "nutrition-vip2": "data/Nutrition_vip2.json",
  "nutrition-vip3": "data/Nutrition_vip3.json",
  "ocd-support-free": "data/OCD_Support_free.json",
  "obesity-management-vip3": "data/Obesity_Management_vip3.json",
  "obesity-management-ii-vip3": "data/Obesity_Management_II_vip3.json",
  "obesity-free": "data/Obesity_free.json",
  "ocd-support-vip1": "data/Ocd_Support_vip1.json",
  "ocd-support-vip2": "data/Ocd_Support_vip2.json",
  "ocd-support-vip3": "data/Ocd_Support_vip3.json",
  "overcome-storm-vip3": "data/Overcome_Storm_vip3.json",
  "philosophy-of-everyday-free": "data/Philosophy_Of_Everyday_free.json",
  "philosophy-of-everyday-vip2": "data/Philosophy_Of_Everyday_Life_vip2.json",
  "philosophy-of-everyday-vip3": "data/Philosophy_Of_Everyday_vip3.json",
  "physical-fitness-and-nutrition-free": "data/Physical_Fitness_And_Nutrition_free.json",
  "productivity-and-focus-free": "data/Productivity_And_Focus_free.json",
  "productivity-and-focus-vip1": "data/Productivity_And_Focus_vip1.json",
  "productivity-and-focus-vip2": "data/Productivity_And_Focus_vip2.json",
  "productivity-and-focus-vip3": "data/Productivity_And_Focus_vip3.json",
  "protecting-what-matters-vip3-4-finance": "data/Protecting_What_Matters_vip3_4_finance.json",
  "ptsd-support-free": "data/Ptsd_Support_free.json",
  "quiet-growth-simple-investing-vip3-3-finance": "data/Quiet_Growth_Simple Investing_vip3.3.finance.json",
  "relational-intelligence-&-erotic-communication-vip3-sub2-sex": "data/Relational_Intelligence_&_Erotic_Communication_vip3_sub2_sex.json",
  "relationship-healing-free": "data/Relationship_Healing_free.json",
  "resilience-and-adaptation-vip4": "data/Resilience_and_Adaptation_vip4.json",
  "sacred-body-curious-self-vip3-sub2-sex": "data/Sacred_Body,_Curious_Self_vip3_sub2_sex.json",
  "see-your-money-clearly-vip3-2-finance": "data/See_Your_Money_Clearly_vip3_2_finance.json",
  "self-love-free": "data/Self_Love_free.json",
  "sexuality-&-curiosity-&-culture-vip3": "data/Sexuality & Curiosity & Culture_vip3.json",
  "sexuality-&-curiosity-free": "data/Sexuality & Curiosity_free.json",
  "sexuality-curiosity-vip3-sub4": "data/sexuality_curiosity_vip3_sub4.json",
  "shadow-work-free": "data/Shadow_Work_free.json",
  "shadow-work-vip1": "data/Shadow_Work_vip1.json",
  "shadow-work-vip2": "data/Shadow_Work_vip2.json",
  "shadow-work-vip3": "data/Shadow_Work_vip3.json",
  "sleep-improvement-free": "data/Sleep_Improvement_free.json",
  "sleep-improvement-vip1": "data/Sleep_Improvement_vip1.json",
  "sleep-improvement-vip3": "data/Sleep_Improvement_vip3.json",
  "social-anxiety-free": "data/Social_Anxiety_free.json",
  "social-anxiety-vip2": "data/Social_Anxiety_vip2.json",
  "social-anxiety-vip3": "data/Social_Anxiety_vip3.json",
  "soulmate-free": "data/Soulmate_free.json",
  "stoicism-free": "data/Stoicism_free.json",
  "strategy-in-life-1-vip3": "data/Strategy_In_Life_1_vip3.json",
  "strategy-in-life-2-vip3": "data/Strategy_In_Life_2_vip3.json",
  "strategy-in-life-3-vip3": "data/Strategy_In_Life_3_vip3.json",
  "strategy-in-life-advanced-tactics-ii-vip3": "data/Strategy_in_Life_ Advanced_Tactics_II_vip3.json",
  "strategy-in-life-foundations-ii-vip3": "data/Strategy_in_Life_Foundations_II_vip3.json",
  "strategy-in-life-mastery-legacy-ii-vip3": "data/strategy_in_life_mastery_legacy_vip3_II.json",
  "stress-free": "data/Stress_free.json",
  "stress-vip2": "data/Stress_vip2.json",
  "stress-vip3": "data/Stress_vip3.json",
  "strategy-in-life-mastery-legacy-vip3": "data/Strategy_In_Life_Mastery_Legacy_vip3.json",
  "trigger-point-release-vip1": "data/Trigger_Point_Release_vip1.json",
  "wealth-&-wisdom-vip3-preview-free": "data/Wealth_&_Wisdom_VIP3_Preview_free.json",
  "weight-loss-and-fitness-free": "data/Weight_Loss_And_Fitness_free.json",
  "weight-loss-and-fitness-vip2": "data/Weight_Loss_And_Fitness_vip2.json",
  "weight-loss-and-fitness-vip3": "data/Weight_Loss_And_Fitness_vip3.json",
  "confidence-vip3": "data/confidence_vip3.json",
  "sleep-improvement-vip2": "data/sleep_Improvement_vip2.json",
  "english-writing-structure-vip2": "data/English-Writing-Structure-vip2.json",
  "english-writing-mastery-vip3": "data/English-Writing-Mastery-vip3.json",
  "english-writing-deep-dive-vip3-ii": "data/english-writing-deep-dive-vip3II.json",
  "english-writing-deep-dive-vip3-ii-ii": "data/english-writing-deep-dive-vip3II-II.json",
  "english-writing-deep-dive-vip3-ii-iii": "data/english-writing-deep-dive-vip3II-III.json",
  "english-writing-deepdive-part4-vip3-ii": "data/english-writing-deepdive-part4-vip3-ii.json",
  "english-writing-deepdive-part5-vip3-ii": "data/english-writing-deepdive-part5-vip3-ii.json",
  "english-writing-deepdive-part6-vip3-ii": "data/english-writing-deepdive-part6-vip3-ii.json",
  "english-writing-deepdive-part7-vip3-ii": "data/english-writing-deepdive-part7-vip3-ii.json",
  "english-writing-deepdive-part8-vip3-ii": "data/english-writing-deepdive-part8-vip3-ii.json",
  "english-writing-deepdive-part9-vip3-ii": "data/english-writing-deepdive-part9-vip3-ii.json",
  "english-writing-deepdive-part10-vip3-ii": "data/english-writing-deepdive-part10-vip3-ii.json",
  "finding-gods-peace-free": "data/Finding_Gods_Peace_free.json",
  "build-skills-vip4-career-3": "data/Build_Skills_vip4_career_3.json",
  "build-skills-vip4-career-3-ii": "data/Build_Skills_vip4_career_3_II.json",
  "discover-self-vip4-career-1": "data/Discover_Self_vip4_career_1.json",
  "explore-world-vip4-career-i-2": "data/Explore_World_vip4_career_I_2.json",
  "explore-world-vip4-career-ii-2": "data/Explore_World_vip4_career_II_2.json",
  "god-with-us-free": "data/Universe_With_Us_free.json",
  "grow-wealth-vip4-career-6": "data/Grow_Wealth_vip4_career_6.json",
  "launch-career-vip4-career-4-ii": "data/Launch_Career_vip4_career_4_II.json",
  "strategy-in-life-advanced-tactics-ii-alt-vip3": "data/Strategy_in_Life_Advanced_Tactics_II_vip3.json",
  "schizophrenia-support-free": "data/schizophrenia-support-free.json",
  "schizophrenia-understanding-vip1": "data/schizophrenia-understanding-vip1.json",
  "schizophrenia-understanding-vip2": "data/schizophrenia-understanding-vip2.json",
  "schizophrenia-mastery-vip3": "data/schizophrenia-mastery-vip3.json",
  "writing-logic-vip3-ii": "data/writing-logic-vip3ii.json",
  "writing-deepdive-part12-vip3-ii": "data/writing-deepdive-part12-vip3-ii.json",
  "writing-vip3ii-part14": "data/writing-vip3ii-part14.json",
  "individual-strategic-mastery-vip9": "data/Individual_Strategic_Mastery_vip9.json",
  "vip6-shadow-self": "data/vip6_shadow_self.json",
  "vip6-projection-triggers": "data/vip6_projection_triggers.json",
  "vip6-emotional-blindspots": "data/vip6_emotional_blindspots.json",
  "vip6-inner-fragmentation": "data/vip6_inner_fragmentation.json",
  "vip6-repressed-desires": "data/vip6_repressed_desires.json",
  "vip6-fear-intimacy": "data/vip6_fear_intimacy.json",
  "vip6-fear-abandonment": "data/vip6_fear_abandonment.json",
  "vip6-suppressed-rage": "data/vip6_suppressed_rage.json",
  "vip6-self-sabotage-relationship": "data/vip6_self_sabotage_relationship.json",
  "vip6-avoidant-behaviors": "data/vip6_avoidant_behaviors.json",
  "vip6-emotional-dependency": "data/vip6_emotional_dependency.json",
  "vip6-fear-failure": "data/vip6_fear_failure.json",
  "vip6-perfectionism-shadows": "data/vip6_perfectionism_shadows.json",
  "vip6-people-pleasing": "data/vip6_people_pleasing.json",
  "vip6-martyr-complex": "data/vip6_martyr_complex.json",
  "vip6-pride-ego-shadow": "data/vip6_pride_ego_shadow.json",
  "vip6-abandonment-wound": "data/vip6_abandonment_wound.json",
  "vip6-attachment-trauma": "data/vip6_attachment_trauma.json",
  "vip6-attachment-wounds": "data/vip6_attachment_wounds.json",
  "vip6-control-patterns": "data/vip6_control_patterns.json",
  "vip6-fear-success": "data/vip6_fear_success.json",
  "vip6-freeze-response": "data/vip6_freeze_response.json",
  "vip6-guilt-hidden-shame": "data/vip6_guilt_hidden_shame.json",
  "vip6-imposter-wounds": "data/vip6_imposter_wounds.json",
  "vip6-inner-saboteur": "data/vip6_inner_saboteur.json",
  "vip6-not-enough-wound": "data/vip6_not_enough_wound.json",
  "vip6-reclaim-power": "data/vip6_reclaim_power.json",
  "vip6-self-worth-distortions": "data/vip6_self_worth_distortions.json",
  "vip6-shadow-envy": "data/vip6_shadow_envy.json",
  "vip6-shadow-in-love": "data/vip6_shadow_in_love.json",
  "vip6-shadow-perfectionism": "data/vip6_shadow_perfectionism.json",
  "strategic_foundations_vip9": "data/strategic_foundations_vip9.json",
  "leverage-thinking-vip9": "data/strategic_leverage_thinking_vip9.json",
  "strategic-long-horizon-vip9": "data/strategic_long_horizon_vip9.json",
  "strategic-deep-work-vip9": "data/strategic_deep_work_vip9.json",
  "strategic-risk-calibration-vip9": "data/strategic_risk_calibration_vip9.json",
  "strategic-self-governance-vip9": "data/strategic_self_governance_vip9.json",
  "strategic-signal-detection-vip9": "data/strategic_signal_detection_vip9.json",
  "strategic_reputation_individual_vip9": "data/strategic_reputation_individual_vip9.json",
  "strategic_negotiation_individual_vip9": "data/strategic_negotiation_individual_vip9.json",
  "strategic_influence_individual_vip9": "data/strategic_influence_individual_vip9.json",
  "strategic_adaptive_intelligence_vip9": "data/strategic_adaptive_intelligence_vip9.json",
  "strategic_cognitive_models_vip9": "data/strategic_cognitive_models_vip9.json",
  "strategic_compounding_learning_vip9": "data/strategic_compounding_learning_vip9.json",
  "strategic_failure_recovery_vip9": "data/strategic_failure_recovery_vip9.json",
  "strategic_lifelong_fitness_vip9": "data/strategic_lifelong_fitness_vip9.json",
  "strategic_antifragile_growth_vip9": "data/strategic_antifragile_growth_vip9.json",
  "corporate_foundations_vip9": "data/corporate_foundations_vip9.json",
  "corporate_org_architecture_vip9": "data/corporate_org_architecture_vip9.json",
  "corporate_resilience_vip9": "data/corporate_resilience_vip9.json",
  "corporate_resource_allocation_vip9": "data/corporate_resource_allocation_vip9.json",
  "corporate_innovation_engines_vip9": "data/corporate_innovation_engines_vip9.json",
  "corporate_execution_systems_vip9": "data/corporate_execution_systems_vip9.json",
  "corporate_cross_functional_vip9": "data/corporate_cross_functional_vip9.json",
  "corporate_crisis_strategy_vip9": "data/corporate_crisis_strategy_vip9.json",
  "corporate_foresight_vip9": "data/corporate_foresight_vip9.json",
  "corporate_analytics_strategy_vip9": "data/corporate_analytics_strategy_vip9.json",
  "corporate_conflict_navigation_vip9": "data/corporate_conflict_navigation_vip9.json",
  "corporate_global_strategy_vip9": "data/corporate_global_strategy_vip9.json",
  "corporate_governance_vip9": "data/corporate_governance_vip9.json",
  "corporate_leadership_leverage_vip9": "data/corporate_leadership_leverage_vip9.json",
  "corporate_long_cycle_planning_vip9": "data/corporate_long_cycle_planning_vip9.json",
  "corporate_negotiation_vip9": "data/corporate_negotiation_vip9.json",
  "corporate_positioning_vip9": "data/corporate_positioning_vip9.json",
  "corporate_scaling_strategy_vip9": "data/corporate_scaling_strategy_vip9.json",
  "corporate_talent_architecture_vip9": "data/corporate_talent_architecture_vip9.json",
  "corporate_team_culture_vip9": "data/corporate_team_culture_vip9.json",
  "national_culture_power_vip9": "data/national_culture_power_vip9.json",
  "national_military_strategy_vip9": "data/national_military_strategy_vip9.json",
  "national_geopolitical_mapping_vip9": "data/national_geopolitical_mapping_vip9.json",
  "national_risk_intelligence_vip9": "data/national_risk_intelligence_vip9.json",
  "national_diplomacy_vip9": "data/national_diplomacy_vip9.json",
  "national_identity_strategy_vip9": "data/national_identity_strategy_vip9.json",
  "national_resilience_systems_vip9": "data/national_resilience_systems_vip9.json",
  "national_cyber_strategy_vip9": "data/national_cyber_strategy_vip9.json",
  "national_space_strategy_vip9": "data/national_space_strategy_vip9.json",
  "national_megatrends_vip9": "data/national_megatrends_vip9.json",
  "national_civilization_cycles_vip9": "data/national_civilization_cycles_vip9.json",
  "national_economic_strategy_vip9": "data/national_economic_strategy_vip9.json",
  "national_energy_security_vip9": "data/national_energy_security_vip9.json",
  "national_power_capability_vip9": "data/national_power_capability_vip9.json"
};

/**
 * Get all unique room base names (without tier suffix)
 */
export function getRoomBaseNames(): string[] {
  const baseNames = new Set<string>();
  
  for (const roomId of Object.keys(PUBLIC_ROOM_MANIFEST)) {
    const baseName = roomId.replace(/-(free|vip1|vip2|vip3|vip3-ii|vip4|vip5|vip6|vip9)$/, '');
    baseNames.add(baseName);
  }
  
  return Array.from(baseNames).sort();
}

/**
 * Get all tiers available for a room base name
 */
export function getAvailableTiers(roomBaseName: string): string[] {
  const tiers: string[] = [];
  
  for (const tier of ['free', 'vip1', 'vip2', 'vip3', 'vip3_ii', 'vip4', 'vip5', 'vip6', 'vip9']) {
    const roomId = `${roomBaseName}-${tier.replace('_', '-')}`;
    if (PUBLIC_ROOM_MANIFEST[roomId]) {
      tiers.push(tier);
    }
  }
  
  return tiers;
}

