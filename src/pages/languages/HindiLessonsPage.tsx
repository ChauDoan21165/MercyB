// src/pages/languages/HindiLessonsPage.tsx - /languages/hindi
//
// Public Hindi lesson page. W3 A2 renders the validated local arrays from
// src/languages/hindi only. Route, hub, and theme wiring belong to W3 A4.
...
import {
  ...
  type HindiCategoryMeta,
  type HindiLesson,
  type HindiLevel,
} from "@/languages/hindi";
...
const HINDI_ACCENT = "#B45309";

const HERO_VI =
  "Tiếng Hindi cho người Việt - từ chữ Devanagari đến văn phong học thuật.";
const HERO_EN =
  "Hindi - local lessons from Devanagari basics to advanced register.";

const DEVANAGARI_RE = /[\u0900-\u097F]/;

export default function HindiLessonsPage() {
  const [level, setLevel] = useState<HindiLevel>("A1");
  const [uiLang] = useLessonUiLang();

  const lessons = HINDI_LESSONS_BY_LEVEL[level] ?? [];

  const lessonsByCategory = useMemo(() => {
    const map = new Map<string, HindiLesson[]>();
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-6">
      <header
        ...
        style={{
          borderColor: `${HINDI_ACCENT}33`,
          background:
            "linear-gradient(135deg, rgb(255 251 235), rgb(254 243 199), rgb(248 250 252))",
        }}
      >
        <p
          className="text-xs font-semibold uppercase tracking-wider"
          style={{ color: HINDI_ACCENT }}
        >
          Tiếng Hindi · Hindi
        </p>
        ...
        {HINDI_VALIDATED_LEVELS.map((lv) => {
          ...
          return (
            <button
              ...
              style={active ? { backgroundColor: HINDI_ACCENT } : undefined}
              aria-pressed={active}
            >
...
function orderedCategorySections(
  lessonsByCategory: Map<string, HindiLesson[]>,
): HindiCategoryMeta[] {
  const categories: HindiCategoryMeta[] = [];
  const seen = new Set<string>();

  for (const category of HINDI_CATEGORIES) {
...
function HindiLessonCard({
  ...
  return (
    <article
      ...
      style={{ borderColor: `${HINDI_ACCENT}22` }}
    >
      <button
        type="button"
      ...
      >
        <span
          ...
          style={{ backgroundColor: HINDI_ACCENT }}
        >
          {lessonNumber}
        </span>
      ...
      {open && (
        <div className="space-y-4 border-t border-amber-100 bg-slate-50/70 px-4 py-4">
...
function SentenceSection({
  ...
  sentences: HindiLesson["sentences"];
  uiLanguage: LessonUiLang;
}) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-3">
      ...
      <div className="space-y-3">
        {sentences.map((sentence, index) => (
          <div key={`${sentence.hi}-${index}`} className="space-y-1">
            <HindiText className="text-base font-semibold">
              {sentence.hi}
            </HindiText>
...
function VocabularySection({
  ...
  vocabulary: NonNullable<HindiLesson["vocabulary"]>;
  uiLanguage: LessonUiLang;
}) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-3">
...
function DialogueSection({
  ...
  dialogue: NonNullable<HindiLesson["dialogue"]>;
  uiLanguage: LessonUiLang;
}) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-3">
...
function ExerciseSection({
  ...
  exercises: NonNullable<HindiLesson["exercises"]>;
  uiLanguage: LessonUiLang;
}) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-3">
          ...
          <div key={index} className="rounded-lg border border-slate-100 bg-slate-50 p-2">
              ...
              <>
                ...
                <div className="mt-2 grid gap-2 sm:grid-cols-2">
                  {exercise.pairs.map((pair, pairIndex) => (
                    <div
                      key={`${pair.hi}-${pairIndex}`}
                      className="rounded border border-slate-200 bg-white p-2"
                    >
                      <HindiText className="text-sm font-semibold">
                        {pair.hi}
                      </HindiText>
            ...
            {exercise.type === "translation" && (
              <>
                ...
                <AnswerLine answer={exercise.hi} romanization={exercise.romanization} />
              </>
            )}
          </div>
        ))}
...
function NotesSection({
  ...
  const cultural =
    uiLanguage === "en" ? lesson.cultural_notes_en : lesson.cultural_notes_vi;
  const tip = uiLanguage === "en" ? lesson.tip_advice_en : lesson.tip_advice_vi;
  const register =
    uiLanguage === "en" ? lesson.register_notes_en : lesson.register_notes_vi;
...
function HindiText({
  ...
  return (
    <p className={`leading-relaxed ${className}`} lang="hi">
      {children}
    </p>
  );
}

function MaybeHindiText({
  children,
  className = "",
}: {
  children: string;
