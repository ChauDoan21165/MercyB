...
import * as PageModule from "../ChineseLessonsPage";
import * as LanguageContent from "../../../languages/chinese";
import { UiLanguageProvider } from "../../../contexts/UiLanguageContext";

const repoRoot = resolve(process.cwd());
const pageSource = readFileSync(resolve(repoRoot, "src/pages/languages/ChineseLessonsPage.tsx"), "utf8");
...
const ChineseLessonsPage = (
  (PageModule as { default?: ComponentType; ChineseLessonsPage?: ComponentType }).default ??
  (PageModule as { default?: ComponentType; ChineseLessonsPage?: ComponentType }).ChineseLessonsPage
) as ComponentType;

const identityLabels = ["Chinese", "Mandarin", "中文", "汉语"];
...
describe("ChineseLessonsPage", () => {
  it("renders a public page backed by local Chinese lesson arrays", () => {
    expect(localLessonTextCandidates.length).toBeGreaterThan(0);
    expect(pageSource.toLowerCase()).toContain("languages/chinese");
  ...
  it("is registered as a public language route", () => {
    expect(routerSource).toContain("ChineseLessonsPage");
    expect(routerSource).toContain('path="/languages/chinese"');
    expect(routerSource).toMatch(/<ChineseLessonsPage\s*\/>/);
  });

  it("has a hub card/link on the languages index", () => {
    expect(languagesIndexSource).toContain("Chinese");
    expect(languagesIndexSource).toContain('slug: "chinese"');
    expect(languagesIndexSource).toContain('href: "/languages/chinese"');
  });
