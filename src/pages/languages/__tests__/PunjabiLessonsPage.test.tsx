...
describe('Punjabi lessons page coverage', () => {
  it('uses local Punjabi lesson content without Supabase loader or fake media promises', () => {
    ...
    expect(source).toMatch(/punjabi/i);
  ...
  it('is wired through app routing', () => {
    const matches = sourceFiles()
      ...
      .filter((file) => {
        ...
        return /PunjabiLessonsPage/.test(source) && /punjabi/i.test(source);
      });

    expect(matches, 'Expected app routing to reference PunjabiLessonsPage and the punjabi slug').not.toHaveLength(0);
  });
  ...
  it('is discoverable from the languages hub', () => {
    const matches = sourceFiles()
      ...
      .filter((file) => {
        const source = readFileSync(file, 'utf8');
        return /Punjabi/.test(source) && /punjabi/i.test(source);
      });

    expect(matches, 'Expected LanguagesIndexPage to expose the Punjabi page/card/link').not.toHaveLength(0);
  });
});
