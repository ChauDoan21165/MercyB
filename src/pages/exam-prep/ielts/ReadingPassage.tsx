...
import { getIeltsReadingPassageById, type IELTSReadingQuestion } from '@/data/exam-prep/ielts/reading-passages';
...
function QuestionBlock({ q, showKey, userAnswer, onChange, feedbackVisible }: {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3">
      ...
      {showKey && q.explanation_vi && (
        <p className="mt-0.5 text-xs italic text-slate-600">{q.explanation_vi}</p>
      )}
      {feedbackVisible && (
...
export default function ReadingPassage() {
  ...
  const passage = useMemo(() => passageId ? getIeltsReadingPassageById(passageId) : null, [passageId]);
  ...
  if (!passage) {
    return (
      <div className="mx-auto w-full max-w-3xl px-4 py-6 text-center">
        <p className="text-slate-600">Passage not found.</p>
        <Link to="/exam-prep/ielts/reading" className="text-sm text-amber-700 underline">Back to Reading list</Link>
      </div>
    );
  }

  const correctCount = passage.questions.reduce((acc, q) => {
    return normalize(answers[q.number] ?? "") === normalize(q.correct_answer) ? acc + 1 : acc;
  }, 0);
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-6">
      <header className="mb-4">
        <Link to="/exam-prep/ielts/reading" className="inline-flex items-center gap-1 text-xs text-slate-600 hover:text-slate-800">
          <ChevronLeft size={14} />
          Quay lại danh sách bài đọc
        </Link>
        <h1 className="mt-2 text-xl font-bold text-slate-900">{passage.title_vi}</h1>
        <p className="text-sm text-slate-600">{passage.title_en}</p>
        <div className="mt-1 flex items-center gap-2 text-xs text-slate-600">
          <span>Band {passage.band}</span>
          <span>·</span>
          <span>{passage.time_minutes} min</span>
          <span>·</span>
      ...
      <div className="rounded-xl border border-primary/15 bg-white/80 p-4 mb-4">
        <p className="text-xs font-semibold text-slate-600 mb-1">{passage.summary_vi}</p>
      ...
      <div className="space-y-3">
        {passage.questions.map((q) => (
          <QuestionBlock
            ...
            feedbackVisible={checked}
          />
        ))}
      </div>

      <div className="mt-4 flex items-center justify-between">
        <Button type="button" variant={revealed ? "ghost" : "default"} onClick={handleCheck}>
