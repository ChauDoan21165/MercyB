  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-6">
      ...
      <PremiumGate>
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2"><span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Band:</span>{(["all", 5.5, 6.5, 7.5, 8.5] as const).map((b) => (<button key={b} type="button" onClick={() => setDiffFilter(b)} className={"rounded-full border px-3 py-0.5 text-[11px] font-semibold " + (diffFilter === b ? "border-amber-600 bg-amber-600 text-white" : "border-slate-300 bg-white text-slate-700 hover:bg-slate-50")}>{b === "all" ? "tất cả" : b.toFixed(1)}</button>))}</div>
        <div className="text-xs text-slate-500 mb-3">Hiển thị {visibleItems.length} / {IELTS_LISTENING_ITEMS.length} bài.</div>
