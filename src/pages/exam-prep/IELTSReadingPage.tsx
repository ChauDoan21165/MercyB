...
function PassageCard({ passage }: { passage: IELTSReadingPassage }) {
  return (
    <Link to={"/exam-prep/ielts/reading/" + passage.id} className="block rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-slate-300 hover:shadow">
      <div className="mb-1 flex items-center gap-2"><span className={"inline-block rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider " + (BAND_BADGE[passage.band] ?? "bg-slate-100 text-slate-700 border-slate-200")}>Band {passage.band}</span><span className="text-[11px] font-bold uppercase tracking-wider text-slate-600">{passage.topic_family.replace(/_/g," ")}</span><span className="ml-auto text-[11px] font-semibold text-slate-600">{passage.time_minutes} min · {passage.questions.length} Q</span></div>
      <h3 className="text-base font-bold text-slate-900">{passage.title_vi}</h3><p className="text-xs text-slate-600">{passage.title_en}</p><p className="mt-1 text-xs leading-relaxed text-slate-600 line-clamp-2">{passage.summary_vi}</p>
    </Link>
  );
}
