...
function QuestionBlock({ q, showKey, userAnswer, onChange, feedbackVisible }: {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3">
      ...
      {showKey && <p className="mt-1 text-xs text-slate-600"><span className="font-semibold">Đáp án:</span> {q.correct_answer}</p>}
      {showKey && q.explanation_vi && <p className="mt-0.5 text-xs italic text-slate-600">{q.explanation_vi}</p>}
      {feedbackVisible && (
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-6">
      <header className="mb-4">
        <Link to="/exam-prep/toefl/reading" className="inline-flex items-center gap-1 text-xs text-slate-600 hover:text-slate-800">
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        {checked && <p className="text-xs text-slate-600">{correctCount} / {passage.questions.length} đúng</p>}
      </div>
    </div>
  );
}
