...
export function simulateReducedMotion(): void {
  ...
  // Preserve original matchMedia once
  if (!originalMatchMedia) originalMatchMedia = window.matchMedia.bind(window);

  // Mock matchMedia for prefers-reduced-motion
  window.matchMedia = ((query: string): MediaQueryList => {
    if (query.includes("prefers-reduced-motion")) {
      // Provide full MediaQueryList surface to satisfy TS + older browsers
      const mql: MediaQueryList = {
        ...
        // legacy (still present in lib.dom)
        addListener: () => {},
        removeListener: () => {},
      };
      return mql;
    ...
    return (originalMatchMedia as typeof window.matchMedia)(query);
...
export function resetBrowserEnv(): void {
  ...
  // Restore matchMedia if we overrode it
