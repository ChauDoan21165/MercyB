...
G. Modern paper, of course, is no longer made from rags but from wood pulp, an industrial process developed in the nineteenth century after timber-rich North America and northern Europe sought a cheaper, faster fibre source. Yet many of the basic steps would still be recognisable to Cai Lun: fibres are macerated in water, drained on a mesh, pressed and dried into sheets. The most striking thing about paper's history is how slowly the technology matured before the printing press, and how quickly it then transformed information access. The intervening eighteen centuries were not a story of ceaseless progress but of patient transmission across cultures that, in many cases, did not directly know one another.`,
    paragraph_count: 7,
    questions: [
...
F. The sector has nonetheless continued to evolve. Many MFIs now offer a wider range of services beyond credit, including savings accounts, low-cost remittances, and crop-failure insurance for smallholder farmers. Mobile-phone banking — particularly the M-Pesa service launched in Kenya in 2007 — has reduced the cost of reaching remote customers, and digital identity systems have made it easier to verify borrowers without paper records. The economic logic that originally justified group lending has accordingly weakened, and many newer MFIs lend to individuals.

G. Modern microfinance therefore looks rather different from the model of the 1980s. The most defensible account treats it not as a self-sufficient escape from poverty but as one piece of a broader rural financial infrastructure. The frontier today lies in linking small-scale lending and savings with public services — health, education, basic income transfers — rather than in expanding microcredit alone. Empirical evaluations have also moved on, asking which households benefit from which products under which local conditions, rather than treating "microfinance" as a single intervention that succeeds or fails. The most useful question is no longer whether microfinance "works" but where, for whom, and combined with what.`,
    paragraph_count: 7,
    questions: [
