// FILE: src/__tests__/roomMetrics.test.ts
//
// BUILD-SAFE FIXES:
// - TS7053: index into by_tier/by_domain with string keys => type maps as Record<string, ...>
// - TS2304: was using `domainGroups` inside "domainEntries" test but it wasn't in scope
//   => use `domainEntries` for accumulation (bugfix).

import { describe, it, expect } from "vitest";

type TierSummary = { rooms: number; entries: number; approx_words_en: number; approx_words_vi: number };
type DomainSummary = { rooms: number; entries: number; approx_words_en: number; approx_words_vi: number };
...
type RoomMetrics = {
  ...
  summary: {
    ...
    by_domain: Record<string, DomainSummary>;
    total_rooms: number;
...
describe("Room Metrics", () => {
  describe("Summary Validation", () => {
    ...
    it("by_tier room counts sum to total_rooms", () => {
      const tierSum = Object.values(mockMetrics.summary.by_tier).reduce((sum, tier) => sum + tier.rooms, 0);
      expect(tierSum).toBe(mockMetrics.summary.total_rooms);
    });

    it("by_domain room counts sum to total_rooms", () => {
      const domainSum = Object.values(mockMetrics.summary.by_domain).reduce((sum, domain) => sum + domain.rooms, 0);
      expect(domainSum).toBe(mockMetrics.summary.total_rooms);
    });

    it("by_tier entry counts sum to total_entries", () => {
      const entrySum = Object.values(mockMetrics.summary.by_tier).reduce((sum, tier) => sum + tier.entries, 0);
      expect(entrySum).toBe(mockMetrics.summary.total_entries);
    });

    it("by_domain entry counts sum to total_entries", () => {
      const entrySum = Object.values(mockMetrics.summary.by_domain).reduce((sum, domain) => sum + domain.entries, 0);
      expect(entrySum).toBe(mockMetrics.summary.total_entries);
    });

    it("by_tier word counts sum to total_words", () => {
      const wordsEnSum = Object.values(mockMetrics.summary.by_tier).reduce((sum, tier) => sum + tier.approx_words_en, 0);
      expect(wordsEnSum).toBe(mockMetrics.summary.total_words_en);

      const wordsViSum = Object.values(mockMetrics.summary.by_tier).reduce((sum, tier) => sum + tier.approx_words_vi, 0);
  ...
  describe("Entry Count Validation", () => {
    ...
    it("individual room entry counts sum correctly per domain", () => {
      ...
      for (const room of mockMetrics.rooms) {
        // ✅ FIX: use domainEntries (domainGroups was out-of-scope and wrong)
