// src/__tests__/jsonParsing.test.tsx
...
describe("JSON Parsing Integration", () => {
  ...
  describe("Room Registry Integration", () => {
    it("should validate room registry entry format (supports public + private)", () => {
      ...
      const privateRegistryEntry = {
        id: "adult-room",
        nameEn: "Adult Room",
        nameVi: "Phòng 18+",
        tier: "vip3",
        path: "private:vip3/sacred_body_curious_self_vip3_sub2_sex.json",
      };
    ...
    it("should handle multiple tier variations (public + private path support)", () => {
      ...
      tiers.forEach((tier) => {
        ...
        expect(tiers).toContain(publicRegistryEntry.tier);

        const privateRegistryEntry = {
          id: `adult-room-${tier}`,
          tier,
          path: `private:${tier}/test-room-${tier}.json`,
        };

        expect(tiers).toContain(privateRegistryEntry.tier);
        expect(privateRegistryEntry.path.startsWith("private:")).toBe(true);
      });
    });
  });
