...
// MB-BLUE-101.5b → MB-BLUE-101.5c-pricing-route — 2026-01-21 (+0700)
//
...
//
// Tier routes (UPDATED):
// - ✅ /tiers            = PRICING page (Free, VIP1, VIP3, VIP9) with Pay buttons
// - ✅ /tier-map         = Tier Map (spine visual)
// - ✅ /tier-map/:tierId = Tier details
//
// ADMIN (LOCKED):
// - Admin routes nested under "/admin/*" with index dashboard.
// - AdminRoute + AdminLayout appear ONCE in AdminShell.
...
// ✅ PRICING page (Pay buttons)
import Tiers from "@/pages/Tiers";

// ✅ Tier Map pages (spine visual + details)
...
/**
 * FIX (101.5c):
 * Legacy tier details route:
 * /tiers/:tierId → /tier-map/:tierId
 * (prevents Pricing "Details →" from 404 if it still links to /tiers/vip1)
 */
function LegacyTierDetailsRedirect() {
  const { tierId } = useParams<{ tierId: string }>();
  return <Navigate to={tierId ? `/tier-map/${tierId}` : "/tier-map"} replace />;
}
  ...
  return (
    <>
      <Routes>
        ...
        {/* ✅ PRICING (Pay buttons live here) */}
        <Route path="/tiers" element={<Tiers />} />

        {/* ✅ Legacy: pricing "Details" links */}
        <Route path="/tiers/:tierId" element={<LegacyTierDetailsRedirect />} />

        {/* ✅ Tier Map moved off /tiers */}
        <Route path="/tier-map" element={<TierIndex />} />
        <Route path="/tier-map/:tierId" element={<TierDetail />} />
...
/* teacher GPT — new thing to learn:
   If your UI “doesn’t change,” it’s often routing: you edited Page A, but the router still renders Page B.
   Always verify the active route mapping (path → component) before polishing the page UI. */
