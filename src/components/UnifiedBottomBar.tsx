...
import { Settings, ZoomIn, ZoomOut, Maximize2, Copy, Circle } from "lucide-react";
...
export const UnifiedBottomBar = () => {
  ...
  return (
    <div className="fixed left-4 right-4 bottom-6 md:bottom-8 h-[50px] z-30 flex gap-2">
      {/* LEFT HALF: Controls */}
      <div className="w-1/2 h-full bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border-2 border-black dark:border-gray-700 rounded-xl shadow-lg px-3 flex items-center gap-2">
        {/* Zoom Controls */}
        <Button
        ...
        <div className="flex items-center gap-1">
          <Circle className="h-3 w-3 fill-blue-500 text-blue-500" title="Online" />
          <Circle className="h-3 w-3 fill-red-500 text-red-500" title="Alerts" />
        </div>
        ...
        {/* Version Dot */}
        <div 
          className="h-5 w-5 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center text-white text-[10px] font-bold shadow-sm shrink-0"
          title="App Version"
        >
          {versionIndicator}
        </div>
        ...
        {showAdminControls && (
          <Button
            ...
            size="sm"
            variant="outline"
            className="rounded-full shadow-sm h-7 w-7 p-0 bg-gray-800 hover:bg-gray-700 border-gray-600 relative shrink-0"
