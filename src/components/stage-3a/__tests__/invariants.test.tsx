/**
 * Stage 3A — LocalWeaknessMap invariant guards. **Tier 1 only.**
 *
 ...
 *      copy").
 *
 * These six describes are the **must-never-leak** surface from the
 * !25 invariant gap audit's Tier 1 ranking (ranks 1–6). They are the
 * smallest set that catches a regression which would silently send
 * user-content data off-device, write back into the read-side store,
 * or surface shame copy.
 *
 * ─── Adding a new Tier 1 guard ─────────────────────────────────────
 *
 * Tier 1 = leak-class regressions: anything that would silently send
 * a user signal off-device, mutate a read-only store, or render copy
 * that breaks the "no shame, no gamification" principle. Add it here.
 *
 * Tier 2 (degraded contract) and Tier 3 (visible-but-recoverable bug)
 * guards live in `invariants.tier23.test.tsx`. If you're not sure
 * which tier your guard is, the rule of thumb is:
 *
 *   - Would a learner be harmed if this silently failed? → Tier 1.
 *   - Would the surface still work but break a stated contract? → Tier 2.
 *   - Would a learner see something weird but not be harmed? → Tier 3.
 *
 * Two implementation modes used here:
 *   - Runtime spies — for the supabase / fetch / setItem axes. We
 *     render `LocalWeaknessMap` with seeded fixture data and assert
 *     no forbidden call fires during mount + render.
 *   - Source-text greps — for the `mercy_user_facts` import + the
 *     gamification regex. Greps run over the component's source file
 *     and its in-tree taxonomy (the only string source the component
 *     pulls copy from at the time of writing).
 */
