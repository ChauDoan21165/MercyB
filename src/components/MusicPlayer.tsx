/**
 * MusicPlayer.tsx — v83.2
 * Generated: 2025-12-22 (+0700)
 * Reporter: teacher GPT
 *
 * NOTE (LOCKED): Append-only mindset. Do not delete small details.
 */

import { useState, useEffect, useRef } from 'react';
import { Volume2, Heart, Music, Shuffle } from 'lucide-react';
  ...
  return (
    <div className="h-full w-full bg-card border border-border rounded-lg shadow-sm">
      <div className="h-full px-3 flex items-center gap-2">
        {/* Play/Pause Button (Talking Face Motif) */}
        <TalkingFaceButton isPlaying={isPlaying} onClick={togglePlay} />
        ...
        <Button
          ...
          className={`h-8 w-8 p-0 transition-colors ${
            isShuffle ? 'bg-amber-400 text-amber-950 hover:bg-amber-500 border-2 border-amber-500' : ''
          }`}
        ...
        >
          <Heart
            className={`h-3 w-3 ${
              isFavorite(currentTrackId) ? 'fill-red-500 text-red-500' : 'text-muted-foreground'
            }`}
          />
        </Button>
        ...
        <div className="flex gap-1">
          <Button
            ...
            className={`h-8 px-2 transition-colors text-xs ${
              mode === 'common' ? 'bg-blue-500 text-white hover:bg-blue-600 border-blue-500 font-bold' : ''
            }`}
          ...
          <Button
            ...
            size="sm"
            className={`h-8 px-2 transition-colors text-xs ${
              mode === 'mercy' ? 'bg-purple-500 text-white hover:bg-purple-600 border-purple-500 font-bold' : ''
            }`}
          ...
          <Button
            ...
            size="sm"
            className={`h-8 px-2 transition-colors text-xs ${
              mode === 'favorites' ? 'bg-pink-500 text-white hover:bg-pink-600 border-pink-500 font-bold' : ''
            }`}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="w-[180px] h-8 text-xs justify-between">
          ...
          >
            <DropdownMenuLabel className="text-xs font-semibold sticky top-0 bg-popover">
              {mode === 'mercy' ? 'MB Songs' : mode === 'favorites' ? 'My Favorites' : 'Common Tracks'} (
              ...
              displayTracks.map((track) => (
                ...
                >
                  <div className="flex items-center justify-between w-full gap-2">
                    ...
                    {isFavorite(track.id) && <Heart className="h-3 w-3 fill-red-500 text-red-500 shrink-0" />}
                    {currentTrackId === track.id && <span className="text-primary shrink-0">▶</span>}
                  </div>
                </DropdownMenuItem>
              ))
            )}
        ...
        <audio ref={audioRef} src={currentTrack.url} preload="none" onEnded={playNextTrack} />
      </div>
    </div>
  );
};