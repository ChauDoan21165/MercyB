...
- One **browser** Supabase client — singleton at `src/lib/supabaseClient.ts` (anon key, the only client that ships to the browser bundle). Server-side service-role clients live in the serverless functions under `api/*` and the Supabase edge functions under `supabase/functions/*` — server-only, never bundled. (There is **no HTML SSR** in this app: `npm run build` is a plain `vite build` SPA and the host rewrites everything to a static `index.html`. The old `src/server/host/*` "SSR renderer" was dead scaffold superseded by the `guide-assistant` edge function and was deleted in `chore/delete-ssr-host-dead-code`.)
- **Post-2026-05-27 migration:** production hosting is **Cloudflare Pages** (web at `mercyblade.com`), deployed AUTOMATICALLY via git integration on push to `main` — DO NOT run `wrangler pages deploy` manually (it races the auto-deploy). Env vars/flags live in Cloudflare Pages → Settings → Variables and secrets; **GitLab** (`gitlab.com:cd12536/mercyB`) is the canonical repo. `vercel.json` still ships and the Vercel project is preserved as the **recovery host** per `docs/runbooks/disaster-recovery.md` §2.2 — Vercel is NOT primary anymore. `.github/workflows/*.yml` are **legacy** (the GitHub Actions runner doesn't fire on GitLab); use them as reference, not as current pipeline. `.gitlab-ci.yml` carries the only scheduled CI job today (nightly Postgres backup). The Stripe webhook is a **Supabase edge function** at `supabase/functions/stripe-webhook/` (registered in `supabase/config.toml` `[functions.stripe-webhook]` with `verify_jwt = false`); the live URL is `https://buemdfxyhxunzpgdoqin.supabase.co/functions/v1/stripe-webhook` — Stripe posts directly to Supabase, not via Netlify/Vercel/Cloudflare. The serverless `api/*` directory at the repo root holds Vercel-style functions for `mercy/grammar`, `mercy-ai`, `mercy-feedback`, `mercy-guide`, and `tts` only (per `vercel.json`); there is no `api/stripe-webhook`. Note: the former stray `upabase/functions/stripe-webhook.ts` root-level typo path was dead code and has been removed; do not recreate it.
