/**
 * scripts/rekey-korean-c2-audio.ts
 *
 * One-shot Supabase Storage re-key for Korean C2 lesson audio, paired with
 * the C2 lesson-id renumber (102–121 → 132–151, +30) that fixes the
 * C1/C2 id collision.
 *
 * WHY THIS EXISTS
 * ---------------
 ...
 * Korean C2 audio is already live in the public `room-audio` bucket under
 * `c2/ko/l102 … c2/ko/l121`. After the renumber the app asks for
 * `c2/ko/l132 … c2/ko/l151`, so without this move every C2 lesson goes
 * silent (404 → silent local fallback, no crash). This script moves each
 * object `c2/ko/l{n}/*` → `c2/ko/l{n+30}/*` for n ∈ 102..121.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/rekey-korean-c2-audio.ts            # preview
 *   npx tsx scripts/rekey-korean-c2-audio.ts --apply    # execute
 *
...
/** List the file objects directly under `c2/ko/l{id}/`. */
async function listLessonFiles(id: number): Promise<string[]> {
  const prefix = `c2/ko/l${id}`;
...
async function main() {
  console.log(
    `[rekey-korean-c2-audio] bucket=${BUCKET} mode=${APPLY ? "APPLY" : "DRY-RUN"}`,
  );
  ...
  for (const oldId of OLD_IDS) {
    ...
    for (const name of srcFiles) {
      const from = `c2/ko/l${oldId}/${name}`;
      const to = `c2/ko/l${newId}/${name}`;
