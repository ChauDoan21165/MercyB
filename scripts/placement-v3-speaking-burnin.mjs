...
export function parseBurninArgs(argv) {
  const options = {
    ...
    json: false,
    ciStress: false,
    emitDebugArtifacts: false,
    debugDir: ".tmp/placement-v3-speaking-burnin",
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (key === "ci-stress") options.ciStress = true;
    else if (key === "emit-debug-artifacts") options.emitDebugArtifacts = true;
    else if (key === "debug-dir") options.debugDir = value || options.debugDir;
...
export function aggregateBurninResults(results, skippedValidations = [], debugTrace = []) {
  ...
  const summary = {
    ...
    deterministicSummaryHash: stableHash({
      acceptedRunIds: accepted.map((result) => result.runId),
      counts: {
        iterationsAttempted: results.length,
        iterationsAccepted: accepted.length,
        iterationsSucceeded: accepted.filter((result) => result.ok).length,
  ...
  if (debugTrace.length) {
    summary.debugArtifactPreview = {
      traceEntries: debugTrace.length,
      traceHash: stableHash(debugTrace),
    };
  }
  return summary;
}

async function runWorker(queue, options, output, debugTrace, workerId) {
  while (queue.length) {
    ...
    debugTrace[index] = {
      ...
      runId,
      scenario,
      mode: output[index].mode,
      ok: output[index].ok,
      fallback: output[index].fallback,
      errorCode: output[index].errorCode ?? null,
      latencyMs: output[index].latencyMs,
    };
  }
}

async function runBurninOnce(options) {
  ...
  const debugTrace = new Array(options.iterations);
  const workers = Array.from(
    { length: Math.min(options.concurrency, options.iterations) },
    (_, workerId) => runWorker(queue, options, output, debugTrace, workerId),
  );
  await Promise.all(workers);
  return {
    summary: aggregateBurninResults(output, skippedValidations, debugTrace),
    debugTrace,
  };
}

function writeDebugArtifacts(options, payload) {
  const debugDir = path.resolve(repoRoot, options.debugDir);
  ...
  const artifact = {
    ...
    options: {
      ...
      ciStress: options.ciStress,
      simulateTimeoutRate: options.simulateTimeoutRate,
      simulateMalformedRate: options.simulateMalformedRate,
    },
    ...payload,
  };
  const target = path.join(debugDir, "latest.json");
...
export async function runBurnin(options) {
  if (options.ciStress) {
    const stressOptions = { ...options, ciStress: false };
    const runs = [];
    for (let replay = 0; replay < 3; replay += 1) {
      const run = await runBurninOnce(stressOptions);
      runs.push({
        replay,
        summary: run.summary,
        debugTrace: run.debugTrace,
        replayHash: stableHash({ summary: run.summary, debugTrace: run.debugTrace }),
      });
    }
    const [first, ...rest] = runs;
    const stable = rest.every((run) => run.replayHash === first.replayHash);
    const summary = {
      ...first.summary,
      ciStress: {
        enabled: true,
        ...
        stable,
        replayHashes: runs.map((run) => run.replayHash),
        deterministicReplayHash: stableHash(runs.map((run) => run.replayHash)),
      },
      blockingFailures: [
        ...first.summary.blockingFailures,
        ...(stable ? [] : [{ runId: "ci-stress", failure: "aggregation_drift" }]),
      ],
    };
    if (options.emitDebugArtifacts) {
      summary.debugArtifact = writeDebugArtifacts(options, {
        ciStress: summary.ciStress,
        replaySummaries: runs.map((run) => run.summary),
        debugTrace: first.debugTrace,
      });
    }
    return summary;
  }

  const run = await runBurninOnce(options);
  if (run.summary.refused) return run.summary;
  if (options.emitDebugArtifacts) {
    run.summary.debugArtifact = writeDebugArtifacts(options, {
      summary: run.summary,
      debugTrace: run.debugTrace,
    });
  }
...
function printHuman(summary) {
  ...
  if (summary.ciStress) console.log(`ci stress stable: ${summary.ciStress.stable}`);
  if (summary.debugArtifact) console.log(`debug artifact: ${summary.debugArtifact}`);
