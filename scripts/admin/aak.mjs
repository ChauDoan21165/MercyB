...
const ownershipPolicy = [
  { pattern: /replay/i, owner_team: "C4" },
  { pattern: /teacher.?mercy/i, owner_team: "C2" },
  { pattern: /coverage/i, owner_team: "C3" },
  { pattern: /judge|decision/i, owner_team: "Admin" },
  { pattern: /eipc|oii|governance|admin/i, owner_team: "Admin" },
  { pattern: /capability/i, owner_team: "Admin" },
];

const lifecycleTransitions = {
  "PLANNING_APPROVED->ENGINEERING_VALIDATED": ["validation_report_ref", "execution_record_ref", "rollback_ref"],
  "DRAFT->PLANNING_APPROVED": ["owner_team"],
  "ENGINEERING_VALIDATED->JUDGE_REVIEWED": ["judge_package_ref", "evidence_bundle_ref"],
};
...
function validateLifecycleCommand(args) {
  ...
  const requiredArtifacts = cloneList(lifecycleTransitions[transitionKey]);
  ...
  if (!transition.from || !transition.to) errors.push({ field: "transition", reason: "missing_required" });
  if (!lifecycleTransitions[transitionKey]) {
    errors.push({ field: "transition", reason: "invalid_transition", allowed: Object.keys(lifecycleTransitions).sort() });
  }

  for (const field of requiredArtifacts) {
    if (!execution[field]) errors.push({ field, reason: "missing_required_for_transition", transition: transitionKey });
  }
...
function expectedOwnerForSubject(subjectType) {
  ...
  for (const entry of ownershipPolicy) {
    if (entry.pattern.test(normalized)) return entry.owner_team;
  }
  return "Admin";
}
