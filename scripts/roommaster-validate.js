...
// Without this list, registry.json (auto-generated manifest) and a
// handful of historical config files were tripping the room schema
// check and producing a CI-blocking false positive on every run.
// See issue #58 for the regression that led to adding the list here.
const IGNORE_FILES = [
  '.gitkeep',
  'Tiers.json',
  'Tiers_.json',
...
async function main() {
  ...
  // Load all JSON files (skipping config + auto-generated entries).
  const files = fs
