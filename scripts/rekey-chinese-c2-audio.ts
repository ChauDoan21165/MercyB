/**
 * scripts/rekey-chinese-c2-audio.ts
 *
 * One-shot Supabase Storage re-key for Chinese C2 lesson audio, paired
 * with the C2 lesson-id renumber (102–121 → 142–161, +40) that fixes the
 * C1/C2 id collision. Mirrors scripts/rekey-korean-c2-audio.ts (#528) /
 * scripts/rekey-japanese-c2-audio.ts; only the lang token and the +40
 * offset differ (Chinese C1 max id is 141, so C2 starts at 142).
 *
 * WHY THIS EXISTS
 * ---------------
 * `lessonAudioBase(lang, id, level)` derives the storage path as
 * `${level}/${lang}/l${id}` with NO id→slug remap layer for numeric ids.
 * Chinese C2 audio is already live in the public `room-audio` bucket
 * under `c2/zh/l102 … c2/zh/l121`. After the renumber the app asks for
 * `c2/zh/l142 … c2/zh/l161`, so without this move every C2 lesson goes
 * silent (404 → silent local fallback, no crash). This script moves each
 * object `c2/zh/l{n}/*` → `c2/zh/l{n+40}/*` for n ∈ 102..121.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/rekey-chinese-c2-audio.ts            # preview
 *   npx tsx scripts/rekey-chinese-c2-audio.ts --apply    # execute
 *
...
const OFFSET = 40; // 102→142 … 121→161 — must match the lessons-c2.ts renumber
const OLD_IDS = Array.from({ length: 20 }, (_, i) => 102 + i); // 102..121

// ── Helpers ─────────────────────────────────────────────────────────────

/** List the file objects directly under `c2/zh/l{id}/`. */
async function listLessonFiles(id: number): Promise<string[]> {
  const prefix = `c2/zh/l${id}`;
...
async function main() {
  console.log(
    `[rekey-chinese-c2-audio] bucket=${BUCKET} mode=${APPLY ? "APPLY" : "DRY-RUN"}`,
  );
  ...
  for (const oldId of OLD_IDS) {
    ...
    for (const name of srcFiles) {
      const from = `c2/zh/l${oldId}/${name}`;
      const to = `c2/zh/l${newId}/${name}`;
