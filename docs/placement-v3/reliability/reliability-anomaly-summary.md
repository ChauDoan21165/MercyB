# Placement V3 Reliability Anomaly Summary

Generated: 2026-05-20T15:34:49.766Z

- Anomaly classification: ANOMALY
- Escalation priority: ELEVATED
- Abnormal runtime spikes: 1
- Timeout spread increase: 43s
- Repeated contention anomalies: 1
- Rerun instability detected: false
- False-positive detector anomalies: 1
- Flaky-pattern clusters: 0
- Confidence adjustment: reduce_to_contention_caution

B1 anomaly detection is reliability intelligence only. Placement V3 remains disabled and enablement remains blocked.
