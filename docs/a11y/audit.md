...
7. **Moderate — bilingual `<lang>` wrapper:** introduce `<Bilingual>` component + migrate WeakAt + Onboarding + Home cards. ~30 lines new, ~50 lines edited. **Partial — Onboarding (O2) shipped 2026-05-27** in `fix/a11y-next-serious-finding`; **WeakAt (W2) shipped 2026-05-27** in `fix/a11y-weakat-taxonomy-lang`. **Still pending:** Home cards (H2-adjacent) and **Pricing VI text (P4)**. No shared `<Bilingual>` wrapper yet — each surface fixed inline first to avoid a half-finished abstraction; once P4 lands, the inline pattern is repeated enough times (3) that extracting a `<Bilingual>` helper is the natural next step.
