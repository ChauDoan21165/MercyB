...
7. **Moderate — bilingual `<lang>` wrapper:** introduce `<Bilingual>` component + migrate WeakAt + Onboarding + Home cards. ~30 lines new, ~50 lines edited. **Partial — Onboarding native peer header shipped 2026-05-27** in `fix/a11y-next-serious-finding` (O2). WeakAt (W2), Home cards (H2-adjacent), and Pricing (P4) still pending; no shared `<Bilingual>` wrapper yet — each surface fixed inline first to avoid a half-finished abstraction.
