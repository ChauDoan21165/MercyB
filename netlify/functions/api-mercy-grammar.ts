...
import { chatJsonWithFailover } from "./_shared/aiProvider";
import {
  asString,
  envValue,
  json,
...
async function isL1FlagEnabled(userId: string): Promise<boolean> {
  ...
  try {
    ...
    console.warn("[netlify:grammar] feature flag lookup failed", err);
    return false;
  }
}
...
export async function handler(event: NetlifyEvent) {
  ...
  try {
    const body = readJsonBody<GrammarBody>(event);
    ...
    if (!result.ok) {
      ...
      return json({ ok: false, correctedText: text, feedback });
    }
    ...
    try {
      if (userId && await isL1FlagEnabled(userId)) {
        l1Hint = firstL1HintFromIssues(
          issues as Array<{ original?: string; corrected?: string }>,
        );
      }
    } catch (err) {
      console.warn("[netlify:grammar] L1 hint failed", err);
      l1Hint = null;
    }
    ...
    console.error("[netlify:grammar] unexpected error", err);
    return json({
      ok: false,
