...
-- Clear existing kids data (guarded for fresh replays)
DO $$
BEGIN
  IF to_regclass('public.kids_entries') IS NOT NULL THEN
    EXECUTE 'TRUNCATE TABLE public.kids_entries CASCADE';
  END IF;

  IF to_regclass('public.kids_rooms') IS NOT NULL THEN
    EXECUTE 'TRUNCATE TABLE public.kids_rooms CASCADE';
  END IF;

  IF to_regclass('public.kids_levels') IS NOT NULL THEN
...
-- (LEVEL 2 + LEVEL 3 INSERTS REMAIN UNCHANGED)
