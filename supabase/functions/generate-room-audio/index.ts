...
interface GenerateAudioRequest {
  slug: string;
  kind: 'content' | 'reflection' | 'dare' | 'welcome' | 'intro';
  language: 'en' | 'vi';
  text: string;
  filename: string;
...
serve(async (req) => {
  ...
  try {
    ...
    if (!expectedToken) {
      console.error('[generate-room-audio] MB_ADMIN_TOKEN not configured');
      return new Response(
    ...
    const { slug, kind, language, text, filename, voice } = body;

    // Validate required fields
    if (!slug || !kind || !language || !text || !filename) {
      return new Response(
        JSON.stringify({ ok: false, error: 'Missing required fields: slug, kind, language, text, filename' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`[generate-room-audio] Generating: slug=${slug}, kind=${kind}, lang=${language}, filename=${filename}`);

    // Generate TTS audio
    const selectedVoice = voice || selectVoiceForLanguage(language);
    ...
    // Determine storage path based on kind
    let storagePath: string;
    if (kind === 'welcome') {
      storagePath = `welcome/${filename}`;
    } else {
      storagePath = `paths/${filename}`;
    }
    ...
    if (uploadError) {
      console.error('[generate-room-audio] Storage upload error:', uploadError);
      return new Response(
    ...
    console.log(`[generate-room-audio] Uploaded successfully: ${publicUrl}`);
    ...
    console.error('[generate-room-audio] Error:', error);
    return new Response(
