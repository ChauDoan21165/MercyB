...
export function createPlacementV3Harness(options: HarnessOptions = {}) {
  ...
  const deps: CoreDeps = {
    ...
    markProfilesNotCurrent: async (userId) => {
      for (const [k, profile] of profiles) {
        if (profile.user_id === userId) profiles.set(k, { ...profile, is_current: false });
      }
    },
...
export type PlacementV3Harness = ReturnType<typeof createPlacementV3Harness>;
export type PlacementV3HarnessOptions = HarnessOptions;
