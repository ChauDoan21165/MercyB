...
//
// Pure entitlement-normalization logic for the `me-entitlement` edge
// function. Extracted verbatim from index.ts (PR: me-entitlement test
// backfill) so it can be unit-tested under vitest — index.ts itself
// can't be imported (top-level esm.sh import + Deno.serve). This module
// is intentionally esm.sh-free, exactly like revenuecat-webhook/auth.ts
// and stripe-webhook/idempotency.ts, so `vitest` can import it.
//
// ZERO behavior change vs the previous inline implementation. index.ts
// now imports from here; the runtime path is identical.
...
export function getSortTimestamp(row: SubscriptionRow): number {
  const candidates = [
    row.updated_at,
...
export function normalizeStatus(row: SubscriptionRow): CanonicalStatus {
  const raw = (
...
export function compareRows(a: SubscriptionRow, b: SubscriptionRow): number {
...
export function normalizeEntitlement(rows: SubscriptionRow[]) {
  const best = [...rows].sort(compareRows)[0];

  if (!best) {
    return {
      is_premium: false,
      source: null as CanonicalSource,
