/**
 * Path: src/lib/mercy-host/talkBudget.ts
 * File: talkBudget.ts
 *
 ...
 * - level0 ... level9
 *
 * Also tolerates common display / legacy forms:
 * - "Level 1", "level 1", "LEVEL_1"
 * - "free" -> "level0"
 * - "vip1" ... "vip9" -> "level1" ... "level9"
 */
export function normalizeTierId(tier: string | null | undefined): TierId {
  ...
  if (levelMatch) {
    return `level${levelMatch[1]}` as TierId;
  }

  const vipMatch = compact.match(/^vip([1-9])$/);
  if (vipMatch) {
    return `level${vipMatch[1]}` as TierId;
  }
 ...
 */
export function wouldExceedCrisisOverrideCap(
  usage: TalkUsage,
  chars: number,
  budget: TalkBudget
): boolean {
 ...
 */
export function getTalkProgress(
  usage: TalkUsage,
  budget: TalkBudget
): number {
  const currentUsage = resetTalkUsageIfNewDay(usage);
  return Math.min(
    100,
    Math.round((currentUsage.usedChars / budget.dailyChars) * 100)
  );
}
