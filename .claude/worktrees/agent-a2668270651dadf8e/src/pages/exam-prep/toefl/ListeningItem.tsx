...
export default function ListeningItem() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-6">
      <Link to="/exam-prep/toefl/listening" className="inline-flex items-center gap-1 text-xs text-slate-500 hover:text-slate-800">
      ...
      <header className="mt-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] uppercase tracking-wider text-slate-500">Band {item.difficulty_band} · {item.estimated_time_minutes} min</span>
        </div>
      ...
      <section className="mt-4 space-y-2">
        <h2 className="text-xs font-bold uppercase tracking-wider text-slate-500">Câu hỏi · Questions</h2>
      ...
      {item.vocabulary_focus.length > 0 && (
        <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <h2 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-500">Từ vựng trọng tâm · Key vocabulary</h2>
          <table className="w-full text-xs"><thead className="text-[10px] uppercase text-slate-500"><tr><th className="text-left">Từ</th><th className="text-left">Nghĩa</th><th className="text-left">IPA</th><th className="text-left">Band</th></tr></thead>
            <tbody>{item.vocabulary_focus.map((v, i) => (<tr key={i} className="border-t border-slate-100"><td className="py-1 font-semibold">{v.word}</td><td className="py-1">{v.vi_translation}</td><td className="py-1 font-mono">{v.ipa}</td><td className="py-1 text-slate-500">{v.band_level}</td></tr>))}</tbody></table>
        </section>
      )}

      {item.vietnamese_speaker_strategies.length > 0 && (
        <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <h2 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-500">Mẹo cho người Việt · VN-speaker strategies</h2>
