...
### As of May 27, 2026 — even later (bilingual copy audit + VI style guide landed)

Diagnostic-only pass on user-facing Vietnamese strings. Two docs landed at `docs/copy/`: `bilingual-audit.md` (the per-string catalog) and `vi-style-guide.md` (the prescriptive companion to the existing `docs/voice-guidelines-vn.md`, which remains authoritative for streak/leaderboard/progress shame). Scope is **UI chrome strings only** — pedagogical content in `src/data/**` and `src/languages/**` is explicitly out of scope (governed by the separate Northern-VI sweep series, `project_vi_sweep_policies`). Within chrome scope, Tier 1 surfaces (MarketingLandingPage, Home, WeakAt, Stage 3B UI + engine strings) were catalogued line-by-line — 27/27 marketing strings OK, all Stage 3B strings OK (post-C5's `chore/stage-3b-vi-copy-audit` revision in `!24`), WeakAt's *"Điểm yếu của bạn"* h1 flagged as borderline-but-OK because its EN companion *"What you're working on"* deliberately reframes; Tier 2 (the canonical `stage-3a/taxonomy.ts` ~280-string bilingual catalog plus `teacher-mercy/tierScripts.ts` level0 Mercy-as-companion voice exemplars) sampled densely — no shame triggers, no MT tells, systematically on-voice; Tier 3 (~2,400 VI lines across ~80 unaudited files including `AccountPage.tsx`, `LoginPage.tsx`, `Tiers.tsx`, `AiTutor.tsx`, `mercy-guide/`, `onboarding/`) catalogued at file-level with per-file priority for a future Phase-2 sweep wave. The style guide names what makes the voice work (first-person "mình" for Mercy, second-person "bạn" for the learner, end particles "nhé/nha" sparingly, "hay" as the frequency softener, passive depersonalization for errors, concrete imagery over abstraction) and what breaks it (bureaucratic "chúng tôi/quý khách/quý vị", brand-speak agentive verbs, "Tuyệt vời!" exclamation-inflation, "Cố lên!" as default encouragement, diminutives for adult learners, MT lexical/syntactic/register tells). Bilingual pairing patterns are codified: VI primary visually dominant, EN secondary serves one of three jobs (identity signal / deliberate reframe / diaspora safety net) — never a translation aid for VI-natives; write VI first, never EN-first-then-translate. **No source strings changed in this MR** — diagnostic only; revisions queued for per-priority-file Phase-2 MRs rather than one giant sweep. No §15 ticks change; the audit confirms the marketing-visible + first-impression surfaces are on-voice today and gives Phase 2 a prioritized work plan.

### As of May 27, 2026 — later (Stage 3B engine + UI shipped, marketing spec landed)

Stage 3B *Suggested Practice* closed end-to-end on `main` the same day Stage 3A's Day 4 did. Two MRs landed: the engine (`src/stage-3b/suggestedPractice.ts` + `types.ts` — a pure, deterministic function that ranks at most one item per kind across L1 → placement → pronunciation, sourced from the Stage 3A aggregator, with labels resolved through `stage-3a/taxonomy.ts` so no engineer-tags leak to the user surface), then the UI (`src/components/stage-3b/SuggestedPracticeList.tsx` — Vietnamese-first card list, mobile-first 375 px, kind chips for *Ngữ pháp / Trình độ / Phát âm*, calm empty state) wired into `/weak-at` below `LocalWeaknessMap`. Hard invariants inherited from Stage 3A hold: no Supabase, no fetch, no `mercy_user_facts`, no `localStorage.setItem` from this surface (read-only via the aggregator's existing seam). The row `onClick` is a `console.log` stub — the practice handoff is the next brick, intentionally deferred so this wave shipped a prescriptive surface, not a routing demo. The companion marketing-screenshot spec landed at `docs/stage-3b/marketing-screenshot-spec.md` — mirrors the Stage 3A doc's structure (DevTools seed snippet, 375 × 812 viewport, scroll instruction, bilingual framing copy, per-platform posting checklist, "what this is NOT" section) but tunes the seed for diverse signals across all three kinds so the suggestion card reads as three distinct domains, not as a *th* cluster. The canonical Stage 3B framing copy: *"Duolingo nhắc bạn luyện đều mỗi ngày. MercyBlade nói rõ ba điều người Việt nên luyện tiếp theo."* / *"Duolingo tells you to keep practicing. MercyBlade tells Vietnamese learners exactly what to practice next."* — the *what* over the *that*, paired but not redundant with the Stage 3A diagnostic line. The Stage 3A spec gained a cross-reference back to the 3B companion so the two pair cleanly when a thread or carousel benefits from both. No §15 ticks change in this pass — Bar #7 (named Vietnamese learner outcome) stays explicitly open; the prescriptive surface is a positioning input, not an outcome. Next brick: wire the row `onClick` from stub to a real focused-drill handoff per the Stage 3B routing design (ROADMAP §3B follow-up).

### As of May 27, 2026 (Stage 3A shipped — Day 4 closed)

Day 4 of the Stage 3A campaign closed: the `/weak-at` route, the `LocalWeaknessMap` UI component, and lightweight performance instrumentation around the aggregator are all on `main` (three new GitLab-native MRs merged 2026-05-27 — the component, the route + Home entry, and the perf instrumentation). The route is reachable from `Home` via the `home-weak-at-link` entry; rendering reads only the three `mb.stage3a.*` `localStorage` keys (no Supabase, no analytics, no `mercy_user_facts`). Section-level emptiness collapses cleanly; the global empty state fires only when all three buckets are empty. Vietnamese is primary throughout — section titles and the example strings come from the learner-language taxonomy (`src/lib/stage-3a/taxonomy.ts`). Bilingual page title on `/weak-at`: *Điểm yếu của bạn / What you're working on*. The marketing-visible demo artifact specification lives at `docs/stage-3a/marketing-screenshot-spec.md` — Chau captures the actual asset on a real device; the spec encodes the seed data, viewport (375 × 812), framing copy (the canonical Duolingo-contrast line in both VI and EN), and per-platform posting checklist for TikTok / Facebook / Zalo. The E2E + invariant-guard test pass is still pending in a separate MR; it strengthens audit confidence but does not gate the launch artifact, which captures against a developer-seeded device store. No §15 ticks change in this pass — Axis 1 Bar #7 (named Vietnamese learner outcome) stays explicitly open until a real testimonial lands; the launch screenshot is a positioning input, not an outcome. Next brick: Stage 3B Suggested Practice — soft suggestions, `(c+)` trigger semantics — fully specified in ROADMAP §3B, ready to author.
