-- ============================================================================
-- A28 — Remediation: gift-code silent-failure VICTIMS — CORRECTED / PASTE-READY
-- ============================================================================
--   This file is reports/REMEDIATION-gift-victim-repair-A28.sql
--   (branch a28/gift-victim-repair-sql, commit 4d5e75112) reproduced
--   VERBATIM except for TWO mechanical CTE-scope fixes. The repair LOGIC,
--   every safety guard, every GATE step, and every comment are byte-
--   identical to A28. Diff this against the original: the ONLY changes are
--   two inserted `WITH …` prefixes (marked « CORRECTION 1 » / « CORRECTION
--   2 » below). No logic changed.
--
--   WHY THIS FILE EXISTS — A28 as committed is NOT runnable through its own
--   GATE. Postgres CTEs are STATEMENT-scoped: a `WITH … AS (…)` binds only
--   to the single `;`-terminated statement that immediately follows it. In
--   A28:
--     • PART A's INSERT-class preview SELECT (`FROM victims_insert v`) is a
--       SEPARATE statement after the top `WITH`'s first SELECT ⇒ at run time
--       it raises `ERROR: relation "victims_insert" does not exist`. This is
--       GATE step 2's INSERT-class confirmation — the operator cannot clear
--       GATE 2.
--     • PART D's D2 GUARD SELECT references `victims_all` but is a SEPARATE
--       statement after D1 ⇒ same `relation "victims_all" does not exist`
--       error. D2 is the scope-leak proof (no user_id outside the
--       transcribed set changed) — GATE step 3 cannot be cleared.
--   Both errors are in READ/VERIFY sections (they fail loud, write nothing),
--   so A28 is not *dangerous* — it is *unfinishable*: a disciplined operator
--   following the GATE hits an error and correctly stops, and the historical
--   victims stay unrepaired. This file makes the GATE actually completable.
--
--   CONSEQUENCE FOR THE OPERATOR — the victim VALUES list must now be
--   transcribed into **6** identical sites (A28 implied 4). Numbered below:
--     ① PART A  top `WITH victims_update, victims_insert`  (UPDATE-class preview)
--     ② PART A  « CORRECTION 1 » `WITH victims_insert`     (INSERT-class preview)
--     ③ PART B1 `WITH victims_update`                      (the UPDATE write)
--     ④ PART B2 `WITH victims_insert`                      (the INSERT write)
--     ⑤ PART D  top `WITH victims_all`                     (D1 success check)
--     ⑥ PART D  « CORRECTION 2 » `WITH victims_all`        (D2 scope-leak guard)
--   Transcribe ONCE, then copy the identical VALUES rows into all 6. The
--   #1 operator-error risk in this whole apply is these 6 lists drifting
--   out of sync — fill them in one pass, then re-read all 6 side by side
--   before running. (sites ①③④⑤ are A28's; ②⑥ are the corrections.)
--
--   COLUMN-SHAPE CONTRACT — VERIFIED. A28 requires, per victim row:
--     user_id           ← A13 Q1 column `user_id`                  ✓ emitted
--     redeemed_at        ← A13 Q1 column `redeemed_at`              ✓ emitted
--     intended_tier_id   ← A13 Q1 column `intended_tier_id_besteffort` ✓ emitted
--     class selector     ← A13 Q1 column `failure_mode`             ✓ emitted
--   The two failure_mode literals A28's GATE keys on —
--     'is_gift_redemption=false (THE #787 fingerprint)'  → victims_update
--     'no_subscription_row (deleted/never persisted)'    → victims_insert
--   are produced VERBATIM by A13 Query 1's CASE expression. No rename, no
--   mismatch. The contract is sound; only the CTE scoping was broken.
--
--   STATUS:    HANDOFF ONLY. NOT EXECUTED. SUPERSEDES
--              reports/REMEDIATION-gift-victim-repair-A28.sql for the APPLY
--              (that file's GATE cannot be cleared as written). Same apply
--              discipline: Supabase SQL Editor, ONCE, human-reviewed, after
--              a fresh A13 Q1 run is transcribed into all 6 sites and the
--              GATE is cleared. Never `supabase db push`. Never an agent.
--              There is no unattended SQL path to this Supabase (CLAUDE.md →
--              Supabase; memory project_db_schema_drift_audit /
--              project_578_rls_applied / project_agent_infra_access).
--
...
--   │    Put EACH real row into ALL 6 sites ①–⑥ (see header). Delete    │
--   │    the all-zero SENTINEL row from a list ONLY after you have       │
--   │    added ≥1 real row to that list (in every site it appears). A   │
--   │    list left at the sentinel is a safe 0-row no-op (do not delete  │
--   │    the sentinel if that class has no victims).                    │
...
--   victim row is transcribed into all 6 sites from a fresh A13 Q1 run and
--   the GATE is cleared. Never `db push`. Never an agent.
-- ============================================================================
--
...
--
-- ▶ SITE ① (PART A · UPDATE-class preview) — and the unused victims_insert
--   here is harmless; the INSERT-class preview now has its own at SITE ②.
-- ---------------------------------------------------------------------------
WITH
...
-- ┌── « CORRECTION 1 » ▶ SITE ② — PART A INSERT-class preview ───────────────┐
-- │ A28 BUG: the SELECT below was a separate statement after the top WITH's │
-- │ first SELECT, so `victims_insert` was OUT OF SCOPE ⇒ ERROR: relation    │
-- │ "victims_insert" does not exist (GATE step 2 INSERT-class confirmation  │
-- │ uncompletable). FIX: give this statement its OWN identical WITH. Keep   │
-- │ the VALUES rows BYTE-IDENTICAL to SITE ① victims_insert and SITE ④.     │
-- └────────────────────────────────────────────────────────────────────────┘
WITH
victims_insert (user_id, redeemed_at, intended_tier_id) AS (
  VALUES
    -- SENTINEL — KEEP IDENTICAL to SITE ① victims_insert / SITE ④.
    ( '00000000-0000-0000-0000-000000000000'::uuid,
      '1970-01-01T00:00:00+00:00'::timestamptz,
      '00000000-0000-0000-0000-000000000000'::uuid )
    -- , ( '‹victim user_id›'::uuid,
...
--
-- ▶ SITE ③ — KEEP victims_update IDENTICAL to SITE ①.
WITH
victims_update (user_id, redeemed_at, intended_tier_id) AS (
  VALUES
    ( '00000000-0000-0000-0000-000000000000'::uuid,
      '1970-01-01T00:00:00+00:00'::timestamptz,
      '00000000-0000-0000-0000-000000000000'::uuid )
    -- , (... real victims_update rows — KEEP IDENTICAL to PART A SITE ① ...)
)
...
--
-- ▶ SITE ④ — KEEP victims_insert IDENTICAL to SITES ① and ②.
WITH
victims_insert (user_id, redeemed_at, intended_tier_id) AS (
  VALUES
    ( '00000000-0000-0000-0000-000000000000'::uuid,
      '1970-01-01T00:00:00+00:00'::timestamptz,
      '00000000-0000-0000-0000-000000000000'::uuid )
    -- , (... real victims_insert rows — KEEP IDENTICAL to PART A SITES ①/② ...)
)
...
--
-- ▶ SITE ⑤ — victims_all = the UNION of every transcribed user_id from
--   BOTH classes (victims_update ∪ victims_insert). KEEP IDENTICAL to the
--   user_id column of SITES ①–④, and to SITE ⑥.
WITH
...
-- ┌── « CORRECTION 2 » ▶ SITE ⑥ — PART D2 GUARD ────────────────────────────┐
-- │ A28 BUG: D2 referenced `victims_all` but was a separate statement after │
-- │ D1, so the WITH above was OUT OF SCOPE ⇒ ERROR: relation "victims_all"  │
-- │ does not exist. D2 is the scope-leak proof (GATE step 3) — without it   │
-- │ the operator cannot prove the write touched ONLY the transcribed users. │
-- │ FIX: give D2 its OWN identical WITH victims_all. KEEP byte-identical to │
-- │ SITE ⑤.                                                                 │
-- └────────────────────────────────────────────────────────────────────────┘
WITH
victims_all (user_id) AS (
  VALUES ( '00000000-0000-0000-0000-000000000000'::uuid )
  -- , ('‹every transcribed victim user_id, update + insert classes — IDENTICAL to SITE ⑤›'::uuid)
)
...
-- victims-A13.md + the operator runbook
-- reports/RUNBOOK-gift-victim-sweep-OPERATOR.md (send the make-good only
-- once this is applied so the promise is true).
-- ============================================================================
