...
> **A2 annotation (2026-05-19, additive — original content unchanged):**
> B66's closeout marked the roll-forward 🔴 blocked ("SQL lost, must be
> regenerated under human review"). A2 regenerated
> `reports/REMEDIATION-mylinh-paid-but-free-B5.sql` (branch
> `b5/mylinh-sql-regen`) to B29 convention, structurally templated on
> `REMEDIATION-price-map-repair-B64.sql`. It supersedes the B57 skeleton:
> psql `:name` binds (non-functional in the Supabase SQL Editor) replaced
> with inline hand-edit placeholders + a 4-step GATE; the conflated
> `subscriptions.status` / `profiles.premium_status` placeholder split —
> `subscriptions.status` is now NOT written (smallest safe diff; recon's
> only proven defect is the stale period), and `profiles` is re-granted by
> mirroring the live `deriveEntitlementFromSubscriptions` /
> `recomputeAndPersistEntitlement` path (core.ts/billing.ts), not a guess.
> Two NOT-RECOVERABLE values remain operator-resolved: the
> `subscriptions.id` PK (from PREVIEW A1) and the live Stripe
> `current_period_end` (Stripe Dashboard; evidence cross-check
> 2026-06-09T06:16:41Z). Still HANDOFF ONLY, NOT applied; SQL-Editor
> human-reviewed path only. The "do not reconstruct/run B5's *verbatim*
> SQL" guidance below still stands — the regenerated file is a reviewable
> spec derived from committed evidence, not that lost text.
